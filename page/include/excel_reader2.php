	...
	function _parsesheet($spos) {
		...
		function _getCellDetails($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				$utcDays = round($numValue - ($this->nineteenFour ? SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS));
				$utcValue = ($utcDays) * SPREADSHEET_EXCEL_READER_MSINADAY;
				$dateinfo = gmgetdate($utcValue);

				$raw = $numValue;
				$fractionalDay = $numValue - round($numValue) + .0000001; // The .0000001 is to fix for php/excel fractional diffs

				$totalseconds = round(SPREADSHEET_EXCEL_READER_MSINADAY * $fractionalDay);
				$secs = $totalseconds % 60;
				$totalseconds -= $secs;
				$hours = round($totalseconds / (60 * 60));
				$mins = round($totalseconds / 60) % 60;
