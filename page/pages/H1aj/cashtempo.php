									...
									<table id="example1" class="table table-striped table-hover" style="width: 130%">
										...
										if($_SESSION[posisi]=='DIREKSI')
											...
											if(!empty($_REQUEST[cari]))
												{
												$q1 = mysql_query("SELECT * FROM tbl_notajual_vw WHERE id%2=0 AND id%2=0 AND jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER' AND (nama LIKE '%$_REQUEST[cari]%' OR tglnota LIKE '%$_REQUEST[cari]%' OR nonota LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
												}
											else
												{
												$q1 = mysql_query("SELECT * FROM tbl_notajual_vw WHERE id%2=0 AND id%2=0 AND jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER' AND tglpelunasan='0000-00-00' ORDER BY id DESC LIMIT 0,20");
												}
											}
											...
											if(!empty($_REQUEST[cari]))
												{
												$q1 = mysql_query("SELECT * FROM tbl_notajual_vw WHERE id%2=0 AND id%2=0 AND idsales='$_SESSION[id]' AND jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER' AND (nama LIKE '%$_REQUEST[cari]%' OR tglnota LIKE '%$_REQUEST[cari]%' OR nonota LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
												}
											else
												{
												$q1 = mysql_query("SELECT * FROM tbl_notajual_vw WHERE id%2=0 AND id%2=0 AND idsales='$_SESSION[id]' AND jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER' AND tglpelunasan='0000-00-00' ORDER BY id DESC LIMIT 0,20");
												}
											}
					            ...
					            <form name="inputpelanggan" onsubmit="return validA();" method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=saveB"?>" enctype="multipart/form-data">
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
				                    	...
				                    	<table width="70%">
											...
											<?
											$dTp = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_history_bcashtempo WHERE id%2=0 AND nonota='$dB[nonota]'"));
											?>
				                    		<tr>
