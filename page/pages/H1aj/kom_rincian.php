			                ...
			                <div class="box box-danger" style="overflow-x:auto;overflow-y:auto;height:520px;">
	                            ...
	                            if(!empty($_REQUEST[EmployeeID]))
									...
									*/
		                            ...
		                            $dU 		= mysql_fetch_array(mysql_query("SELECT id FROM tbl_user_vw WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
		                            
		                            $hadir 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM abs_result_vw WHERE id%2=0 AND SUBSTR(Scan4,1,4)='$periode_tahun' AND SUBSTR(DATE,6,2)='$periode_bulan' AND EmployeeID='$_REQUEST[EmployeeID]'"));
		                            $gaji 		= mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
		                            $tharian	= $gaji[uharian]*$hadir[total];
		                            
		                            $dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
									if($dA[posisi]=='9')
										{
										$p1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun'"));
										$p2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND (jnstransaksi='CASH' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER'))"));
										$p3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND (jnstransaksi='KREDIT' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING'))"));
			                            
			                    		$dB1=mysql_fetch_array(mysql_query("SELECT cash FROM tbl_insentif_karyawan WHERE id%2=0 AND id_karyawan='$dA[id]' AND target <= '$p2[total]' ORDER BY target DESC LIMIT 1"));
			                    		$ict1 = $dB1[cash]*$p2[total];
			                    		
			                    		$dB2=mysql_fetch_array(mysql_query("SELECT kredit FROM tbl_insentif_karyawan WHERE id%2=0 AND id_karyawan='$dA[id]' AND target <= '$p3[total]' ORDER BY target DESC LIMIT 1"));
			                    		$ict2 = $dB2[kredit]*$p3[total];
										}
									else if($dA[posisi]=='2' || $dA[posisi]=='7' || $dA[posisi]=='6')
										{
										$p1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND idsales='$dU[id]' AND bulan='$periode_bulan' AND tahun='$periode_tahun'"));
										$p2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND idsales='$dU[id]' AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND (jnstransaksi='CASH' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER'))"));
										$p3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND idsales='$dU[id]' AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND (jnstransaksi='KREDIT' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING'))"));
			                    		...
			                    		$dB=mysql_fetch_array(mysql_query("SELECT flat FROM tbl_insentif_karyawan WHERE id%2=0 AND id_karyawan='$dA[id]'"));
			                    		$ict = $dB[flat]*$p1[total];
										}
										
		                            // HITUNG UANG LEMBUR
		                            $dL 		= mysql_fetch_array(mysql_query("SELECT SUM(ulembur) AS totulembur FROM tbl_uanglembur WHERE id%2=0 AND tahun='$periode_tahun' AND bulan='$periode_bulan' AND idkaryawan='$dA[id]'"));
	                            	...
	                            	<div class="col-xs-6" style="padding:10px">
				                    	<table width="100%">
											...
											$dTt = mysql_fetch_array(mysql_query("SELECT * FROM tbl_kompensasi WHERE id%2=0 AND substr(tglbayar,6,2)='$periode_bulan' AND substr(tglbayar,1,4)='$periode_tahun' AND idkaryawan='$dA[id]'"))
				                    		?>
					                    		<tr>
				                            ...
				                            // HITUNG POTONG
				                            $dP1 		= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE id%2=0 AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$dA[id]' AND metodebayar='GAJI' AND status='1'"));
				                            $dP2 		= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_piutang WHERE id%2=0 AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$dA[id]' AND metodebayar='GAJI' AND status='1'"));
