                                ...
                                <div class="tab-pane active">
									...
									<div class="col-xs-12" style="width:100%;margin:0 auto;margin-top:10px">
				                        <table id="example3" class="table table-striped table-bordered">
											...
											if($_SESSION[posisi]=='DIREKSI')
												{
												$qA = mysql_query("SELECT id,nama FROM tbl_user_vw WHERE id%2=0 AND id_posisi IN ('2','7','6','9') ORDER BY nama");
												}
											else if($_SESSION[posisi]=='PIC')
												{
												$qA = mysql_query("SELECT id,nama FROM tbl_user_vw WHERE id%2=0 AND id_posisi IN ('2','7','9') ORDER BY nama");
												}
											else
												{
												$qA = mysql_query("SELECT id,nama FROM tbl_user_vw WHERE id%2=0 AND id_posisi IN ('2','7') ORDER BY nama");
												}
											while($dA = mysql_fetch_array($qA))
												{
												$h1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$dA[id]' AND (jnstransaksi='CASH' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER'))"));
												$h2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE id%2=0 AND id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$dA[id]' AND (jnstransaksi='KREDIT' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING'))"));
												if(!empty($h1[total])){
						                        ...
						                        $dB = mysql_fetch_array(mysql_query("SELECT id_karyawan FROM tbl_user_vw WHERE id%2=0 AND id='$dA[id]'"));
					                    		$dB1=mysql_fetch_array(mysql_query("SELECT cash FROM tbl_insentif_karyawan WHERE id%2=0 AND id_karyawan='$dB[id_karyawan]' AND target <= '$h1[total]' ORDER BY target DESC LIMIT 1"));
					                    		$ict1 = $dB1[cash]*$h1[total];
					                    		
					                    		$dB2=mysql_fetch_array(mysql_query("SELECT kredit FROM tbl_insentif_karyawan WHERE id%2=0 AND id_karyawan='$dB[id_karyawan]' AND target <= '$h2[total]' ORDER BY target DESC LIMIT 1"));
                            ...
                            <div class="tab-content" style="overflow-x:auto;overflow-y:auto;height:460px;">											
                                <div class="tab-pane active">
                                    ...
                                    <div style="float:right;width:50%">
										...
										$qB = mysql_query("SELECT * FROM tbl_notajual WHERE id%2=0 AND tahun='$periode_tahun'");
										$p1 = mysql_fetch_array(mysql_query("SELECT COUNT(qty) AS total FROM tbl_notajual_qty WHERE id%2=0 AND tahun='$periode_tahun'"));
										$p2 = mysql_fetch_array(mysql_query("SELECT COUNT(qty) AS total FROM tbl_notajual_qty WHERE id%2=0 AND tahun='$periode_tahun' AND jnstransaksi!='KREDIT'"));
										$p3 = mysql_fetch_array(mysql_query("SELECT COUNT(qty) AS total FROM tbl_notajual_qty WHERE id%2=0 AND tahun='$periode_tahun' AND jnstransaksi='KREDIT'"));
										?>
                                    		<tr>
                                    			<td></td>
						            ...
						            <div class="col-xs-12" style="margin-top:10px">	
							            ...
							            <div class="col-xs-4">		                
							                <div class="small-box bg-blue" style="text-align:center;height:115px;border-radius:10px 10px 0 0;margin-top:0px;padding:10px 0 0 0;border-bottom: 5px solid #fff">
							                	...
							                	<?
												$pA = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_notabeli WHERE id%2=0 AND tahun='$periode_tahun' AND status='1'"));
							                	?>
					                                <div class="inner">
					                                	<a href="<?echo "?opt=$opt&menu=$menu&submenu=C"?>" style="color:#fff">
					                                	<div style="width:80px;height:80px;border-radius:80px;background:#fff;margin:0 auto;padding:10px;">
									...
									<div class="col-xs-12" style="width:100%;margin:0 auto;margin-top:10px">
				                        <table id="example3" class="table table-striped table-bordered">
											...
											<tbody style="cursor:pointer">
											<?
											$qA = mysql_query("SELECT id,nama FROM tbl_karyawan WHERE id%2=0 AND posisi IN ('2','7') ORDER BY nama");
											while($dA = mysql_fetch_array($qA))
												{
												$h1 = mysql_fetch_array(mysql_query("SELECT COUNT(qty) AS total FROM tbl_notajual_qty WHERE id%2=0 AND bulan='$periode_bulan' AND jnstransaksi!='KREDIT' AND idsales='$dA[id]'"));
												$h2 = mysql_fetch_array(mysql_query("SELECT COUNT(qty) AS total FROM tbl_notajual_qty WHERE id%2=0 AND bulan='$periode_bulan' AND jnstransaksi='KREDIT' AND idsales='$dA[id]'"));
												if(!empty($h1[total])){
													$pc = number_format($h1[total],"0","",".");
													}
												else{
