			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>GUDANG & PDI <small>DAFTAR CEK FISIK</small></h4>
									...
									<table id="example1" class="table table-striped table-hover">
			                            ...
			                            <?
			                            /*
										$q1 = mysql_query("SELECT * FROM tbl_notajual WHERE id%2=0 AND cekfisik='0' AND jnstransaksi IN ('CASH','KREDIT') AND (
																								nonota IN (SELECT nomor FROM tbl_kwitansi WHERE id%2=0 AND jnskwitansi='lunas') OR 
																								nopesan IN (SELECT nomor FROM tbl_kwitansi WHERE id%2=0 AND jnskwitansi='umuka'))
																								OR
																							cekfisik='0' AND jnstransaksi IN ('CASH TEMPO') AND (
																								nopesan IN (SELECT nomor FROM tbl_kwitansi WHERE id%2=0 AND jnskwitansi IN ('umuka','titip'))	
																								) ");
										*/
										$q1 = mysql_query("SELECT * FROM tbl_notajual WHERE id%2=0 AND 
																	(
																	cekfisik='0' AND 
																...
																OR
																	...
																	jnstransaksi IN ('CASH TEMPO') AND 
																		(
																		nopesan IN (SELECT nomor FROM tbl_kwitansi WHERE id%2=0 AND jnskwitansi IN ('umuka','titip')) OR 
																		nonota IN (SELECT nomor FROM tbl_kwitansi WHERE id%2=0 AND jnskwitansi='CASHTEMPO')	
																		)
																	)"
															);
			                            ...
			                            while($d1 = mysql_fetch_array($q1))
			                            	...
			                            	$d3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_notajual_det WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                            ?>
...
<?
		...
		$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_kwitansi WHERE id%2=0 AND nomor='$dB[nopesan]' AND jnskwitansi='penambahan' AND status='0'"));
		if(!empty($dCek[id])){
...
<?
        ...
        $qTemp  = mysql_query("SELECT * FROM tbl_notajual_det WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
    	while($dTemp = mysql_fetch_array($qTemp))
    		{
    		$id 			= $dTemp[id];
			...
			*/
			...
			if(!empty($helm)){
				mysql_query("UPDATE stok_helm SET helm=helm-1 WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
				}
			if(!empty($spion)){
				mysql_query("UPDATE stok_spion SET spion=spion-1 WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
				}
			if(!empty($accu)){
				mysql_query("UPDATE stok_accu SET accu=accu-1 WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
				}
			if(!empty($toolkit)){
				mysql_query("UPDATE stok_toolkit SET toolkit=toolkit-1 WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
				}
			if(!empty($alaskaki)){
				mysql_query("UPDATE stok_alaskaki SET alaskaki=alaskaki-1 WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
				}
			*/	
		...
		$q2 = mysql_query("UPDATE tbl_notajual SET cekfisik='1',iduserpdi='$_SESSION[id]' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
		...
		<script type='text/javascript'>
			window.open('printaj/cekfisik.php?nonota=$_REQUEST[nonota]','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
		</script>";
			                	...
			                	<h4>GUDANG & PDI <small>RIWAYAT CEK FISIK</small></h4>
									...
									<table id="example2" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_notajual_vw WHERE id%2=0 AND cekfisik='1' AND nonota LIKE '%$_REQUEST[cari]%' OR nopdi LIKE '%$_REQUEST[cari]%' OR nama LIKE '%$_REQUEST[cari]%'");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM tbl_notajual_vw WHERE id%2=0 AND cekfisik='1' ORDER BY id DESC LIMIT 0,20");
											}
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND id='$d1[idpelanggan]'"));
			                            	$d3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_notajual_det WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                            	$d4 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id%2=0 AND id='$d1[iduserpdi]'"));
			                            ?>
...
<?
		...
		$d3 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_leasing WHERE id%2=0 AND id='$dB[idleasing]'"));
		$d4 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit WHERE id%2=0 AND norangka='$dB[norangka]'"));
		$d5 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_cekfisik WHERE id%2=0 AND nonota='$dB[nonota]'"));
...
?>
			...
			<script type="text/javascript">
				...
				function popup_print(){
					window.open('printaj/cekfisik.php?nonota=<?echo $dB[nonota]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
					}
			</script>
			<aside class="right-side">
			    <section class="content">
