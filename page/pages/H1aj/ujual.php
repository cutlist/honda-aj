			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>KASIR <small>UPDATE PENJUALAN</small></h4>
									...
									<table id="example1" class="table table-striped table-hover">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM tbl_notajual WHERE id%2=0 AND status='0' AND cekfisik='1'");
...
<?
		...
		$dC = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pesanan WHERE id%2=0 AND nopesan='$dB[nopesan]'"));
		...
		if(!empty($_REQUEST[reject]))
			...
			mysql_query("UPDATE tbl_notajual SET cekfisik='0',ketreject='$ketreject' WHERE id%2=0 AND id='$_REQUEST[reject]'");
			mysql_query("DELETE FROM tbl_cekfisik WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
			                ...
			                <div class="box box-danger">
					            ...
					            <form name="inputpelanggan" onsubmit="return validA();" method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=save"?>" enctype="multipart/form-data">
	                            	...
	                            	while($dTemp = mysql_fetch_array($qTemp))
										...
										$dC   = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notajual_det WHERE id%2=0 AND norangka='$dTemp[norangka]'"));
										$dD   = mysql_fetch_array(mysql_query("SELECT * FROM tbl_grossubsidi WHERE id%2=0 AND status='1'"));
			                    	?>
		                            	<tr><td>
		                            	<div class="col-xs-6">
					                    	<table width="80%">
...
<?
    	...
    	while($dTemp = mysql_fetch_array($qTemp))
			...
			$dCnb = mysql_fetch_array(mysql_query("SELECT nonota FROM tbl_stokunit WHERE id%2=0 AND norangka='$dTemp[norangka]'"));
			if($dTemp[helm]=='1'){
				mysql_query("UPDATE stok_helm SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[spion]=='1'){
				mysql_query("UPDATE stok_spion SET jual=jual+2 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[accu]=='1'){
				mysql_query("UPDATE stok_accu SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[toolkit]=='1'){
				mysql_query("UPDATE stok_toolkit SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[alaskaki]=='1'){
				mysql_query("UPDATE stok_alaskaki SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[anakkunci]=='1'){
				mysql_query("UPDATE stok_anakkunci SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[jaket]=='1'){
				mysql_query("UPDATE stok_jaket SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
			if($dTemp[bukuservis]=='1'){
				mysql_query("UPDATE stok_bukuservis SET jual=jual+1 WHERE id%2=0 AND nonota='$dCnb[nonota]'");
				}
		...
		$q1 = mysql_query("UPDATE tbl_notajual SET status='1' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>KASIR <small>UPDATE PENJUALAN</small></h4>
									...
									<table id="example1" class="table table-striped">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM tbl_notajual WHERE id%2=0 AND status='1' AND cekfisik='1'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND id='$d1[idpelanggan]'"));
			                            	$d3 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_masterbarang WHERE id%2=0 AND id='$d1[idbarang]'"));
			                            	$d4 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_leasing WHERE id%2=0 AND id='$d1[idleasing]'"));
			                            	if(!empty($d4[namaleasing])){
												$leasing = $d4[namaleasing];
												}
...
<?
		...
		$d6 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id%2=0 AND id='$d5[user]'"));
?>
