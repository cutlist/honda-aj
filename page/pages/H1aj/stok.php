			                	...
			                	<h4>LIHAT STOK</h4>
                                    <div style="float:right;" class="col-xs-5">
			                   			...
			                   			if(!empty($_REQUEST[idgudang]))
			                   				{
											$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND idgudang='$_REQUEST[idgudang]' GROUP BY idbarang,idgudang");
											}
										else
			                   				{
			                        ...
			                        <table id="example1" class="table table-striped table-hover">
			                            ...
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$dS = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS stok FROM tbl_stokunit WHERE id%2=0 AND idbarang='$d1[idbarang]' AND idgudang='$d1[idgudang]' AND (status='STOK' OR substr(status,1,2)='NP') AND id%2=0"))
			                            ?>
...
<?
	...
	if($submenu == 'B')
		{
		$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_masterbarang WHERE id%2=0 AND id='$_REQUEST[idbarang]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>LIHAT STOK </h4>
			                        ...
			                        <table id="example1" class="table table-striped table-hover">
			                            ...
			                            <?
										if(!empty($_REQUEST[idgudang])){
											$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND idbarang='$_REQUEST[idbarang]' AND idgudang='$_REQUEST[idgudang]' AND (status='STOK' OR substr(status,1,2)='NP')");
				                            }
										if(empty($_REQUEST[idgudang])){
											$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND idbarang='$_REQUEST[idbarang]' AND (status='STOK' OR substr(status,1,2)='NP')");
				                            }
										while($d1 = mysql_fetch_array($q1))
			                            	{
			                            ?>
