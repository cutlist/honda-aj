	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$dck = mysql_fetch_array(mysql_query("SELECT id FROM tbl_potkompensasi WHERE id%2=0 AND idkaryawan='$idkaryawan' AND tgl='$tgl' AND jumlah='$jumlah' AND metodebayar='$_REQUEST[metodebayar]' AND ket='$ket'"));
				if(!empty($dck[id]))
					{
				...
				if($_REQUEST[metodebayar]=='GAJI')	
					{
					$dCG = mysql_fetch_array(mysql_query("SELECT ugapok FROM tbl_karyawan WHERE id%2=0 AND id='$idkaryawan'"));
				...
				$q1 = mysql_query("INSERT INTO tbl_potkompensasi (
												idkaryawan, 
												nama, 
												tgl, 
												jumlah, 
				...
				{	
				$q1 = mysql_query("DELETE FROM tbl_potkompensasi WHERE id%2=0 AND id='$_REQUEST[del]'");
				$q2 = mysql_query("DELETE FROM tbl_kwitansi WHERE id%2=0 AND nomor='$_REQUEST[del]'");
				$q3 = mysql_query("DELETE FROM tbl_abis_dkonfirmasi WHERE id%2=0 AND idpotkompensasi='$_REQUEST[del]'");
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>SDM <small>POTONGAN KOMPENSASI</small></h4>
									...
									if(!empty($_REQUEST[note]))
										...
										if($_REQUEST[note]=="km")
											{
											$ket = "<p>Proses Berhasil, Mohon Tunggu Konfirmasi Dari Pihak Manajemen.</br>Mohon Melanjutkan Ke Menu Kwitansi Pembayaran Piutang Tunai Dan Potongan Kompensasi Tunai Pada Bagian Kasir Setelah Pihak Manajemen Selesai Melakukan Konfirmasi (Jika Metode Pembayaran Tunai).</p>";
				                   	...
				                   	<form method="post" action="" enctype="multipart/form-data">
                                    	<table width="100%">
                                    		<tr>
												...
												?>
						                           		<td width="1%">
													<button type="button"  onclick="window.open('printaj/h1/potongankompensasi.php?periode=<?echo $_REQUEST[periode]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
			                        ...
			                        <table id="example3" class="table table-hover table-striped">
			                            ...
			                            if(empty($_REQUEST[periode])){
											if(!empty($_REQUEST[cari])){
												$q1 = mysql_query("SELECT * FROM tbl_potkompensasi WHERE id%2=0 AND (nama LIKE '%$_REQUEST[cari]%' OR metodebayar LIKE '%$_REQUEST[cari]%' OR tgl LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
												}
											else{
												$q1 = mysql_query("SELECT * FROM tbl_potkompensasi WHERE id%2=0 ORDER BY id DESC LIMIT 0,20");
												}
											}
			                            if(!empty($_REQUEST[periode])){
											...
											if(!empty($_REQUEST[cari])){
												$q1 = mysql_query("SELECT * FROM tbl_potkompensasi WHERE id%2=0 AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND (nama LIKE '%$_REQUEST[cari]%' OR metodebayar LIKE '%$_REQUEST[cari]%' OR tgl LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
												}
											else{
												$q1 = mysql_query("SELECT * FROM tbl_potkompensasi WHERE id%2=0 AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' ORDER BY id DESC LIMIT 0,20");
												}
											}
											
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$dB = mysql_fetch_array(mysql_query("SELECT * FROM tbl_kwitansi WHERE id%2=0 AND idpotkom='$d1[id]' AND jnskwitansi='tunai'"));
			                            	if($d1[status]=="0"){
...
<?
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				{	
				$dPiuX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE id%2=0 AND jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
				$dPbyX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE id%2=0 AND jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>SDM <small>POTONGAN KOMPENSASI &nbsp; <i class="fa fa-angle-right"></i> &nbsp; Lihat Riwayat Piutang</small></h4>
			                		...
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
										...
										<?
										$dPiu = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE id%2=0 AND jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
										$dPby = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE id%2=0 AND jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
					                    		...
					                    		<tr>
					                    			<td align="center">
									                    ...
									                    <table id="example2" class="table table-striped" style="width:100%"> 
															...
															<tbody>
								                    	<?
								                    		$qA = mysql_query("SELECT * FROM tbl_potkompensasi WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' ORDER BY tgl DESC");
