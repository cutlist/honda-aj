<?
	if($submenu == 'A')
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM tbl_uangharian WHERE id%2=0 AND dari='$_REQUEST[del]'");
			}
?>
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>UANG HARIAN</small></h4>
                                    <div class="col-xs-4">
										...
										$q1 = mysql_query("SELECT COUNT(nik) AS jk,SUM(totuharian) AS totuharian,dari,sampai FROM tbl_uangharian WHERE id%2=0 AND idkaryawan IN (SELECT id FROM tbl_karyawan WHERE id%2=0 AND posisi!='6' AND status='AKTIF') AND substr(dari,6,2)='$periode_bulan' AND substr(dari,1,4)='$periode_tahun' GROUP BY dari ORDER BY id DESC");
										?>
                                    		<tr>
				                    	...
				                    	<table border="0" width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2">
		                                            	...
		                                            	<input type="text" name="periode" required style="height:33px" <?if(empty($_REQUEST[periode])){?>placeholder="Pilih Periode"<?} else {?>value="<?echo $_REQUEST[periode]?>"<?}?>  class="form-control pull-right" id="reservation"/>
		                                            </div>
		                                        </td>
				                    		</tr>
				                    	...
				                    	<table border="0" width="50%">
				                    		<tr>
				                    			...
				                    			<td colspan="2">
		                                            	...
		                                            	<input type="text" name="periode" style="height:33px" placeholder="Pilih Periode" class="form-control pull-right" id="reservation" required="" />
		                                            </div>
		                                        </td>
				                    		</tr>
					                    	<input type="hidden" name="input" value="1">
...
<?
		...
		while (strtotime($awal) <= strtotime($akhir)) 
			{
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_uangharian WHERE id%2=0 AND dari <= '$awal' AND sampai >= '$awal'"));
			if(!empty($dCek[id])){
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
					            ...
					            <form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=save"?>" enctype="multipart/form-data">
			                        ...
			                        <table id="example1" class="table table-bordered table-striped table-hover" style="width:100%;padding-right:20px">
			                            ...
			                            <tbody>
			                            <?
										$q2	 = mysql_query("SELECT * FROM abs_employee WHERE id%2=0 AND EmployeeID IN (SELECT nik FROM tbl_karyawan)");
										while($d2  = mysql_fetch_array($q2))
											{
											$dD	   = mysql_fetch_array(mysql_query("SELECT * FROM abs_department WHERE id%2=0 AND DepartmentID='$d2[DepartmentID]'"));
											$awal  = date("Y-m-d",strtotime($pecah[0]));
											$akhir = date("Y-m-d",strtotime($pecah[1]));
			                            	
											while (strtotime($awal) <= strtotime($akhir)) 
												{
												$d3  = mysql_fetch_array(mysql_query("SELECT * FROM abs_status WHERE id%2=0 AND EmployeeID='$d2[EmployeeID]' AND awal <= '$awal' AND akhir >= '$awal'"));
												$d4  = mysql_fetch_array(mysql_query("SELECT * FROM abs_result_vw WHERE id%2=0 AND EmployeeID='$d2[EmployeeID]' AND substr(Date,1,10)='$awal'"));
												
												$awal = date ("Y-m-d", strtotime("+1 day", strtotime($awal)));
												}
												
												$hadir 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM abs_result_vw WHERE id%2=0 AND substr(Scan4,1,10) BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND EmployeeID='$d2[EmployeeID]' GROUP BY EmployeeID"));
												$gaji 		= mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan WHERE id%2=0 AND nik='$d2[EmployeeID]'"));
...
<?
		...
		while($d2  = mysql_fetch_array($q2))
			...
			while (strtotime($awal) <= strtotime($akhir)) 
				...
				$hadir 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM abs_result_vw WHERE id%2=0 AND substr(Date,1,10) BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND EmployeeID='$d2[EmployeeID]' GROUP BY EmployeeID"));
				$gaji 		= mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan WHERE id%2=0 AND nik='$d2[EmployeeID]'"));
   				$tharian	= $gaji[uharian]*$hadir[total];
   				
   				mysql_query("INSERT INTO tbl_uangharian (
   													nik, 
   													idkaryawan, 
   													nama, 
   													dari, 
		...
		if(!empty($_REQUEST[bayar]))
			...
			$q1 = mysql_query("UPDATE tbl_uangharian SET status='1',tglbayar='$tglbayar',updatex='$updatex' WHERE id%2=0 AND id='$_REQUEST[update]'");
			...
			$q2 = mysql_query("INSERT INTO log_act VALUES (										
								...
								");
			mysql_query("UPDATE tbl_potkompensasi SET status='1' WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' AND status='0' AND metodebayar='UANG HARIAN'");
			mysql_query("UPDATE tbl_piutang SET status='1' WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' AND status='0' AND metodebayar='UANG HARIAN'");
			}
		if(!empty($_REQUEST[batal]))
			{
			$q1 = mysql_query("UPDATE tbl_uangharian SET status='0',tglbayar='',updatex='$updatex' WHERE id%2=0 AND id='$_REQUEST[batal]'");
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>INPUT UANG HARIAN</small></h4>	
			                        <table id="example1" class="table table-striped table-hover" style="width:100%;padding-right:20px">
			                            ...
			                            <tbody>
			                            <?
										$q2	 = mysql_query("SELECT * FROM tbl_uangharian WHERE id%2=0 AND idkaryawan IN (SELECT id FROM tbl_karyawan WHERE id%2=0 AND posisi!='6' AND status='AKTIF') AND dari='$_REQUEST[dari]'");
										while($d2 = mysql_fetch_array($q2))
											{
											$dD	= mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan_vw WHERE id%2=0 AND nik='$d2[nik]'"));
											if($d2[totuharian]!='0'){
				                            	if($d2[status]=='0'){
						        ...
						        <?  
								$q1 = mysql_query("SELECT * FROM tbl_uangharian WHERE id%2=0 AND dari='$_REQUEST[dari]'");
								while($d1 = mysql_fetch_array($q1))
	                            	{
						        ?>
							<!-- ################## MODAL ########################################################################################## -->
