<?
	...
	if($submenu == 'A')
		...
		if(!empty($_REQUEST[input1]))
			...
			$q1 = mysql_query("UPDATE x23_notaservice_det SET tgltagihan='$tgltagihan',idtagihan='$_SESSION[id]' WHERE id%2=0 AND id='$_REQUEST[input1]'");
		...
		$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_penagihankpb WHERE id%2=0 AND id='$_REQUEST[input2]'"));
		if(!empty($_REQUEST[input2]))
			...
			if($_REQUEST[statuspembayaran]=="TERBAYAR")
				...
				$q1 = mysql_query("UPDATE x23_penagihankpb SET tglpembayaran='$tglpembayaran',jumlahbayar='$jumlahbayar',idbayar='$_SESSION[id]',statuspembayaran='$_REQUEST[statuspembayaran]' WHERE id%2=0 AND id='$_REQUEST[input2]'");
			...
			if($_REQUEST[statuspembayaran]=="DITOLAK")
				...
				if($_REQUEST[tagihkembali]=="TIDAK")
					{	
					$q1 = mysql_query("UPDATE x23_penagihankpb SET tagihkembali='$_REQUEST[tagihkembali]',kettolak='$kettolak',idbayar='$_SESSION[id]',statuspembayaran='$_REQUEST[statuspembayaran]' WHERE id%2=0 AND id='$_REQUEST[input2]'");
					}
		...
		if($_REQUEST[reset]=='1')
			{
			$dC1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_penagihankpb WHERE id%2=0 AND id='$_REQUEST[id]'"));
			if($dC1[statuspembayaran]=="DITOLAK" && $dC1[tagihkembali]=="YA"){
				mysql_query("DELETE FROM x23_penagihankpb WHERE id%2=0 AND nonotaservis='$dC1[nonotaservis]' AND id!='$_REQUEST[id]'");
				}
			
			mysql_query("UPDATE x23_penagihankpb SET tglpembayaran='',jumlahbayar='',tagihkembali='',kettolak='',idbayar='$_SESSION[id]',statuspembayaran='' WHERE id%2=0 AND id='$_REQUEST[id]'");
                           		...
                           		<div class="tab-content" style="overflow-x:auto;overflow-y:auto;height:460px;">	
					                   	...
					                   	<form method="post" action="" enctype="multipart/form-data">
	                                    	<table width="100%">
	                                    		<tr>
	                                    			...
	                                    			<td width="1%">
	                                    				<a href="#" onclick="window.open('printaj/penagihankpb.php?periode=<?echo $_REQUEST[periode]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')"><button type="button" class="btn btn-danger pull-left"><i class="fa fa-print"></i> Export Ke Excel</button></a>
	                                    			</td>
	                                    		</tr>
	                                    	</table>
	                                    </form>
									...
									?>	
			                        	<table id="example2" class="table table-bordered table-striped" style="min-width:220%">
											...
											$qA = mysql_query("SELECT * FROM x23_penagihankpb WHERE id%2=0 AND tglkpb BETWEEN '$periode_awal' AND '$periode_akhir' AND nonotaservis!=''");
											$q1 = mysql_query("SELECT * FROM x23_penagihankpb WHERE id%2=0 AND tglkpb BETWEEN '$periode_awal' AND '$periode_akhir' AND nonotaservis!=''");
...
<?
	...
	if($submenu == 'B')
		...
		if(!empty($_REQUEST[input1]))
			{
			$tgltagihan   = date("Y-m-d", strtotime($_REQUEST['tgltagihan']));
			$q1 = mysql_query("UPDATE x23_notajual_det SET tgltagihan='$tgltagihan',idtagihan='$_SESSION[id]' WHERE id%2=0 AND id='$_REQUEST[input1]'");
		...
		if(!empty($_REQUEST[input2]))
			...
			$q1 = mysql_query("UPDATE x23_notajual_det SET tglbayarkpb='$tglbayarkpb',jumlahbayarkpb='$jumlahbayarkpb',idbayar='$_SESSION[id]',statusbayar='1' WHERE id%2=0 AND id='$_REQUEST[input2]'");
                           		...
                           		<div class="tab-content" style="overflow-x:auto;overflow-y:auto;height:460px;">	
									...
									?>	
			                        	<table id="example2" class="table table-bordered table-striped" style="min-width:200%">
											...
											$qA = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND tglnota BETWEEN '$periode_awal' AND '$periode_akhir' AND nonota IN (SELECT nonota FROM x23_notaservice_det1_vw WHERE id%2=0 AND jnskj='KPB')");
											$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND tglnota BETWEEN '$periode_awal' AND '$periode_akhir' AND nonota IN (SELECT nonota FROM x23_notaservice_det1_vw WHERE id%2=0 AND jnskj='KPB')");
											while($d1 = mysql_fetch_array($q1))
				                            	{
