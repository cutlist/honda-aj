<?
	if($submenu == 'save')
		{
		mysql_query("UPDATE x23_notajual SET totalqty='$_REQUEST[tqty]',totdiskon='$_REQUEST[ttotdiskon]',tothargabelibersih='$_REQUEST[ttothargabelibersih]',grandtotal='$_REQUEST[ttotal]' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
		mysql_query("UPDATE x23_kwitansi SET jumlah='$_REQUEST[ttotal]' WHERE id%2=0 AND nomor='$_REQUEST[nonota]'");
			                	...
			                	<h4>PENJUALAN <small>RIWAYAT PENJUALAN</small></h4>
									...
									<table id="example1" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM x23_notajual_vw WHERE id%2=0 AND id%2=0 AND (nama LIKE '%$_REQUEST[cari]%' OR nonota LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM x23_notajual_vw WHERE id%2=0 AND id%2=0 ORDER BY id DESC LIMIT 0,20");
											}
			                            while($d1 = mysql_fetch_array($q1))
			                            	...
			                            	$dcek = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det WHERE id%2=0 AND nonota='$d1[nonota]' AND notaindent=''"));
...
<?
		...
		{
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_vw WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			    ...
			    <section class="content">
	                            	...
	                            	<div style="overflow-x:auto;overflow-y:auto;">
				                        <table id="example2" class="table table-striped table-hover" style="width:150%">
				                            ...
				                            while($dA = mysql_fetch_array($qA))
												...
												$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE id%2=0 AND nonota='$dA[notabeli]'"));
				                            ?>
                    ...
                    while($dE = mysql_fetch_array($qE))
						...
						if($_REQUEST[temp]==$dE[id])
							{
							$dcek = mysql_fetch_array(mysql_query("SELECT status FROM x23_kwitansi WHERE id%2=0 AND nomor='$_REQUEST[nonota]' AND jnskwitansi='penjualan'"));
							if($dcek[status]=='1'){
							...
							if(empty($dStok[id])){
								...
								*/
								...
								$dU = mysql_fetch_array(mysql_query("SELECT *,SUM(qty) AS tqty, SUM(total) AS ttotal, 
																								SUM(totdiskon) AS ttotdiskon,
																								SUM(tothargabelibersih) AS ttothargabelibersih
																								FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_REQUEST[nonota]'"));
										
								//echo "<script>alert ('$_REQUEST[nonota].$dU[tqty].$dU[nonota]')</script>";
								//exit();
																											
								mysql_query("UPDATE x23_notajual SET totalqty='$dU[tqty]',totdiskon='$dU[ttotdiskon]',tothargabelibersih='$dU[ttothargabelibersih]',grandtotal='$dU[ttotal]' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
								mysql_query("UPDATE x23_kwitansi SET jumlah='$dU[ttotal]' WHERE id%2=0 AND nomor='$_REQUEST[nonota]'");
                    ...
                    while($dC = mysql_fetch_array($qC))
                    	{
                    	$dCsC = mysql_fetch_array(mysql_query("SELECT stok FROM x23_stokpart WHERE id%2=0 AND nonota='$dC[notabeli]' AND idgudang='$dC[idgudang]' AND idbarang='$dC[idbarang]' AND rak='$dC[rak]'"));
                    ?>
<!-- ################## MODAL PILIH JUAL ########################################################################################## -->
				                    	...
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="notabeli<?echo $dC[id]?>" class="form-control select1" id="NJ2<?echo $dC[id]?>" style="font-size:12px;padding:3px;width: 50%" onchange="populateSelectNJ2<?echo $dC[id]?>(this.value)" required="">
													<option value='' selected>Pilih</option>
																		<?
																			$q1 = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$dC[idbarang]' AND stok>'0' GROUP BY nonota,idbarang ORDER BY nonota");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
			                	...
			                	<h4>PENJUALAN <small>RIWAYAT PENJUALAN</small></h4>
	                           		...
	                           		<div style="float:right" class="col-xs-7">
										...
										if($_SESSION[posisi]=='DIREKSI')
											{
										?>
											<button type="button"  onclick="window.open('printaj/h2/historyjualbarang.php?cari=<?echo $_REQUEST[cari]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           			<?
	                           				}
	                           			?>
	                           		</div>
									<table id="example1" class="table table-striped table-hover" style="width:160%">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND (namabarang LIKE '%$_REQUEST[cari]%' OR kodebarang LIKE '%$_REQUEST[cari]%') AND substr(nonota,1,2)='NJ' LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND substr(nonota,1,2)='NJ' ORDER BY id DESC LIMIT 0,20");
											}
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE id%2=0 AND nonota='$d1[notabeli]'"));
			                            ?>
...
<?
		...
		{
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_vw WHERE id%2=0 AND nonota='$_REQUEST[nonota]'"));
		$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(qty) AS tqty, SUM(total) AS ttotal FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$d1[nonota]'"));
?>
