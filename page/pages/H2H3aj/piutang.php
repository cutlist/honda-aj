	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$q1 = mysql_query("INSERT INTO x23_piutang (
												jenis, 
												idkaryawan, 
												nama, 
												tgl, 
				...
				$q2 = mysql_query("INSERT INTO x23_abis_dkonfirmasi (
			        ...
			        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM x23_kwitansi WHERE jnskwitansi='piutang' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
					...
					$nokwitansi = "KPU2$p_tahun2$p_bulan$p_tgl-$dig1$dig2$dig3";
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>SDM <small>PIUTANG</small></h4>
									...
									if(!empty($_REQUEST[note]))
										...
										if($_REQUEST[note]=="km")
											{
											$ket = "<p>Proses Berhasil, Mohon Tunggu Konfirmasi Dari Pihak Manajemen.</br>Mohon Melanjutkan Ke Menu Kwitansi Piutang Karyawan Pada Bagian Kasir Setelah Pihak Manajemen Selesai Melakukan Konfirmasi.</p>";
											}
									?>
			                        ...
			                        <table id="example1" class="table table-bordered table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT idkaryawan,nama,SUM(jumlah) AS total FROM x23_piutang WHERE jenis='piutang' AND status='1' AND nama LIKE '%$_REQUEST[cari]%' GROUP BY idkaryawan ORDER BY id DESC LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT idkaryawan,nama,SUM(jumlah) AS total FROM x23_piutang WHERE jenis='piutang' AND status='1' GROUP BY idkaryawan ORDER BY id DESC LIMIT 0,20");
											}
										
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$d2 = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE jenis='pembayaran' AND status='1' AND idkaryawan='$d1[idkaryawan]' GROUP BY idkaryawan"));
			                            ...
			                            ?>
			                                <tr style="cursor:pointer" onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=B&idkaryawan=$d1[idkaryawan]"?>'">
			                                    <td><?echo $d1[nama]?></td>
			                                    <td align="right"><span style="padding-right:40%"><?echo number_format(round($d1[total]/2),"0","",".")?></span></td>
			                                    <td align="right"><span style="padding-right:40%"><?echo number_format(round($sisa/2),"0","",".")?></span></td>
			                                </tr>
			                        ...
			                        <div class="modal-body">
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			...
				                    			<td>:</td>
				                    			<td><input type="text" name="tgl" class="form-control" value="<?echo date("d-m-Y")?>" style="width: 49%" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask readonly></td>
											</tr>
				                    		<tr>
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				if($_REQUEST[metodebayar]=='GAJI')	
					{
					$dCG = mysql_fetch_array(mysql_query("SELECT ugapok FROM x23_karyawan WHERE id='$_REQUEST[idkaryawan]'"));
				...
				$dPiuX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
				$dPbyX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>SDM <small>PIUTANG &nbsp; <i class="fa fa-angle-right"></i> &nbsp; Lihat Riwayat Piutang</small></h4>
			                		...
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
										...
										<?
										$dPiu = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
										$dPby = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
				                    	...
				                    	<table width="70%">
				                    		<tr>
				                    			...
				                    			<td colspan="2">
				                                    <div class="input-group">
				                                        <span class="input-group-addon">RP.</span>
				                                        <input type="text" value="<?echo number_format(round($totpiutang/2),"0","",".")?>"class="form-control" placeholder="0" style="width:40%;text-align:right" maxlength="12" onfocus="this.select();" onkeypress="return buat_angka(event,'0123456789')" readonly>
				                                    </div>
						                        </td>
				                    		</tr>
				                    		<tr>
				                    			...
				                    			<td colspan="2">
				                                    <div class="input-group">
				                                        <span class="input-group-addon">RP.</span>
				                                        <input type="text" value="<?echo number_format(round($totpembayaran/2),"0","",".")?>" class="form-control" placeholder="0" style="width:40%;text-align:right" maxlength="12" onfocus="this.select();" onkeypress="return buat_angka(event,'0123456789')" readonly>
				                                    </div>
						                        </td>
				                    		</tr>
				                    		<tr>
				                    			...
				                    			<td colspan="2">
				                                    <div class="input-group">
				                                        <span class="input-group-addon">RP.</span>
				                                        <input type="text" value="<?echo number_format(round($sisapiutang/2),"0","",".")?>" class="form-control" placeholder="0" style="width:40%;text-align:right" maxlength="12" onfocus="this.select();" onkeypress="return buat_angka(event,'0123456789')" readonly>
				                                    </div>
						                        </td>
				                    		</tr>
		                            	</table>
										...
										<div style="margin-top:20px">
					                    	<table width="100%">
					                    		<tr>
					                    			<td align="center">
                                    					<button type="button"  onclick="window.open('printaj/h2/historypiutang.php?idkaryawan=<?echo $_REQUEST[idkaryawan]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
									                    ...
									                    <table id="example2" class="table table-striped table-bordered" style="width:100%"> 
															...
															<tbody>
								                    	<?
								                    		$qA = mysql_query("SELECT * FROM x23_piutang WHERE idkaryawan='$_REQUEST[idkaryawan]' ORDER BY tgl DESC");
								                    		while($dA=mysql_fetch_array($qA))
								                    			{
																$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_kwitansi WHERE nomor='$dA[id]' AND idpotkom='0'"));
								                            	if($dA[status]=="0"){
																	if($dB[status]=="0"){
								                    	...
								                    	?>
																<tr style="cursor:pointer">
																	...
																	<td align="right"><span style="padding-right:20%"><?echo number_format(round($dA[jumlah]/2),"0","",".")?></span></td>
...
<?
					...
					if($mod=="del")
						{
						mysql_query("DELETE FROM x23_piutang WHERE id='$_REQUEST[id]'");
						mysql_query("DELETE FROM x23_abis_dkonfirmasi WHERE idbyrpiutang='$_REQUEST[id]'");
						mysql_query("DELETE FROM x23_kwitansi WHERE jnskwitansi='tunai' AND nomor='$_REQUEST[id]' AND idpotkom='0'");
