			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>KONFIRMASI NOTA CLAIM <small>OLI KE MPM</small></h4>
			                        ...
			                        <table id="example1" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM x23_notabeli WHERE id%2=0 AND scan='0' AND jns='CLAIM'");
			                	...
			                	<h4>KONFIRMASI NOTA CLAIM <small>OLI KE MPM</small></h4>
			                        ...
			                        <table class="table table-striped" id="example2">
			                            ...
			                            while($d2 = mysql_fetch_array($q1))
			                            	{
			                            	$dD1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_claimoli_det WHERE id%2=0 AND id='$d2[id_claimoli_det]'"));
...
?>
			<aside class="right-side">
			    <section class="content">
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>KONFIRMASI NOTA CLAIM <small>OLI KE MPM</small></h4>
			                        ...
			                        <table class="table table-striped" id="example2" style="width: 130%">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM temp_x23_konfclaim_det WHERE id%2=0 AND notabeli='$d1[nonota]'");
			                            while($dA = mysql_fetch_array($q1))
			                            	{
			                            	$dD1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_claimoli_det WHERE id%2=0 AND id='$dA[id_claimoli_det]'"));
											$dB  = mysql_fetch_array(mysql_query("SELECT gudang FROM x23_gudang WHERE id%2=0 AND id='$dA[idgudang]]'"));
			                            ?>
			                                <tr style="cursor:pointer;">
			                        ...
			                        ?>
				           			...
				           			<div class="col-xs-12">
				                        ...
				                        <?
			                                if($d1[konf]=='0'){
												$dc1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM temp_x23_konfclaim_det WHERE id%2=0 AND notabeli='$d1[nonota]' AND statusclaim=''"));
				                                if($dc1[total]>'0'){}
												else{
			                            ?>
		            ...
		            <?
					$q3 = mysql_query("SELECT * FROM temp_x23_konfclaim_det WHERE id%2=0 AND notabeli='$d1[nonota]'");
		            while($d3 = mysql_fetch_array($q3))
		            	{
						//$d4 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det2_vw WHERE id%2=0 AND id='$d3[id]'"));
		            ?>
<!-- ################## MODAL UBAH BARANG ########################################################################################## -->
...
<?
		...
		$qA  = mysql_query("SELECT * FROM temp_x23_konfclaim_det WHERE id%2=0 AND notabeli='$_REQUEST[nonota]'");
        ...
        while($dA = mysql_fetch_array($qA))
        	{   
        	mysql_query("UPDATE x23_claimoli_det SET statusclaim='$dA[statusclaim]',tagihkembali='$dA[tagihkembali]',kettolak='$dA[kettolak]' WHERE id%2=0 AND id='$dA[id_claimoli_det]'");
        	if($dA[statusclaim] == "TERGANTIKAN")
				{
	        	mysql_query("UPDATE x23_notabeli_det SET tgltiba=CURDATE(),idgudang='$dA[idgudang]',rak='$dA[rak]' WHERE id%2=0 AND id='$dA[idnotabelidet]'");	
				...
				if($dA[tagihkembali] == "YA")
					{
					$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_claimoli_det WHERE id%2=0 AND id='$dA[id_claimoli_det]'"));
		...
		$dH1 = mysql_fetch_array(mysql_query("SELECT *,COUNT(statusclaim) AS total FROM x23_claimoli_det WHERE id%2=0 AND nonota='$_REQUEST[nonota]' AND statusclaim='DITOLAK'"));
    	if($dH1[total] > '0'){
			$p_tahun = date("Y");
			$p_bulan = date("m");	
		...
		mysql_query("UPDATE x23_notabeli SET totalqty='$qty',idsupplier='1',scan='1',konf='1',harga='0',gtbayar=grandtotal,iduserkonf='$_SESSION[id]',updatex='$updatex' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
