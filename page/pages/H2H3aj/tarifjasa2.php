	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$dCek  = mysql_fetch_array(mysql_query("SELECT pangkat FROM x23_karyawan WHERE id%2=0 AND id='$idmekanik'"));
				$dCek2 = mysql_fetch_array(mysql_query("SELECT tarif FROM x23_tarifjasa WHERE id%2=0 AND idjasa='$idjasa' AND pangkat='$dCek[pangkat]'"));
				...
				if($jnsjasa=="RETAIL")
					{
					$dCek3 = mysql_fetch_array(mysql_query("SELECT id FROM x23_tarifjasa2 WHERE id%2=0 AND idjasa='$idjasa' AND idmekanik='$idmekanik'"));
					if(!empty($dCek3[id]))
						{
				...
				if($jnsjasa=="PAKET")
					{
					$dCek3 = mysql_fetch_array(mysql_query("SELECT id FROM x23_tarifjasa2 WHERE id%2=0 AND kodepaket='$kodepaket' AND idmekanik='$idmekanik'"));
					if(!empty($dCek3[id]))
						{
						echo "<script>alert (' Mohon Ulangi, Karena Mekanik Dan Jasa Sudah Ada Pada Database.')</script>";
						print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=$submenu'/>";
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_tarifjasa2 WHERE id%2=0 AND id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_tarifjasa2',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[ubah]))
				...
				$dCek2 = mysql_fetch_array(mysql_query("SELECT tarif FROM x23_tarifjasa WHERE id%2=0 AND idjasa='$_REQUEST[idjasa]' AND pangkat='$_REQUEST[pangkat]'"));
				if(!empty($dCek2[tarif]))
					{
					if($dCek2[tarif] < $tarif)
						{
			                	...
			                	<h4>MASTER <small>LAYANAN JASA MEKANIK</small></h4>
			                        ...
			                        <table id="example1" class="table table-bordered table-striped">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM x23_tarifjasa2_vw WHERE id%2=0 AND nama LIKE '%$_REQUEST[cari]%' OR jnsjasa LIKE '%$_REQUEST[cari]%' OR namajasa LIKE '%$_REQUEST[cari]%' LIMIT 0,20");
											}
										else
											{
			                            ...
			                            while($d1 = mysql_fetch_array($q1))
			                            	...
			                            	if($d1[jnsjasa]=='RETAIL'){
												$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterjasa WHERE id%2=0 AND id='$d1[idjasa]'"));
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_tarifjasa2_vw WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:520px;">
