<?
	if($submenu == 'saveA')
        ...
        $dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_lembur WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' AND tanggal='$tanggal'"));
        if(!empty($dCek[id]))
        	{
		...
		$dK    = mysql_fetch_array(mysql_query("SELECT nik FROM x23_karyawan WHERE id%2=0 AND id='$_REQUEST[idkaryawan]'"));
		$dCek2 = mysql_fetch_array(mysql_query("SELECT id FROM abs_x23_status WHERE id%2=0 AND EmployeeID='$dK[nik]' AND  awal <= '$tanggal' AND akhir >= '$tanggal'"));
		if(!empty($dCek2[id]))
			{
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM x23_lembur WHERE id%2=0 AND id='$_REQUEST[del]'");
			}
?>
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>DAFTAR LEMBUR</small></h4>
                                    <div style="float:right" class="col-xs-6">
										...
										$q1 = mysql_query("SELECT * FROM x23_lembur_vw WHERE id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun'");
										?>
                                    		<tr>
									...
									<table id="example2" class="table table-striped">
			                            ...
			                            <?
	                                    	if($_SESSION[posisi]=='DIREKSI')
	                                    		{		
												$d2	= mysql_fetch_array(mysql_query("SELECT id FROM x23_uanglembur WHERE id%2=0 AND idkaryawan='$d1[idkaryawan]' AND tanggal='$d1[tanggal]'"));
				                            	if(empty($d2[id]))
				                            		{
			                           	?>
				                    	...
				                    	<table border="0" width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="idkaryawan" class="form-control" id="select1" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			$q1 = mysql_query("SELECT id,nama FROM x23_karyawan WHERE id%2=0 AND posisi NOT IN ('1','6') AND status='AKTIF' ORDER BY nama");
...
<?
		...
		if(!empty($_REQUEST[batal]))
			{
			mysql_query("DELETE FROM x23_uanglembur WHERE id%2=0 AND id='$_REQUEST[batal]'");
			}
?>
			<aside class="right-side">
			    <section class="content">
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>PEMBAYARAN UANG LEMBUR</small></h4>
                                    <div style="float:right" class="col-xs-6">
										...
										$q1 = mysql_query("SELECT * FROM x23_lembur_vw WHERE id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND idkaryawan IN (SELECT id FROM x23_karyawan WHERE id%2=0 AND status='AKTIF')");
										$q2 = mysql_query("SELECT * FROM x23_lembur_vw WHERE id%2=0 AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND idkaryawan IN (SELECT id FROM x23_karyawan WHERE id%2=0 AND status='AKTIF')");
										?>
                                    		<tr>
                                    			<td width="70%"><select name="bulan" class="form-control" style="height:35px">
														<option value='' >- PILIH -</option>
												...
												?>
						                           		<td width="1%">
													<button type="button"  onclick="window.open('printaj/h2/pembayaranlembur.php?bulan=<?echo $_REQUEST[bulan]?>&tahun=<?echo $_REQUEST[tahun]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
								...
								while($dA = mysql_fetch_array($q2))
	                            	{
	                            	//$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_lembur_vw WHERE id%2=0 AND id='$d2[id]'"));
						        ?>
							<!-- ################## MODAL ########################################################################################## -->
							                    		...
							                    		<tr>
							                    			...
							                    			<td><div class="input-group">
							                                        <span class="input-group-addon" style="min-width:45px;text-align:center;"><i class="fa fa-calendar"></i> &nbsp;</span>
							                                        	<input type="text" name="tglbayar" value="<?echo date("d-m-Y")?>" class="form-control" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask required="" style="width:60%" readonly="">
							                                    </div>                                        		
							                                </td>
							                    		</tr>
							                    		<tr>
