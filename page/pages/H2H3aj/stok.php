...
?>
		<script type="text/javascript">
			...
			function popup_print(){
				window.open('printaj/h2/stoksparepart.php?idgudang=<?echo $_REQUEST[idgudang]?>&cari=<?echo $_REQUEST[cari]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
				}
		</script>
			                	...
			                	<h4>LIHAT STOK</h4>
			                        ...
			                        <table id="example1" class="table table-striped table-hover" style="width:120%">
										...
										if(empty($_REQUEST[cari]) AND empty($_REQUEST[idgudang]))
											{
											$q1 = mysql_query("SELECT * FROM x23_stokpart_group_vw2 WHERE id%2=0 GROUP BY idbarang,nonota,idgudang,rak LIMIT 0,60");
											}
										if(empty($_REQUEST[cari]) AND !empty($_REQUEST[idgudang]))
											{
											$q1 = mysql_query("SELECT * FROM x23_stokpart_group_vw2 WHERE id%2=0 AND idgudang='$_REQUEST[idgudang]' GROUP BY idbarang,nonota,idgudang,rak LIMIT 0,60");
											}
										if(!empty($_REQUEST[cari]) AND empty($_REQUEST[idgudang]))
											{
											$q1 = mysql_query("SELECT * FROM x23_stokpart_group_vw2 WHERE id%2=0 AND kodebarang LIKE '%$_REQUEST[cari]%' OR namabarang LIKE '%$_REQUEST[cari]%' OR varian LIKE '%$_REQUEST[cari]%' GROUP BY idbarang,nonota,idgudang,rak LIMIT 0,60");
											}
										if(!empty($_REQUEST[cari]) AND !empty($_REQUEST[idgudang]))
											{
											$q1 = mysql_query("SELECT * FROM x23_stokpart_group_vw2 WHERE id%2=0 AND idgudang='$_REQUEST[idgudang]' AND (kodebarang LIKE '%$_REQUEST[cari]%' OR namabarang LIKE '%$_REQUEST[cari]%' OR varian LIKE '%$_REQUEST[cari]%') GROUP BY idbarang,nonota,idgudang,rak LIMIT 0,60");
											}
