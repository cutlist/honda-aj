<?
	if($submenu == 'A')
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM x23_indent WHERE id%2=0 AND noindent='$_REQUEST[del]'");
			mysql_query("DELETE FROM x23_indent_det WHERE id%2=0 AND noindent='$_REQUEST[del]'");
			}
...
<?
		...
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_indent_vw WHERE id%2=0 AND id='$_REQUEST[id]'"));
		$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(qty) AS tqty FROM x23_indent_det_vw WHERE id%2=0 AND noindent='$d1[noindent]'"));
		
		$dC = mysql_fetch_array(mysql_query("SELECT *,SUM(jumlah) AS jumlah FROM x23_kwitansi WHERE id%2=0 AND nomor='$d1[noindent]' AND jnskwitansi='indent' AND status='1'"));
		if(empty($dC[jumlah])){
			$status = "1";
			}
		else{
		...
		if($_REQUEST[tambahdp]=="1")
			{	
	        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM x23_kwitansi WHERE id%2=0 AND jnskwitansi='indent' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qA = mysql_query("SELECT * FROM x23_indent_det_vw WHERE id%2=0 AND noindent='$d1[noindent]'");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
			                            	$d2 = mysql_fetch_array(mysql_query("SELECT SUM(totalstok) AS stok FROM x23_stokpart_group_vw WHERE id%2=0 AND idbarang IN ($dA[idbarang]) GROUP BY idbarang"));
			                            ?>
			                                <tr style="cursor:pointer">
...
<?
		...
		mysql_query("DELETE FROM x23_notajual_det WHERE id%2=0 AND notabeli=''");
		mysql_query("DELETE FROM x23_notajual_det WHERE id%2=0 AND nonota='$nonota'");
		
		mysql_query("DELETE FROM x23_abis_ikesalahan WHERE id%2=0 AND idnotaindent='$_REQUEST[idnotaindent]'");
			
		$qA = mysql_query("SELECT * FROM x23_indent_det WHERE id%2=0 AND noindent='$_REQUEST[noindent]'");
		while($dA = mysql_fetch_array($qA))
			{
        ...
        $dNJ = mysql_fetch_array(mysql_query("SELECT nonota FROM x23_notajual WHERE id%2=0 AND tglnota=CURDATE() ORDER BY SUBSTR(nonota,-3,3) DESC LIMIT 1"));
		...
		$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_indent_vw WHERE id%2=0 AND noindent='$_REQUEST[noindent]'"));
		...
		if(!empty($_REQUEST[del]))
			{
			$dBrg = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id%2=0 AND id='$_REQUEST[idbarang]'"));
			$dP = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND id='$dA[idpelanggan]'"));
			$q1 = mysql_query("DELETE FROM x23_notajual_det WHERE id%2=0 AND id='$_REQUEST[del]'");		
			
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_abis_ikesalahan WHERE id%2=0 AND idnotaindent='$dA[id]' AND notajual='$nonotaik' AND idbarangindent='$_REQUEST[idbarang]'"));
			if(empty($dCek[id]))
				{
		...
		$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(qtyindent) AS tqtyindent,SUM(qty) AS tqty, SUM(total) AS ttotal, SUM(totdiskon) AS totdiskon, SUM(tothargabelibersih) AS tothargabelibersih FROM x23_notajual_det WHERE id%2=0 AND nonota=''"));
		$dZ = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS jumlah FROM x23_kwitansi WHERE id%2=0 AND jnskwitansi='indent' AND nomor='$_REQUEST[noindent]'"));
			    ...
			    <section class="content">
			        <div class="row">
			                    	...
			                    	<div id="spoiler" style="display:none">
				           			...
				           			<div class="col-xs-12">
				                        <div class="modal-footer clearfix">
					                    	...
					                    	<?
											$dCh = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det WHERE id%2=0 AND nonota=''"));
											if(!empty($dCh[id]))
												{
											?>
			            ...
			            <div class="col-xs-12">
			                <div class="box box-info">
			                    ...
			                    <?
			                    $dC1 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det WHERE id%2=0 AND nonota=''"));
			                    if(!empty($dC1[id]))
			                    	{
			                    ?>
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qC = mysql_query("SELECT * FROM x23_notajual_det2_vw WHERE id%2=0 AND nonota=''");
										$qD = mysql_query("SELECT * FROM x23_notajual_det2_vw WHERE id%2=0 AND nonota=''");
										$qE = mysql_query("SELECT * FROM x23_notajual_det2_vw WHERE id%2=0 AND nonota=''");
			                            while($dD = mysql_fetch_array($qD))
			                            	{
											$dX = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND id='$dD[id]'"));
			                            ?>
			                                <tr style="cursor:pointer">
			                                    ...
			                                    <td align="center"><a href="<?echo "?opt=$opt&menu=$menu&submenu=$submenu&del=$dD[id]&idbarang=$dD[idbarang]&noindent=$_REQUEST[noindent]"?>"><i class="fa fa-trash-o"></i></a></td>
                    ...
                    while($dE = mysql_fetch_array($qE))
						...
						if($_REQUEST[temp]==$dE[id])
							...
							$dStok = mysql_fetch_array(mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$idbarang' AND nonota='$notabeli' AND idgudang='$idgudang' AND 
																										 rak='$rak'"));	
							...
							if(empty($dStok[id])){
								...
								if($diskon > $dStok[hargajual]){
									//$dStok = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det_vw WHERE id%2=0 AND idbarang='$_REQUEST[idbarang]' AND tglnota='$tglnota' LIMIT 1,1"));
									echo "<script>alert ('Mohon Ulangi, Karena Diskon Melebihi Harga Jual!')</script>";
									print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&mod=$mod&submenu=$submenu&noindent=$_REQUEST[noindent]'/>";
									exit();
									}
									
								$dCek2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det WHERE id%2=0 AND notaindent='$_REQUEST[noindent]' AND idbarang='$idbarang' AND 
								...
								if($qty < $_REQUEST[qtyindentsisa])
									...
									$dCek1 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det WHERE id%2=0 AND notaindent='$_REQUEST[noindent]' AND idbarang='$idbarang' AND qtyindentsisa='$sisa'"));
									if(!empty($dCek1[id]))
										{
				                    	...
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="notabeli<?echo $dC[id]?>" class="form-control select1" id="NJ2<?echo $dC[id]?>" style="font-size:12px;padding:3px;width: 50%" onchange="populateSelectNJ2<?echo $dC[id]?>(this.value)" required="">
													<option value='' selected>Pilih</option>
																		<?
																			$q1 = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$dC[idbarang]' AND stok>'0' AND hargajual!='' GROUP BY nonota,idbarang ORDER BY nonota");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
...
<?
	...
	if($submenu == 'NJ1')
		...
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$q' AND stok>'0' GROUP BY nonota,idbarang ORDER BY nonota");
	...
	if($submenu == 'NJ2')
		...
		echo "<option value=''>Pilih</option>";
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$_SESSION[idbarang]' AND nonota='$q1' AND stok>'0' GROUP BY nonota,idbarang,tglnota ORDER BY tglnota");
		while($d = mysql_fetch_array($qA))
			{
	...
	if($submenu == 'NJ4')
		...
		if(empty($dNJ[nonota]))
			...
			$nonota = "NJ2$p_tahun2$p_bulan$p_tgl-$dig1$dig2$dig3";
			  ...
			  mysql_query("UPDATE x23_indent SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			  mysql_query("UPDATE x23_notajual_det SET nonota='$nonota' WHERE id%2=0 AND notaindent='$_REQUEST[noindent]'");
			  
		$dJ = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det WHERE id%2=0 AND nonota='$nonota'"));
		if(!empty($dJ[id]))
			{
	        ...
	        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM x23_kwitansi WHERE id%2=0 AND jnskwitansi='penjualan' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
