	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_masterbarang WHERE id%2=0 AND id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_masterbarang',
			                                    CURDATE(),
			                	...
			                	<h4>MASTER <small>NAMA BARANG</small></h4>
			                        ...
			                        <table id="example1" class="table table-bordered table-striped">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM x23_masterbarang WHERE id%2=0 AND kodebarang LIKE '%$_REQUEST[cari]%' OR namabarang LIKE '%$_REQUEST[cari]%' OR varian LIKE '%$_REQUEST[cari]%' LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM x23_masterbarang WHERE id%2=0 ORDER BY id DESC LIMIT 0,20");
											}
				                    	...
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select class="form-control" name="idsupplier" style="width:100%" required>
																...
																<?
																	$q1 = mysql_query("SELECT * FROM x23_supplier WHERE id%2=0 AND status='1' ORDER BY nama");
																	while($dA=mysql_fetch_array($q1))
																		{
																?>
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:520px;">
