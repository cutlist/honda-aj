				                	...
				                	<form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=saveB"?>" enctype="multipart/form-data">
				           			<div class="col-xs-12">
					                	...
					                	<?
											...
											if(!empty($dcp[id])){
												$dcp2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_SESSION[nonota]' AND total='0' AND totdiskon='0' AND qty='1'"));	
												if(empty($dcp2[id])){
...
<?
	...
	if($submenu == 'NJ2')
		...
		echo "<option value=''>Pilih</option>";
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$_SESSION[idbarang]' AND nonota='$q' AND stok>'0' GROUP BY nonota,idbarang,tglnota ORDER BY tglnota");
		while($d = mysql_fetch_array($qA))
			{
