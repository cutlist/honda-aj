<?
	//SYNC ABSEN ==========================================================================
	$conn 	= odbc_connect("absenh2h3", "", "T8Q 51MQ");
	$sync1  	= odbc_exec($conn, "SELECT * FROM Result_T");
	while($dA = odbc_fetch_array($sync1)){
		mysql_query("INSERT INTO abs_x23_result (
										EmployeeID,
										Date,
										ShiftID,
										Scan1,
	...
	mysql_query("TRUNCATE abs_x23_department");
	$sync2  	= odbc_exec($conn, "SELECT * FROM Department_T");
	while($dA = odbc_fetch_array($sync2)){
		mysql_query("INSERT INTO abs_x23_department (
										DepartmentID,
										DepartmentName
										) 
								VALUES (
	...
	mysql_query("TRUNCATE abs_x23_employee");
	$sync3  	= odbc_exec($conn, "SELECT * FROM Employee_T");
	while($dA = odbc_fetch_array($sync3)){
		mysql_query("INSERT INTO abs_x23_employee (
										EmployeeID,
										FirstName,
										LastName,
										DepartmentID,
