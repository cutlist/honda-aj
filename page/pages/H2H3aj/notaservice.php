...
<?
	if($submenu == 'saveB')
		...
		$dNan = mysql_fetch_array(mysql_query("SELECT noantrian FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() ORDER BY noantrian DESC LIMIT 0,1"));
		...
		$dNan2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() AND noantrian='$_REQUEST[noantrian]'"));
		if(empty($dNan2[id])){
			echo "<script>alert ('Proses Gagal, Karena Nomor Antrian Tidak Terdaftar, Mohon Ulangi Lagi.')</script>";
			print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&mod=$mod&submenu=B&id=$_REQUEST[idpelanggan]'/>";
			exit();
		...
		$dNan3 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE() AND noantrian='$_REQUEST[noantrian]'"));
		if(!empty($dNan3[id])){
			echo "<script>alert ('Proses Gagal, Karena Nomor Antrian Tidak Terdaftar, Mohon Ulangi Lagi.')</script>";
			print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&mod=$mod&submenu=B&id=$_REQUEST[idpelanggan]'/>";
			exit();
		...
		$dNP = mysql_fetch_array(mysql_query("SELECT nopol FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() AND noantrian='$_REQUEST[noantrian]'"));
		
		mysql_query("UPDATE x23_antrian SET status='1',jammulai=CURTIME() WHERE id%2=0 AND tanggal=CURDATE() AND noantrian='$_REQUEST[noantrian]'");
		...
		$qA = mysql_query("SELECT * FROM kd_kecamatan WHERE id%2=0 AND kodekab='$q1' ORDER BY namakec");
		...
		$dB = mysql_fetch_array( mysql_query("SELECT * FROM kd_kelurahan WHERE id%2=0 AND kodekab ='$q1' AND kodekec='$q2'"));
		$dC = mysql_fetch_array(mysql_query("SELECT * FROM kd_kecamatan WHERE id%2=0 AND kodekab ='$q1' AND kodekec='$q2'"));
		...
		if(!empty($dB['namakel']))
			...
			echo "<option value=''>- Pilih Kelurahan -</option>";
				$qA = mysql_query("SELECT * FROM kd_kelurahan WHERE id%2=0 AND kodekab ='$q1' AND kodekec='$q2' ORDER BY namakel");
			                	...
			                	<h4>SERVIS <small>INPUT NOTA SERVIS</small></h4>
									...
									<table id="example1" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND grup=0 AND nama LIKE '%$_REQUEST[cari]%' OR ohc LIKE '%$_REQUEST[cari]%' OR notelepon LIKE '%$_REQUEST[cari]%' LIMIT 0,100");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND grup=0 ORDER BY id DESC LIMIT 0,20");
											}
			            ...
			            <div class="col-xs-6">		
                            <div class="small-box bg-aqua" style="text-align:center;height:150px;border-radius:5px;margin-top:0px;padding:20px 0 0 0;border-bottom: 5px solid #fff;cursor: pointer;">
                                <div class="inner" style="height:110px;">
			                		...
			                		<?
			                		$dP1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE()"));
			                		?>
			            ...
			            <div class="col-xs-6">		
                            <div class="small-box bg-green" style="text-align:center;height:150px;border-radius:5px;margin-top:0px;padding:20px 0 0 0;border-bottom: 5px solid #fff;cursor: pointer;">
                                <div class="inner" style="height:110px;">
			                		...
			                		<?
			                		$dP2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() AND status='0'"));
			                		?>
	                                	<a href="<?echo "?opt=$opt&menu=".md5(historyjual)."&submenu=A"?>" style="color:#fff">
		                                	<div style="width:80px;height:80px;border-radius:80px;background:#fff;margin:0 auto;padding:10px;margin-top:20px">
				                ...
				                <div class="modal-content">
			                        ...
			                        <div class="modal-body"style="overflow-y:auto;overflow-x:hidden;height:520px;">
										<table id="example4" class="table table-striped" >
				                            ...
				                            <?
				                            $no = 1;
				                            $q1 = mysql_query("SELECT * FROM log_act WHERE id%2=0 AND tbl='x23_notaservice' ORDER BY id DESC LIMIT 0,50");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
				                                <tr>
...
<?
		...
		$dC2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() AND status='0'"));
		if(empty($dC2[id]))
			{
		...
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND id='$_REQUEST[id]'"));
		if($d1[grup]=='1')
			{
        ...
        $dNA = mysql_fetch_array(mysql_query("SELECT noantrian FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE() ORDER BY noantrian DESC LIMIT 1"));
		...
		$dNan2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() AND noantrian='$noantrian'"));
		if(empty($dNan2[id])){
			$dNan3 = mysql_fetch_array(mysql_query("SELECT noantrian FROM x23_antrian WHERE id%2=0 AND tanggal=CURDATE() AND noantrian NOT IN (SELECT noantrian FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE()) ORDER BY noantrian ASC LIMIT 0,1"));
        ...
        $dNC = mysql_fetch_array(mysql_query("SELECT noclaim FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE() ORDER BY SUBSTR(noclaim,-3,3) DESC LIMIT 1"));
        ...
        $dP = mysql_fetch_array(mysql_query("SELECT nopkb FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE() ORDER BY SUBSTR(nopkb,-3,3) DESC LIMIT 1"));
		...
		if(empty($dP[nopkb]))
	        ...
	        $dNS = mysql_fetch_array(mysql_query("SELECT nonota FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE() ORDER BY SUBSTR(nonota,-3,3) DESC LIMIT 1"));
			                    	...
			                    	<table width="90%">
			                    		...
			                    		<tr>
			                    			...
			                    			<td colspan="2"><select name="idmekanik" class="form-control select1" style="font-size:12px;padding:3px;width:60%" required="">
																		<option value='' selected>Pilih</option>
																			<?
																				$q1 = mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND posisi='4' AND status='AKTIF' ORDER BY nama");
																				while($dA=mysql_fetch_array($q1))
																					{
																			?>
										...
										<tr>
											...
											<td colspan="2"><select name="kodepaket" class="form-control select1" style="font-size:12px;padding:3px;width:100%" disabled="">
																	<option value='' selected>Pilih</option>
																		<?
																			$q1 = mysql_query("SELECT * FROM x23_kelompokjasa WHERE id%2=0 AND jnskj='KPB' AND status='1' ORDER BY kode");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
			                    	...
			                    	<div id="spoiler" style="display:none">
										...
										<table class="table table-striped table-hover" >
											<thead>
				                       		...
				                       		<tbody>
				                            <?
				                            $q1 = mysql_query("SELECT * FROM x23_notaservice WHERE id%2=0 AND idpelanggan='$_REQUEST[id]' AND jamselesai!='00:00:00' AND tglselesai!='0000-00-00' ORDER BY tglnota DESC");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
...
<?
        ...
        $dNP = mysql_fetch_array(mysql_query("SELECT nopesan FROM tbl_pesanan WHERE id%2=0 AND tahun='$p_tahun' AND bulan='$p_bulan' ORDER BY SUBSTR(nopesan,-3,3) DESC LIMIT 1"));
