...
?>
			<script type="text/javascript">
				...
				function popup_print(){
					window.open('printaj/h2/laporanws3.php?periode_awal=<?echo $_SESSION[periode_awal]?>&periode_akhir=<?echo $_SESSION[periode_akhir]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
					}
			</script>
			        ...
			        <div class="row">
			            <div class="col-xs-12">	     
			                ...
			                <?
										...
										$dA = mysql_fetch_array(mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'"));
										$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jns='SPARE PART'"));
										$dC = mysql_fetch_array(mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jns='OLI'"));
										
										$q1 = mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY nonota");
			                            while($d1 = mysql_fetch_array($q1))
											...
											*/
											...
											$d3 = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM x23_notajual_det_ws1c WHERE id%2=0 AND nonota='$d1[nonota]' AND jns='SPARE PART'"));
											$d4 = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM x23_notajual_det_ws1c WHERE id%2=0 AND nonota='$d1[nonota]' AND jns='OLI'"));
