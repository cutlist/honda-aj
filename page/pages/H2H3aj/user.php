	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$d1 = mysql_fetch_array(mysql_query("SELECT user FROM x23_user WHERE id%2=0 AND user='$user' OR id_karyawan='$id_karyawan'"));
				...
				if($pass1 != $pass2)
					...
					if(!empty($d1[user]))
						...
						{				
						$q1 = mysql_query("INSERT INTO x23_user (
													id_karyawan, 
													user, 
													pass,
													hakakses,
			...
			if(!empty($_REQUEST[resetpass]))
				...
				if($pass1 != $pass2)
					...
					{
					$q1 = mysql_query("UPDATE x23_user SET pass='$pass',pic_user='$_SESSION[user]' WHERE id%2=0 AND id='$_REQUEST[resetpass]'");
					$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_user',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[ubahdetail]))
				{
				$q1 = mysql_query("UPDATE x23_user SET hakakses='$_REQUEST[hakakses]',pic_user='$_SESSION[user]' WHERE id%2=0 AND id='$_REQUEST[ubahdetail]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
		                                    '',
		                                    'x23_user',
		                                    CURDATE(),
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_user WHERE id%2=0 AND id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_user',
			                                    CURDATE(),
...
?>
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>MASTER <small>USER</small></h4>
			                        ...
			                        <table id="example1" class="table table-bordered table-striped">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM x23_user_vw WHERE id!='1' ORDER BY nama");
			    ...
			    <?
                    $q2 = mysql_query("SELECT * FROM x23_user_vw");
			                        ...
			                        <div class="modal-body">
				                    	<table width="100%">
				                    		<tr>
				                    			<td width="30%">NAMA</td>
				                    			<td width="2%">:</td>
				                    			<td colspan="2"><input type="text" name="nama" value="<?echo $d2[nama]?>" class="form-control" readonly></td>
											</tr>
				                    		<tr>
				                    			<td>USERNAME</td>
				                    			<td>:</td>
				                    			<td colspan="2"><input type="text" name="user"value="<?echo $d2[user]?>" class="form-control"readonly=""></td>
				                    		</tr>
				                    		<tr>
				                    			<td>POSISI</td>
				                    			<td>:</td>
				                    			<td colspan="2"><input type="text" name="posisi" value="<?echo $d2[posisi]?>" class="form-control"readonly=""></td>
				                    		</tr>
				                    		<tr>
				                    			<td>HAK AKSES</td>
				                    			<td>:</td>
				                    			<td colspan="2"><select class="form-control" name="hakakses" style="width:40%" required>
																...
																<?
																	...
																	while($dA=mysql_fetch_array($q1))
																		{
																?>
																			<option value='<?echo $dA[hakakses]?>' <?if($d2[hakakses]==$dA[hakakses]){?>selected<?}?>><?echo $dA[hakakses]?></option>
																<?
																		}
																?>
													</select></td>
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select class="form-control select1" name="id_karyawan" style="width:70%;font-size:12px;padding:3px" required>
																	<option value=''>Pilih</option>
																<?
																	$q1 = mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND posisi!='6' AND status='AKTIF' AND id NOT IN (SELECT id_karyawan FROM x23_user) ORDER BY nama");
																	while($dA=mysql_fetch_array($q1))
																		{
																?>
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_user_vw WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:520px;">
				                    	...
				                    	<table style="width:50%;">
				                    		...
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select class="form-control" name="posisi" readonly style="width: 50%">
																	<option value=''>Pilih</option>
																<?
																	$q1 = mysql_query("SELECT * FROM x23_posisi ORDER BY posisi");
																	while($dA=mysql_fetch_array($q1))
																		{
																?>
