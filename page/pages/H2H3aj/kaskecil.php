<?
	if($submenu == 'A')
		...
		if($_REQUEST[input]=='1')
			...
			$dcek = mysql_fetch_array(mysql_query("SELECT id FROM x23_kaskecil WHERE id%2=0 AND jenis='$jenis' AND tanggal='$tanggal' AND keterangan='$keterangan' AND jumlah='$jumlah'"));
			if(!empty($dcek[id]))
				{
				print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=$submenu&periode=$_REQUEST[periode]'/>";
				exit();
		...
		if(!empty($_REQUEST[delkas]))
			{
			$q1 = mysql_query("DELETE FROM x23_abis_dkonfirmasi WHERE id%2=0 AND idkaskecil='$_REQUEST[delkas]'");
			$q1 = mysql_query("DELETE FROM x23_kaskecil WHERE id%2=0 AND id='$_REQUEST[delkas]'");
			$q2 = mysql_query("INSERT INTO log_act VALUES (										
		                                    '',
		                                    'x23_kaskecil',
		                                    CURDATE(),
...
?>
			<aside class="right-side">
			    <section class="content">
			        ...
			        <?
			        $qM = mysql_query("SELECT * FROM x23_kaskecil WHERE id%2=0 AND tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' ORDER BY id DESC");
				                ...
				                <div class="modal-content">
			                        ...
			                        <div class="modal-body" style="overflow-y:auto;overflow-x:hidden;height:500px;">
										<table id="example4" class="table table-striped">
				                            ...
				                            <?
				                            $no = 1;
				                            $q1 = mysql_query("SELECT * FROM log_act WHERE id%2=0 AND tbl='x23_kaskecil' ORDER BY id DESC LIMIT 100");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
				                                <tr>
