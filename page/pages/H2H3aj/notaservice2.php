...
<?
	if($submenu == 'saveB')
		...
		$qA = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
        while($dA = mysql_fetch_array($qA))
        	{
			mysql_query("UPDATE x23_stokpart SET stok=stok-$dA[qty] WHERE id%2=0 AND idgudang='$dA[idgudang]' AND rak='$dA[rak]' AND nonota='$dA[notabeli]' AND idbarang='$dA[idbarang]'");
			}
		...
		$q1 = mysql_query("UPDATE x23_penagihankpb SET nonotaservis='$_REQUEST[nonota]' WHERE id%2=0 AND nopkb='$_REQUEST[nopkb]'");
						
        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM x23_kwitansi WHERE id%2=0 AND jnskwitansi='servis' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
		...
		$q1 = mysql_query("INSERT INTO x23_kwitansi (
						...
						");
		/*				
		$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice WHERE id%2=0 AND tglnota=CURDATE() AND status='0' ORDER BY noantrian ASC LIMIT 0,1"));
		mysql_query("UPDATE x23_notaservice SET jammulai=CURTIME() WHERE id%2=0 AND noantrian='$dB[noantrian]'");
		mysql_query("UPDATE x23_antrian SET status='1',jammulai=CURTIME() WHERE id%2=0 AND tanggal=CURDATE() AND noantrian='$dB[noantrian]'");
		*/
		...
		$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_pelanggan WHERE id%2=0 AND ohc='$ohc'"));		
...
<?
					if(empty($mod))
						...
						if(!empty($_REQUEST[del]))
							{
							mysql_query("DELETE FROM x23_notaservice WHERE id%2=0 AND id='$_REQUEST[del]'");
							mysql_query("DELETE FROM x23_antrian WHERE id%2=0 AND noantrian='$_REQUEST[noantrian]'");
							}
?>
			            <div class="col-xs-12">	     
									...
									<table id="example1" class="table table-striped table-hover" style="width:120%">
			                            <thead style="color:#666;font-size:13px">
			                                <tr>
			                                    ...
			                                    <th width="" style="padding:7px">NO. NOTA SERVIS</th>
			                                    <th width="" style="padding:7px">WAKTU MULAI</br>SERVIS</th>
			                                    <th width="9%" style="padding:7px">NO. ANTRIAN</th>
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM x23_notaservice_vw WHERE id%2=0 AND status='0'");
										while($d1 = mysql_fetch_array($q1))
			                            	{
											$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id='$d1[idmekanik]'"));
											//$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_user_vw WHERE id%2=0 AND id='$d1[idmekanik]'"));
			                            ?>
			                                <tr style="cursor:pointer">
			                                    <td onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=B&id=$d1[id]"?>'"><?echo date("d-m-Y",strtotime($d1[tglnota]))?></td>
			                                    <td onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=B&id=$d1[id]"?>'"><?echo $d1[nonota]?></td>
			                                    <td onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=B&id=$d1[id]"?>'" align="center"><?echo $d1[jammulai]?></td>
...
<?
		...
		{
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice_vw WHERE id%2=0 AND id='$_REQUEST[id]'"));
		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND nomesin='$d1[nomesin]' AND status='TERJUAL'"));
		...
		/*
        $dNS = mysql_fetch_array(mysql_query("SELECT nonota FROM x23_notaservice WHERE id%2=0 AND tglselesai=CURDATE() ORDER BY SUBSTR(nonota,-3,3) DESC LIMIT 1"));
			                	...
			                	<h4>SERVIS <small>INPUT NOTA SERVIS</small></h4>
				                	...
				                	<div style="padding:10px">
					           			...
					           			<div class="col-xs-6">
					                        <table width="100%">
					                        	<tr>
					                        		<td width="35%">TGL NOTA SERVIS</td>
					                        		<td width="3%">:</td>
						           			...
						           			<div class="col-xs-12">
						                    	<table width="90%">
						                    		<tr>
						                    			...
						                    			<td colspan="2"><select name="idmekanik" class="form-control select1" style="font-size:12px;padding:3px;width:60%" disabled="">
																					<option value='' selected>Pilih</option>
																						<?
																							$q1 = mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND posisi='4' ORDER BY nama");
						                    		...
						                    		<tr>
						                    			<td>NOMOR RANGKA</td>
						                    			<td>:</td>
						                    			<td colspan="2"><input type="text" name="norangka" class="form-control" style="width: 40%" value="<?echo $d2[norangka]?>" maxlength="30" readonly=""></td>
						                    		</tr>
													<?
														}
		                        	...
		                        	if(!empty($_SESSION[noservis]))
		                        		{
		                        		$dS = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice WHERE id%2=0 AND nonota='$_SESSION[noservis]'"));
									?>
					                	<div style="padding:10px">
						           			<div class="col-xs-12">
						                    	<table width="90%">
						                    		...
						                    		<tr>
						                    			<td>NOMOR RANGKA</td>
						                    			<td>:</td>
						                    			<td colspan="2"><input type="text" name="norangka" class="form-control" style="width: 40%" value="<?echo $dS[norangka]?>" maxlength="30" readonly=""></td>
						                    		</tr>
				                            	</table>
			                            	</div>
			                            </div>
...
<?
		...
		if(!empty($_REQUEST[norangka]))
			{				
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_masterbarang WHERE id%2=0 AND kodebarang='$d1[kodemotor]'"));
		...
		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id='$d1[idmekanik]'"));
		$d3 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice_det1_vw WHERE id%2=0 AND nonota='$_SESSION[nonota]'"));
					
			
		if(!empty($_REQUEST[temp]))
			{
			$dCek1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_det WHERE id%2=0 AND notabeli='$_REQUEST[notabeli]' AND idgudang='$_REQUEST[idgudang]' AND 
																						 rak='$_REQUEST[rak]' AND nonota='$_REQUEST[temp]' AND idbarang='$_REQUEST[idbarang]'"));
								
			$dStok = mysql_fetch_array(mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$_REQUEST[idbarang]' AND nonota='$_REQUEST[notabeli]' AND idgudang='$_REQUEST[idgudang]' AND 
																						 rak='$_REQUEST[rak]'"));	
			...
			if(empty($dStok[id])){
				...
				if($qty <= $dStok[stok])
					{
					$dCek2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_indent_det WHERE id%2=0 AND noindent='$_SESSION[noindent]' AND idbarang='$dStok[idbarang]'"));
					if(!empty($dCek2[id])){
					...
					if($_REQUEST[oli]=="1")
						{
						$dKP = mysql_fetch_array(mysql_query("SELECT * FROM x23_kelompokjasa WHERE id%2=0 AND kode='$d3[kodepaket]'"));
						if($dStok[stok] < $dKP[qtyoli]){
							echo "<script>alert ('Proses Gagal, Karena Stok Oli Tidak Mencukupi.')</script>";
							print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&mod=$mod&submenu=$submenu&id=$_REQUEST[id]'/>";
							exit();
							}
		...
		if(!empty($_REQUEST[del]))
			...
			mysql_query("DELETE FROM x23_claimoli_det WHERE id%2=0 AND id_notajual_det='$_REQUEST[del]'");
			}
		...
		if(!empty($_REQUEST[tempsvc]))
			...
			if($_REQUEST[jnsservice]=='PAKET')
				{
				/*
				$qA = mysql_query("SELECT * FROM x23_kelompokjasa_det_vw WHERE id%2=0 AND kode='$_REQUEST[kodepaket]'");
				while($dA=mysql_fetch_array($qA))
					{
					$dB=mysql_fetch_array(mysql_query("SELECT * FROM x23_kelompokjasa WHERE id%2=0 AND kode='$_REQUEST[kodepaket]'"));
				...
				*/	
					...
					$dA=mysql_fetch_array(mysql_query("SELECT * FROM x23_kelompokjasa WHERE id%2=0 AND kode='$_REQUEST[kodepaket]'"));
					...
					if($_REQUEST[optdis] == "2")
						{
						$diskon	= ROUND((($dA[harga]*$_REQUEST[diskon1])/100),0);
						}
					...
					$dCek1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]' AND tglnota=CURDATE() AND 
			...
			if($_REQUEST[jnsservice]=='RETAIL')
				{
					$dB=mysql_fetch_array(mysql_query("SELECT * FROM x23_tarifjasa WHERE id%2=0 AND idjasa='$_REQUEST[idjasa]'"));
					...
					if($_REQUEST[optdis] == "2")
						{
						$diskon	= ROUND((($dB[tarif]*$_REQUEST[diskon1])/100),0);
						}
		...
		if(!empty($_REQUEST[delsvc]))
			{
			mysql_query("DELETE FROM x23_notaservice_det WHERE id%2=0 AND id='$_REQUEST[delsvc]'");
			$dCsvc = mysql_fetch_array(mysql_query("SELECT *,COUNT(tarif) AS tqty, SUM(tarif) AS ttotal, SUM(tarifasli) AS tottarifasli, SUM(diskon) AS totdiskon FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]'"));
			
			//echo "<script>alert ('$dCsvc[ttotal]')</script>";
			//exit();
						
			if(empty($dCsvc[ttotal])){
		...
		if(!empty($_REQUEST[back]))
			{
			mysql_query("DELETE FROM x23_notajual_det WHERE id%2=0 AND nonota='$_SESSION[nonota]'");
			mysql_query("DELETE FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]' AND nopkb=''");
			print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=A'/>";
			exit();
			}	
			
		$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(qty) AS tqty, SUM(total) AS ttotal, SUM(totdiskon) AS totdiskon, SUM(tothargabelibersih) AS tothargabelisp FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_SESSION[nonota]'"));
		$dC = mysql_fetch_array(mysql_query("SELECT *,COUNT(tarif) AS tqty, SUM(tarif) AS ttotal, SUM(tarifasli) AS tottarifasli, SUM(diskon) AS totdiskon FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]'"));
		$ppnjasa = round($dC[ttotal] * 0.1 , 0);
		$ttotal = $dB[ttotal]+$dC[ttotal]+$ppnjasa;
		
		mysql_query("UPDATE x23_notaservice_det SET nonota='$_SESSION[nonota]' WHERE id%2=0 AND nopkb='$d1[nopkb]'");
?>
			                	...
			                	<h4>SERVIS <small>INPUT NOTA SERVIS</small></h4>
				                	...
				                	<form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=saveB"?>" enctype="multipart/form-data">
				           			<div class="col-xs-12">
										...
										<?
				                            $dA1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_SESSION[nonota]'"));
				                            $dA2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]'"));
				                            /*
					                	...
					                	<?
											...
											$dcp = mysql_fetch_array(mysql_query("SELECT id,kodepaket FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]' AND tarifkpb NOT IN ('','0')"));
											if(!empty($dcp[id])){
												$dKP = mysql_fetch_array(mysql_query("SELECT * FROM x23_kelompokjasa WHERE id%2=0 AND kode='$d3[kodepaket]'"));
												$dcp2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_SESSION[nonota]' AND total='0' AND totdiskon='0' AND qty='$dKP[qtyoli]'"));	
												if(empty($dcp2[id])){
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:270px;">
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qA  = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$_SESSION[nonota]' ORDER BY id DESC");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
			                            ?>
			                                <tr style="cursor:pointer">
										...
										$qA = mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]'");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
			                            ...
			                            if(!empty($_SESSION[noclaim]))
			                            	{
											$qA = mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[noservis]'");
				                            while($dA = mysql_fetch_array($qA))
				                            	{
				                            	if(!empty($dA[kodepaket]))
				                            		{
				            ...
				            <div class="modal-dialog" style="width:65%;">
				                <div class="modal-content">
									...
									function populateSelectB(str)
										{
											pilihan = document.service.optdis.value;
											if(pilihan==''){
											document.service.diskon1.disabled = 1;
											document.service.diskon2.disabled = 0;
											}
											else if(pilihan=='1'){
											document.service.diskon1.disabled = 1;
											document.service.diskon2.disabled = 0;
											}else{
											document.service.diskon1.disabled = 0;
											document.service.diskon2.disabled = 1;
											}
										}
									</script>
			                        ...
			                        <div class="modal-body">
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			...
				                    			<td width="2%">:</td>
				                    			<td width="" colspan="3"><select name="kodepaket" class="form-control select1" style="font-size:12px;padding:3px;width:100%" disabled="">
																		<option value='' selected>Pilih</option>
																		<!--
																		<?
																		...
																		-->
																		<?
																			$dCkpb = mysql_fetch_array(mysql_query("SELECT id FROM x23_notaservice_det WHERE id%2=0 AND nonota='$_SESSION[nonota]' AND tarifkpb NOT IN ('','0')"));
																			if(empty($dCkpb[id])){
																				//$k1 = "'LAINNYA','KPB'";
																				$k1 = "'LAINNYA'";
																				}
																			...
																			$q1 = mysql_query("SELECT * FROM x23_kelompokjasa WHERE id%2=0 AND jnskj IN ($k1) AND status='1' ORDER BY nama");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
				                    		...
				                    		<tr>
				                    			<td width="20%">JASA</td>
				                    			<td width="2%">:</td>
				                    			<td width="" colspan="3"><select name="idjasa" class="form-control select1" style="font-size:12px;padding:3px;width:100%" disabled="">
																		<option value='' selected>Pilih</option>
																		<?
																			...
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
																					<option value='<?echo $dA[idjasa]?>'><?echo "$dA[namajasa] | $dA[kodejasa]"?></option>
																		<?
																				}
																		?>
																    </select></td>
				                    		...
				                    		<tr>
				                    			<td>DISKON</td>
				                    			<td>:</td>
				                    			<td colspan="3" width=""><select name="optdis" class="form-control" style="font-size:12px;padding:3px;width:8%" onchange="populateSelectB(this.value)" required="">
				                    		...
				                    		<tr>
				                    			<td></td>
				                    			<td></td>
				                    			<td width="17%"><div class="input-group">
				                                    	...
				                                    	<span class="input-group-addon" style="width:60%;text-align:left">% Per PCS</span>
				                                    </div>
						                        </td>
				                    			<td></td>
				                    		</tr>
				                ...
				                <div class="modal-content">
			                        ...
			                        <div class="modal-body">
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			<td>NO. NOTA BELI / TG NOTA BELI</td>
				                    			<td>:</td>
				        ...
				        <div class="modal fade " id="compose-modal-tambaholi" tabindex="-1" role="dialog" aria-hidden="true">
				            <div class="modal-dialog" style="width:55%;">
				                <div class="modal-content">
				                    <div class="modal-header">
				                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				                        <h4 class="modal-title">PILIH OLI <?//echo $dB[kodejasa]?></h4>
				                    </div>
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="idbarang" class="form-control select1" onchange="populateSelectNO1(this.value)" style="font-size:12px;padding:3px" required="">
																		<option value='' selected>Pilih</option>
																		<?
																		$q1 = mysql_query("SELECT * FROM x23_kelompokjasa_oli_vw WHERE id%2=0 AND kode='$kodekpb'");
																		while($dA=mysql_fetch_array($q1))
																			{
																			$dB = mysql_fetch_array(mysql_query("SELECT id FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$dA[idoli]' AND stok>'0' AND hargajual!=''"));
																			if(!empty($dB[id])){
																		?>
...
<?
	...
	if($submenu == 'NJ1')
		...
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$q' AND stok>'0' AND hargajual!='' GROUP BY nonota,idbarang ORDER BY nonota");
	...
	if($submenu == 'NJ2')
		...
		echo "<option value=''>Pilih</option>";
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$_SESSION[idbarang]' AND nonota='$q' AND stok>'0' GROUP BY nonota,idbarang,gudang ORDER BY gudang");
		while($d = mysql_fetch_array($qA))
			{
	...
	if($submenu == 'NJ3')
		...
		echo "<option value=''>Pilih</option>";
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$_SESSION[idbarang]' AND  nonota='$_SESSION[notabeli]' AND tglnota='$q' AND stok>'0' GROUP BY nonota,idbarang,tglnota,gudang ORDER BY gudang");
		while($d = mysql_fetch_array($qA))
			{
			echo "<option value='$d[idgudang]'>$d[gudang]</option>";
			}
	...
	if($submenu == 'NJ4')
		...
		echo "<option value=''>Pilih</option>";
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND idbarang='$_SESSION[idbarang]' AND  nonota='$_SESSION[notabeli]' AND idgudang='$q' AND stok>'0' GROUP BY nonota,idbarang,tglnota,gudang,rak ORDER BY rak");
		while($d = mysql_fetch_array($qA))
			{
