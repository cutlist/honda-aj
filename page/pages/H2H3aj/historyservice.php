                                    	...
                                    	<table width="100%">
                                    		<tr>
                                    			<td>
		                                            	...
		                                            	<input type="text" style="height:34px" name="cari" autofocus placeholder="CARI NO. NOTA SERVIS / NO. NOTA SERVIS JR / NO. PKB / NAMA PELANGGAN ..." class="form-control"/>
		                                            </div>
                                    			</td>
...
<?
		...
		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id='$d1[idmekanik]'"));
		$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(qty) AS tqty, SUM(total) AS ttotal, SUM(totdiskon) AS totdiskon, SUM(tothargabelibersih) AS tothargabelisp FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$d1[nonota]'"));
		$dC = mysql_fetch_array(mysql_query("SELECT *,COUNT(tarif) AS tqty, SUM(tarif) AS ttotal, SUM(tarifasli) AS tottarifasli, SUM(diskon) AS totdiskon FROM x23_notaservice_det WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:auto;height:270px;">
			                        <table id="example2" class="table table-striped table-hover" style="width:130%">
			                            ...
			                            $dA1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$d1[nonota]'"));
										
										$qA  = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND nonota='$d1[nonota]'");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
											$dNbl = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE id%2=0 AND nonota='$dA[notabeli]'"));
			                            ?>
			                            ...
			                            $dA2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$d1[nonota]'"));
										$qA = mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$d1[nonota]'");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
			                            ...
			                            $dA2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$d1[noservis]' AND nonota!=''"));
										$qA = mysql_query("SELECT * FROM x23_notaservice_det WHERE id%2=0 AND nonota='$d1[noservis]' AND nonota!=''");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
			                            	if(!empty($dA[kodepaket]))
			                            		{
										...
										$dA3 = mysql_fetch_array(mysql_query("SELECT * FROM x23_kwitansi WHERE id%2=0 AND jnskwitansi='servis' AND nomor='$d1[nonota]'"));
			                            ?>
			                            </tbody>
			                            <tfoot>
			                            	<tr>
