	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$dCek2 = mysql_fetch_array(mysql_query("SELECT tarif FROM x23_tarifjasa2_vw WHERE id%2=0 AND idjasa='$idjasa' AND pangkat='$pangkat' ORDER BY tarif DESC LIMIT 0,1"));
				...
				$dCek3 = mysql_fetch_array(mysql_query("SELECT id FROM x23_tarifjasa WHERE id%2=0 AND idjasa='$idjasa' AND pangkat='$pangkat' ORDER BY tarif DESC LIMIT 0,1"));
				if(!empty($dCek3[id]))
					{
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_tarifjasa WHERE id%2=0 AND id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_tarifjasa',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[ubah]))
				...
				*/
				$dCek2 = mysql_fetch_array(mysql_query("SELECT tarif FROM x23_tarifjasa2_vw WHERE id%2=0 AND idjasa='$_REQUEST[idjasa]' AND pangkat='$_REQUEST[pangkat]' ORDER BY tarif DESC LIMIT 0,1"));
				if(!empty($dCek2[tarif]))
					{
					if($dCek2[tarif] > $tarif)
						{
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_tarifjasa WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:520px;">
