	...
	if($submenu == 'r1')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM tbl_pelanggan WHERE id%2=0 AND id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'tbl_pelanggan',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[delhleasing]))
				{
				$q1 = mysql_query("DELETE FROM tbl_hleasing WHERE id%2=0 AND id='$_REQUEST[delhleasing]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'tbl_hleasing',
			                                    CURDATE(),
			                	...
			                	<h4>PELANGGAN <small>DAFTAR PELANGGAN &nbsp; <i class="fa fa-angle-right"></i> &nbsp; Lihat Detail Pelanggan</small></h4>
			                		<div style="padding:20px">
										...
										<table class="table table-striped" >
				                            ...
				                            <?
				                            $no = 1;
				                            $q1 = mysql_query("SELECT * FROM tbl_hleasing_vw WHERE id%2=0 AND id_pelanggan='$_REQUEST[id]' ORDER BY tanggal DESC");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            	if($d1[status]=='1'){
...
<?
		...
		if(!empty($_REQUEST[idpelanggan]))
			...
			$q2 = mysql_query("UPDATE tbl_pelanggan SET ohc='$ohc', kadaluarsaohc='$kadaluarsaohc' WHERE id%2=0 AND id='$_REQUEST[idpelanggan]'");
			}
?>
			<aside class="right-side">
			    <section class="content">
...
?>
			                ...
			                <div class="box box-danger">
								...
								?>	
	                           		<table id="example1" class="table table-bordered table-striped">
										...
										$q1 = mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND kadaluarsaohc BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											if($d1[kadaluarsaohc]=="0000-00-00"){
												$kadaluarsaohc = "-";
		            ...
		            <?
						$qA = mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND kadaluarsaohc!='0000-00-00' AND kadaluarsaohc <= CURDATE()");
			            while($dA = mysql_fetch_array($qA))
			            	{
		            ?>
	<!-- ################## MODAL RENEW OHC ########################################################################################## -->
				                ...
				                <div class="modal-content">
			                        ...
			                        <div class="modal-body"style="overflow-y:auto;overflow-x:hidden;height:520px;">
										<table id="example4" class="table table-striped" >
				                            ...
				                            <?
				                            $no = 1;
				                            $q1 = mysql_query("SELECT * FROM log_act WHERE id%2=0 AND tbl='tbl_pelanggan' ORDER BY id DESC");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
				                                <tr>
