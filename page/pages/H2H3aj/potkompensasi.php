	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$dck = mysql_fetch_array(mysql_query("SELECT id FROM x23_potkompensasi WHERE id%2=0 AND idkaryawan='$idkaryawan' AND tgl='$tgl' AND jumlah='$jumlah' AND metodebayar='$_REQUEST[metodebayar]' AND ket='$ket'"));
				if(!empty($dck[id]))
					{
				...
				if($_REQUEST[metodebayar]=='GAJI')	
					{
					$dCG = mysql_fetch_array(mysql_query("SELECT ugapok FROM x23_karyawan_vw WHERE id%2=0 AND id='$idkaryawan'"));
				...
				if($_REQUEST[metodebayar]=='TUNAI')	
			        ...
			        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM x23_kwitansi WHERE id%2=0 AND jnskwitansi='tunai' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
				...
				{	
				$q1 = mysql_query("DELETE FROM x23_potkompensasi WHERE id%2=0 AND id='$_REQUEST[del]'");
				$q2 = mysql_query("DELETE FROM x23_kwitansi WHERE id%2=0 AND nomor='$_REQUEST[del]'");
				$q3 = mysql_query("DELETE FROM x23_abis_dkonfirmasi WHERE id%2=0 AND idpotkompensasi='$_REQUEST[del]'");
			                        ...
			                        <table id="example1" class="table table-hover table-striped">
			                            ...
			                            if(empty($_REQUEST[periode])){
											if(!empty($_REQUEST[cari])){
												$q1 = mysql_query("SELECT * FROM x23_potkompensasi WHERE id%2=0 AND (nama LIKE '%$_REQUEST[cari]%' OR metodebayar LIKE '%$_REQUEST[cari]%' OR tgl LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
												}
											else{
			                            ...
			                            if(!empty($_REQUEST[periode])){
											...
											if(!empty($_REQUEST[cari])){
												$q1 = mysql_query("SELECT * FROM x23_potkompensasi WHERE id%2=0 AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND (nama LIKE '%$_REQUEST[cari]%' OR metodebayar LIKE '%$_REQUEST[cari]%' OR tgl LIKE '%$_REQUEST[cari]%') LIMIT 0,20");
												}
											else{
												$q1 = mysql_query("SELECT * FROM x23_potkompensasi WHERE id%2=0 AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' ORDER BY id DESC LIMIT 0,20");
												}
											}
											
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_kwitansi WHERE id%2=0 AND idpotkom='$d1[id]' AND jnskwitansi='tunai'"));
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="karyawan" class="form-control" id="select1" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			$q1 = mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id!='1' AND status='AKTIF' AND posisi!= '6' ORDER BY nama");
...
<?
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				{	
				$dPiuX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_potkompensasi WHERE id%2=0 AND jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
				$dPbyX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_potkompensasi WHERE id%2=0 AND jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>SDM <small>POTONGAN KOMPENSASI &nbsp; <i class="fa fa-angle-right"></i> &nbsp; Lihat Riwayat Piutang</small></h4>
			                		...
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
					                    	...
					                    	<table width="70%">
					                    		<tr>
					                    			...
					                    			<td colspan="2"><select class="form-control" name="posisi" readonly style="width: 50%">
																		<option value=''>Pilih</option>
																	<?
																		$q1 = mysql_query("SELECT * FROM x23_posisi WHERE id%2=0 AND id!='1' ORDER BY posisi");
																		while($dA=mysql_fetch_array($q1))
																			{
																	?>
										...
										<?
										$dPiu = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_potkompensasi WHERE id%2=0 AND jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
										$dPby = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_potkompensasi WHERE id%2=0 AND jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
					                    		...
					                    		<tr>
					                    			<td align="center">
									                    ...
									                    <table id="example2" class="table table-striped" style="width:100%"> 
															...
															<tbody>
								                    	<?
								                    		$qA = mysql_query("SELECT * FROM x23_potkompensasi WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' ORDER BY tgl DESC");
