			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                    ...
			                    ?>
				                        <table id="example3" class="table table-bordered table-striped table-hover" style="width:100%;padding-right:20px">
				                            ...
				                            <tbody>
					                        <?
												$q1	 = mysql_query("SELECT * FROM x23_abis_dkonfirmasi WHERE id%2=0 AND bulan='$periode_bulan'  AND tahun='$periode_tahun' GROUP BY inputx ORDER BY id DESC");
...
<?
		...
		{
		$dX = mysql_fetch_array(mysql_query("SELECT * FROM x23_abis_dkonfirmasi WHERE id%2=0 AND id='$_REQUEST[id]'"));
		if(!empty($dX[idkaskecil]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_kaskecil WHERE id%2=0 AND id='$dX[idkaskecil]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
...
<?
		...
		if(!empty($dX[idnotabeli]))
			{
			$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE id%2=0 AND id='$dX[idnotabeli]'"));
			$d2 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id%2=0 AND id='$d1[iduserkonf]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                	...
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI QTY TIBA</small></h4>	
									...
									<?
									$dC2 = mysql_fetch_array(mysql_query("SELECT jns FROM x23_notabeli WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                			...
			                			<div style="padding:20px;overflow-y:auto;overflow-x:hidden;height:300px;margin-top:-20px">
											...
											?>
						                        ...
						                        <table class="table table-striped" id="example4">
						                            ...
						                            while($dA = mysql_fetch_array($q1))
						                            	{
														$dC = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id%2=0 AND id='$dA[idbarang]]'"));
														$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_stokpart WHERE id%2=0 AND nonota='$dA[nonota]' AND idbarang='$dA[idbarang]'"));
														$dD = mysql_fetch_array(mysql_query("SELECT gudang FROM x23_gudang WHERE id%2=0 AND id='$dB[idgudang]]'"));
						                            	if($dB[stok] != $dA[qty]){
							                                if($d1[konf]=='1')
							                                	{
																$red = "color:#ff0227";
...
<?
		...
		if(!empty($dX[idnotabeli2]))
			{
			$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE id%2=0 AND id='$dX[idnotabeli2]'"));
			$d2 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id%2=0 AND id='$d1[iduserbyr]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:hidden;height:300px;margin-top:-20px">
										...
										?>
					                        ...
					                        <table id="example4" class="table table-striped table-hover">
					                            ...
					                            <tbody>
					                            <?
												$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND nonota='$d1[nonota]'");
					                            while($dA = mysql_fetch_array($qA))
					                            	{
					                            ?>
...
<?
		...
		if(!empty($dX[idreturbeli]))
			{   
			$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_returbeli_vw WHERE id%2=0 AND id='$dX[idreturbeli]'"));
			$d2 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id%2=0 AND id='$d1[idgdg]'"));
			$d3 = mysql_fetch_array(mysql_query("SELECT SUM(qtykeluar) AS total FROM x23_returbeli_det WHERE id%2=0 AND noretur='$d1[noretur]' AND tanggal='$d1[tanggal]'"));  
			                	...
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI RETUR BELI</small></h4>	
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:auto;height:300px;margin-top:-20px">
				                        ...
				                        <table id="example4" class="table table-striped table-hover" style="width:160%">
				                            ...
				                            <?
												$cs = 1;
												$dZ = mysql_fetch_array(mysql_query("SELECT SUM(qtykeluar) AS qtykeluar,SUM(totalkeluar) AS totalkeluar FROM x23_returbeli_det_vw WHERE id%2=0 AND noretur='$d1[noretur]'"));
												$qY = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE id%2=0 AND noretur='$d1[noretur]'");
					                            while($dY = mysql_fetch_array($qY))
					                            	{
													$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det2_vw WHERE id%2=0 AND idbarang='$dY[idbarang]' AND idgudang='$dY[idgudang]' AND rak='$dY[rak]' AND nonota='$dY[nonota]'"));
													$dSt = mysql_fetch_array(mysql_query("SELECT stok FROM x23_stokpart WHERE id%2=0 AND idbarang='$dY[idbarang]' AND idgudang='$dY[idgudang]' AND rak='$dY[rak]' AND nonota='$dY[nonota]'"));
...
<?
		...
		if(!empty($dX[idpiutang]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_piutang_vw WHERE id%2=0 AND id='$dX[idpiutang]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
...
<?
		...
		if(!empty($dX[idbyrpiutang]))
			...
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_piutang_vw WHERE id%2=0 AND id='$dX[idbyrpiutang]'"));
			
			$dPiu = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND jenis='piutang' AND idkaryawan='$dA[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
			$dPby = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND jenis='pembayaran' AND idkaryawan='$dA[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
			
			$totpiutang    = $dPiu[total];
			$totpembayaran = $dPby[total];
			$sisapiutang   = $dPiu[total]-$dPby[total];
			
			$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id='$dA[idkaryawan]'"));
			$dC = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS totjum FROM x23_potkompensasi WHERE id%2=0 AND idkaryawan='$dA[idkaryawan]' AND metodebayar='GAJI' AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND status='1'"));
			$dD	= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$dA[idkaryawan]' AND metodebayar='GAJI' AND status='1'"));
...
<?
		...
		if(!empty($dX[idpotkompensasi]))
			...
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_potkompensasi_vw WHERE id%2=0 AND id='$dX[idpotkompensasi]'"));
			$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id='$dA[idkaryawan]'"));
			$dC = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS totjum FROM x23_potkompensasi WHERE id%2=0 AND idkaryawan='$dA[idkaryawan]' AND metodebayar='GAJI' AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND status='1'"));
			$dD	= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$dA[idkaryawan]' AND metodebayar='GAJI' AND status='1'"));
...
<?
		...
		if(!empty($dX[idpindah]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_pindah WHERE id%2=0 AND id='$dX[idpindah]'"));
			$d2 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id%2=0 AND id='$dA[user]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			                	...
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI PINDAH LOKASI</small></h4>		
				                    	...
				                    	<table width="90%">
				                        	...
				                        	<tr>
				                        		...
				                        		<td><select name="rak1" class="form-control" disabled style="width:100%">
																...
																<?
																	$qB = mysql_query("SELECT * FROM x23_stokpart WHERE id%2=0 AND idgudang='$dA[idgudang1]' GROUP BY rak ORDER BY rak");
																	while($dB=mysql_fetch_array($qB))
																		{
																?>
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:hidden;height:210px;margin-top:-20px">
				                        <table id="example2" class="table table-striped">
				                            ...
				                            <?
											$no=1;
											$q1 = mysql_query("SELECT * FROM x23_pindah_det_vw WHERE id%2=0 AND idpindah='$dX[idpindah]'");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
				                                <tr style="cursor:pointer">
...
<?
		...
		if(!empty($dX[idopname]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_opname_vw WHERE id%2=0 AND id='$dX[idopname]'"));
			if($dA[totselisih] < 0){
				$selisihX = (-1)*($dA[totselisih]);
				$selisihY = number_format($selisihX,"0","",".");
				$Tselisih = "LEBIH $selisihY PCS";
				}
			else if($dA[totselisih] > 0){
				$selisihY = number_format($dA[totselisih],"0","",".");
				$Tselisih = "KURANG $selisihY PCS";
				}
			else if($dA[totselisih] == 0){
				$Tselisih = "0 PCS";
				}
													
			if($dA[totjumselisih] < 0){
				$totselisihX = (-1)*($dA[totjumselisih]);
				$totselisihY = number_format($totselisihX,"0","",".");
				$ttotselisih = "LEBIH $totselisihY";
				}
			else if($dA[selisih] > 0){
				$totselisihY = number_format($dA[totjumselisih],"0","",".");
				$ttotselisih = "KURANG $totselisihY";
				}
			else if($dA[totjumselisih] == 0){
				$ttotselisih = "0";
				}
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="height:520px;">
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI STOCK OPNAME</small></h4>		
			                		<div style="padding:20px">
				                    	<table width="70%">
				                        	<tr>
				                        		<td width="30%" valign="top">KASUS</td>
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:auto;height:300px;margin-top:-20px">
				                        <table id="example2" class="table table-striped table-hover" style="width:140%">
				                            <thead style="color:#666;font-size:13px">
				                                <tr>
				                                    <th width="5%" style="padding:7px">RAK</th>
				                                    <th width="10%" style="padding:7px"><center>TGL NOTA BELI</center></th>
				                                    <th style="padding:7px">KODE BARANG </th>
				                                    <th style="padding:7px">NAMA BARANG</th>
				                                    <th style="padding:7px">VARIAN</th>
				                                    <th width="10%" style="padding:7px">HARGA BELI (RP)</th>
				                                    <th width="5%" style="padding:7px">STOK</th>
				                                    <th width="5%" style="padding:7px">OPNAME</th>
				                            ...
				                            <tbody>
				                            <?	
											$qIm = mysql_query("SELECT * FROM x23_opname_det_vw WHERE id%2=0 AND idopname='$dX[idopname]'");
				                            ...
				                            <tfoot>
				                            	<tr>
				                            		...
				                            		<td colspan="" align="right"><b><?echo $Tselisih?></b></td>
				                            		<td colspan="" align="right"><span style="margin-right:20%"><b><?echo $ttotselisih?></b></span></td>
				                            	</tr>
				                            </tfoot>
										</table>
...
<?
		...
		if(!empty($dX[idtutupservis]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_tutupservis WHERE id%2=0 AND id='$dX[idtutupservis]'"));
			$d1 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id%2=0 AND id='$dA[iduser]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="height:520px;">
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI TUTUP SERVIS</small></h4>		
			                		<div style="padding:20px">
				                    	<table width="70%">
				                        	<tr>
				                        		<td width="30%" valign="top">KASUS</td>
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:auto;height:300px;margin-top:-20px">
				                        <table id="example4" class="table table-bordered table-striped table-hover">
				                            ...
				                            <tbody>
				                                <tr>
													...
													<td align="right"><span style="margin-right:30%"><?echo $dA[asmulaiservis]?></span></td>
				                                </tr>
				                                <tr>
													<td>Jumlah Motor Masuk Bengkel</td>
													<td align="right"><span style="margin-right:30%"><?echo $dA[asmasukbengkel]?></span></td>
				                                </tr>
				                                <tr>
													<td>Jumlah Motor Keluar Bengkel</td>
													<td align="right"><span style="margin-right:30%"><?echo $dA[askeluarbengkel]?></span></td>
				                                </tr>
				                                <tr>
													<td>Jumlah Motor Selesai Servis</td>
													<td align="right"><span style="margin-right:30%"><?echo $dA[asselesaiservis]?></span></td>
				                                </tr>
				                                <tr>
													<td>Jumlah Kwitansi Servis</td>
													<td align="right"><span style="margin-right:30%"><?echo $dA[askwitansiservis]?></span></td>
				                                </tr>
				                                <tr>
													<td>Jumlah Motor Menginap Hasil Scan Infra Red</td>
													<td align="right"><span style="margin-right:30%"><?echo $dA[asnginap]?></span></td>
				                                </tr>
				                            </tbody>
				                        </table>
					                    	
				                   		<form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=setuju&id=$_REQUEST[id]"?>" enctype="multipart/form-data">
				                        <div class="modal-body">
					                    	<table width="70%">
					                    		<tr>
					                    			<td width="30%">QTY MENGINAP TUTUP SERVIS</td>
					                    			<td width="2%">:</td>
					                    			<td width="25%">
					                                    <div class="input-group">
					                                        <input type="text" name="nginap" value="<?ECHO $dA[nginap]?>" style="width:100%;text-align:right;" class="form-control uang" maxlength="6" onkeypress="return buat_angka(event,'1234567890')" <?if($dX[status]=='0'){?>required<?}else{?>disabled=""<?}?>>
					                	...
					                	<div class="col-xs-12">
						                    ...
						                    if($dX[status]=='0')
						                    	{
											?>
												<button type="submit" class="btn btn-primary pull-left" onclick="return confirm('Anda Yakin Akan Menyetujui?')"><i class="fa fa-thumbs-o-up"></i> &nbsp;Setuju</button>
												<a href="<?echo "?opt=$opt&menu=$menu&submenu=tolak&id=$_REQUEST[id]"?>"><button type="button" style="margin-left: 5px" class="btn btn-danger pull-left" onclick="return confirm('Anda Yakin Akan Menolak?')"><i class="fa fa-thumbs-o-down"></i> &nbsp;Tolak</button></a>
						                    <?
												}	
						                    ?>
...
<?
		...
		if(!empty($dX[idtutupharian]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_tutupharian WHERE id%2=0 AND id='$dX[idtutupharian]'"));
			$d1 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id%2=0 AND id='$dA[iduser]'"));
			
			mysql_query("UPDATE x23_abis_dkonfirmasi SET status='3' WHERE id%2=0 AND id='$_REQUEST[id]'");
			if(!empty($_REQUEST[del])){
				mysql_query("DELETE FROM x23_tutupharian WHERE id%2=0 AND id='$_REQUEST[del]'");
				print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=A&note=5'/>";
				exit();
				}
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="height:520px;">
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI TUTUP HARIAN</small></h4>		
			                		<div style="padding:20px">
				                    	<table width="70%">
				                        	<tr>
				                        		<td width="30%" valign="top">KASUS</td>
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:hidden;height:200px;margin-top:-20px">
					                	...
					                	<div class="col-xs-12">
					                        <div class="modal-footer clearfix">
											<?
											$dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_tutupharian WHERE id%2=0 AND tanggal=CURDATE()"));	
											if(!empty($dCek[id])){
											?>
												<a href="<?echo "?opt=$opt&menu=$menu&submenu=$submenu&id=$_REQUEST[id]&del=$dX[idtutupharian]"?>"><button type="button" style="margin-left: 5px" class="btn btn-danger pull-left" onclick="return confirm('Anda Yakin Akan Membuka Akses?')"><i class="fa fa-stop"></i> &nbsp;Buka Akses</button></a>
						                    <?
												}
											?>
						                      	<button type="button" class="btn btn-warning" onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=A"?>'"><i class="fa fa-mail-reply"></i> &nbsp;Kembali</button>
...
<?
		...
		{
		$dC2 = mysql_fetch_array(mysql_query("SELECT jns,idsupplier FROM x23_notabeli WHERE id%2=0 AND nonota='$_REQUEST[nonota]'"));
		...
		if($dC2[jns]=="PEMBELIAN")
	        ...
	        while($dA = mysql_fetch_array($qA))
	        	{
				$dB = mysql_fetch_array(mysql_query("SELECT id FROM x23_stokpart WHERE id%2=0 AND nonota='$dA[nonota]' AND idbarang='$dA[idbarang]' AND idgudang='$dA[idgudang]'AND rak='$dA[rak]'"));
	        	$id = $dA[id];
		...
		if($dC2[jns]=="CLAIM")
			{
			$qA = mysql_query("SELECT * FROM x23_notabeli_det WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
			
	        while($dA = mysql_fetch_array($qA))
	        	{
				$dB = mysql_fetch_array(mysql_query("SELECT id FROM x23_stokpart WHERE id%2=0 AND nonota='$dA[nonota]' AND idbarang='$dA[idbarang]'"));
	        	$id = $dA[id];
	        	...
	        	if($selisih > "0")
			        ...
			        $dNA = mysql_fetch_array(mysql_query("SELECT nonota FROM x23_notajual_det WHERE id%2=0 AND tglnota=CURDATE() AND notabeli='CLAIM' ORDER BY nonota DESC LIMIT 1"));
					if(empty($dNA[nonota]))
						{
						$dig3=1;
						$dig2=0;	
        ...
        $dC = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS totalqty FROM x23_notabeli_det WHERE id%2=0 AND nonota='$_REQUEST[nonota]'"));
        $dD = mysql_fetch_array(mysql_query("SELECT SUM(tot) AS grandtotal FROM x23_stokpart_gt_vw WHERE id%2=0 AND nonota='$_REQUEST[nonota]'"));		                           
		if($dC2[idsupplier]=="1"){
		...
		$q1 = mysql_query("UPDATE x23_notabeli SET scan='1',totalqty='$dC[totalqty]',grandtotal='$dD[grandtotal]',grandtotalppn=$dD[grandtotal]+$ppn,gtbayar=$dD[grandtotal]+$ppn,updatex='$updatex' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
		
		$q2 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
		...
		{
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");
        while($dA = mysql_fetch_array($qA))
        	{
        	$id = $dA[id];
			...
			if(!empty($_REQUEST[hargajual.$id]) && !empty($_REQUEST[hargabelibersih.$id]))
				{
		    	mysql_query("UPDATE x23_stokpart SET hargajual='$hargajual',hargabelibersih='$hargabelibersih' WHERE id%2=0 AND id='$id'");
				}
        	}
        	
		$q1 = mysql_query("UPDATE x23_notabeli SET harga='1',dk='0' WHERE id%2=0 AND nonota='$_REQUEST[nonota]'");	
								
		$q3 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
		...
		if(!empty($dX[idkaskecil]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_kaskecil SET status='1' WHERE id%2=0 AND id='$dX[idkaskecil]'");
		...
		if(!empty($dX[idreturbeli]))
			{		
			$dZ = mysql_fetch_array(mysql_query("SELECT * FROM x23_returbeli WHERE id%2=0 AND id='$dX[idreturbeli]'"));
			$qY = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE id%2=0 AND noretur='$dZ[noretur]'");
            while($dY = mysql_fetch_array($qY))
            	{
				$dSt = mysql_fetch_array(mysql_query("SELECT stok FROM x23_stokpart WHERE id%2=0 AND idbarang='$dY[idbarang]' AND idgudang='$dY[idgudang]' AND rak='$dY[rak]' AND nonota='$dY[nonota]'"));
        		if($dSt[stok] < $dY[qtykeluar]){
			...
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_returbeli SET status='2',qtykeluar='$_REQUEST[qtykeluar]',totalkeluar='$_REQUEST[totalkeluar]',updatex='$updatex' WHERE id%2=0 AND id='$dX[idreturbeli]'");
							 
			$qY = mysql_query("SELECT * FROM x23_returbeli_det WHERE id%2=0 AND noretur='$_REQUEST[noretur]'");
            while($dY = mysql_fetch_array($qY))
            	{
            	mysql_query("UPDATE x23_stokpart SET stok=stok-$dY[qtykeluar] WHERE id%2=0 AND idbarang='$dY[idbarang]' AND idgudang='$dY[idgudang]' AND rak='$dY[rak]' AND nonota='$_REQUEST[nonota]'");
            	}
		...
		if(!empty($dX[idpiutang]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_piutang SET status='0',updatex='$updatex' WHERE id%2=0 AND id='$dX[idpiutang]'");
			$q3 = mysql_query("UPDATE x23_kwitansi SET status='1' WHERE id%2=0 AND nomor='$dX[idpiutang]'");
		...
		if(!empty($dX[idbyrpiutang]))
			{ 	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_piutang SET status='0',updatex='$updatex' WHERE id%2=0 AND id='$dX[idbyrpiutang]'");
			
			$dcek = mysql_fetch_array(mysql_query("SELECT * FROM x23_piutang WHERE id%2=0 AND id='$dX[idbyrpiutang]'"));
			if($dcek[metodebayar]=="GAJI"){
				mysql_query("UPDATE x23_piutang SET status='1',updatex='$updatex' WHERE id%2=0 AND id='$dX[idbyrpiutang]'");
				}
			
			$q3 = mysql_query("UPDATE x23_kwitansi SET status='1' WHERE id%2=0 AND nomor='$dX[idbyrpiutang]'");
		...
		if(!empty($dX[idpotkompensasi]))
			...
			if($_REQUEST[metodebayar]=='TUNAI')
				{
				$q2 = mysql_query("UPDATE x23_potkompensasi SET status='0',updatex='$updatex' WHERE id%2=0 AND id='$dX[idpotkompensasi]'");
				$q3 = mysql_query("UPDATE x23_kwitansi SET status='1' WHERE id%2=0 AND idpotkom='$dX[idpotkompensasi]'");
				$note = "A1";
				//echo "<script>alert ('Proses Berhasil, Mohon Melanjutkan Ke Menu Kwitansi Pada Bagian Kasir.')</script>";	
				}
			else{
				$q2 = mysql_query("UPDATE x23_potkompensasi SET status='1',updatex='$updatex' WHERE id%2=0 AND id='$dX[idpotkompensasi]'");
				}
			}
		...
		if(!empty($dX[idpindah]))
			{                       	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_pindah SET status='1' WHERE id%2=0 AND id='$dX[idpindah]'");
			
	        //$dA = mysql_fetch_array(mysql_query("SELECT idgudang2 FROM tbl_pindah WHERE id%2=0 AND id='$dX[idpindah]'"));
			
			$qA = mysql_query("SELECT * FROM x23_pindah_det WHERE id%2=0 AND idpindah='$dX[idpindah]'");
	        while($dA = mysql_fetch_array($qA))
	        	{
				...
				mysql_query("UPDATE x23_stokpart SET stok=stok-$dA[qty], updatex='$updatex' WHERE id%2=0 AND idgudang='$dA[idgudang1]' AND rak='$dA[rak1]' AND idbarang='$dA[idbarang]' AND nonota='$dA[nonota]' LIMIT 1");
        		}
		...
		if(!empty($dX[idopname]))
			{
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_opname SET status='1' WHERE id%2=0 AND id='$dX[idopname]'");
			
			$qA = mysql_query("SELECT * FROM x23_opname_det WHERE id%2=0 AND idopname='$dX[idopname]' AND opname!=''");
	        while($dA = mysql_fetch_array($qA))
	        	{
				mysql_query("UPDATE x23_stokpart SET stok='$dA[opname]' WHERE id%2=0 AND id='$dA[idstok]'");
				}
		...
		if(!empty($dX[idtutupservis]))
			...
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='1' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_tutupservis SET status='1',nginap='$nginap' WHERE id%2=0 AND id='$dX[idtutupservis]'");
		...
		if(!empty($dX[idkaskecil]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_kaskecil SET status='2' WHERE id%2=0 AND id='$dX[idkaskecil]'");
		...
		if(!empty($dX[idreturbeli]))
			{                       	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_returbeli SET status='3',updatex='$updatex' WHERE id%2=0 AND id='$dX[idreturbeli]'");
		...
		if(!empty($dX[idpiutang]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_piutang SET status='2',updatex='$updatex' WHERE id%2=0 AND id='$dX[idpiutang]'");
		...
		if(!empty($dX[idbyrpiutang]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_piutang SET status='2',updatex='$updatex' WHERE id%2=0 AND id='$dX[idbyrpiutang]'");
			$q3 = mysql_query("UPDATE x23_kwitansi SET status='2',cetak='1' WHERE id%2=0 AND nomor='$dX[idbyrpiutang]'");
		...
		if(!empty($dX[idpotkompensasi]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_potkompensasi SET status='2',updatex='$updatex' WHERE id%2=0 AND id='$dX[idpotkompensasi]'");
			$q3 = mysql_query("UPDATE x23_kwitansi SET status='2' WHERE id%2=0 AND idpotkom='$dX[idpotkompensasi]'");
		...
		if(!empty($dX[idpindah]))
			{                       	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_pindah SET status='2' WHERE id%2=0 AND id='$dX[idpindah]'");
		...
		if(!empty($dX[idopname]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_opname SET status='2' WHERE id%2=0 AND id='$dX[idopname]'");
		...
		if(!empty($dX[idtutupservis]))
			{              	
			$q1 = mysql_query("UPDATE x23_abis_dkonfirmasi SET status='2' WHERE id%2=0 AND id='$_REQUEST[id]'");
			$q2 = mysql_query("UPDATE x23_tutupservis SET status='2' WHERE id%2=0 AND id='$dX[idtutupservis]'");
