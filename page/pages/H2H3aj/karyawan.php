	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_karyawan WHERE id%2=0 AND id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_karyawan',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[ubah]))
				...
				$dC = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS totjum FROM x23_potkompensasi WHERE id%2=0 AND idkaryawan='$_REQUEST[ubah]' AND metodebayar='GAJI' AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND status='1'"));
				$dD	= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$_REQUEST[ubah]' AND metodebayar='GAJI' AND status='1'"));
			                	...
			                	<h4>MASTER <small>KARYAWAN</small></h4>
	                           		<div style="float:right" class="col-xs-7">
										...
										if($_SESSION[posisi]=='DIREKSI')
											{
										?>
											<button type="button"  onclick="window.open('printaj/h2/karyawan.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           			<?
	                           				}
	                           			?>
	                           		</div>
			                        <table id="example1" class="table table-bordered table-striped">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM x23_karyawan_vw WHERE id%2=0 AND id !='1' ORDER BY nama");
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan_vw WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
...
?>
						<script type="text/javascript">
							...
							function popup_print(){
								window.open('printaj/karyawanh23.php?id=<?echo $_REQUEST[id]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
								}
						</script>
			            <div class="col-xs-12">
			                <div class="box box-danger">
