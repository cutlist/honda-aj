	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$dCek = mysql_fetch_array(mysql_query("SELECT * FROM x23_komsetbruto WHERE id%2=0 AND omsetbruto='$omsetbruto'"));
				if(!empty($dCek[id])){
			...
			if(!empty($_REQUEST[del]))
				{
				$q1 = mysql_query("DELETE FROM x23_komsetbruto WHERE id%2=0 AND id='$_REQUEST[del]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_komsetbruto',
			                                    CURDATE(),
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_komsetbruto WHERE id%2=0 AND id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:520px;">
	...
	if($submenu == 'B')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				mysql_query("UPDATE x23_kjasa SET tgl2='$tanggal' WHERE id%2=0 AND status='1'");
			...
			if(!empty($_REQUEST[del]))
				{
				$q1 = mysql_query("DELETE FROM x23_kjasa WHERE id%2=0 AND id='$_REQUEST[del]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_kjasa',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[aktif]))
				{
				$q1 = mysql_query("UPDATE x23_kjasa SET status='1',tgl2='' WHERE id%2=0 AND id='$_REQUEST[aktif]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_kjasa',
			                                    CURDATE(),
...
<!-- ################## MODAL INPUT ########################################################################################## -->
                        <?
						$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_kjasa WHERE id%2=0 AND status='1'"));
                        ?>
