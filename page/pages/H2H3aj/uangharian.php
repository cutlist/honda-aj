<?
	if($submenu == 'A')
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM x23_uangharian WHERE id%2=0 AND dari='$_REQUEST[del]'");
			}
?>
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>UANG HARIAN</small></h4>
                                    <div class="col-xs-4">
										...
										$q1 = mysql_query("SELECT COUNT(nik) AS jk,SUM(totuharian) AS totuharian,dari,sampai FROM x23_uangharian WHERE id%2=0 AND idkaryawan IN (SELECT id FROM x23_karyawan WHERE id%2=0 AND posisi!='6') AND substr(dari,6,2)='$periode_bulan' AND substr(dari,1,4)='$periode_tahun' GROUP BY dari ORDER BY id DESC");
										?>
                                    		<tr>
...
<?
		...
		while (strtotime($awal) <= strtotime($akhir)) 
			{
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_uangharian WHERE id%2=0 AND dari <= '$awal' AND sampai >= '$awal'"));
			if(!empty($dCek[id])){
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
					            ...
					            <form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=save"?>" enctype="multipart/form-data">
			                        ...
			                        <table id="example1" class="table table-bordered table-striped table-hover" style="width:100%;padding-right:20px">
			                            ...
			                            <tbody>
			                            <?
										$q2	 = mysql_query("SELECT * FROM abs_x23_employee WHERE id%2=0 AND EmployeeID IN (SELECT nik FROM x23_karyawan WHERE id%2=0 AND posisi!='6' AND status='AKTIF')");
										while($d2  = mysql_fetch_array($q2))
											...
											while (strtotime($awal) <= strtotime($akhir)) 
												...
												$gaji 		= mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND nik='$d2[EmployeeID]'"));
...
<?
		...
		while($d2  = mysql_fetch_array($q2))
			...
			while (strtotime($awal) <= strtotime($akhir)) 
				...
				$hadir 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM abs_x23_result_vw WHERE id%2=0 AND substr(Date,1,10) BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND EmployeeID='$d2[EmployeeID]' GROUP BY EmployeeID"));
				$gaji 		= mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND nik='$d2[EmployeeID]'"));
   				$tharian	= $gaji[uharian]*$hadir[total];
		...
		if(!empty($_REQUEST[bayar]))
			...
			$q1 = mysql_query("UPDATE x23_uangharian SET status='1',tglbayar='$tglbayar',updatex='$updatex' WHERE id%2=0 AND id='$_REQUEST[update]'");
			...
			$q2 = mysql_query("INSERT INTO log_act VALUES (										
								...
								");
			mysql_query("UPDATE x23_potkompensasi SET status='1' WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' AND status='0' AND metodebayar='UANG HARIAN'");
			mysql_query("UPDATE x23_piutang SET status='1' WHERE id%2=0 AND idkaryawan='$_REQUEST[idkaryawan]' AND status='0' AND metodebayar='UANG HARIAN'");
			}
		if(!empty($_REQUEST[batal]))
			{
			$q1 = mysql_query("UPDATE x23_uangharian SET status='0',tglbayar='',updatex='$updatex' WHERE id%2=0 AND id='$_REQUEST[batal]'");
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>INPUT UANG HARIAN</small></h4>	
			                        <table id="example1" class="table table-striped table-hover" style="width:100%;padding-right:20px">
			                            ...
			                            <tbody>
			                            <?
										$q2	 = mysql_query("SELECT * FROM x23_uangharian WHERE id%2=0 AND dari='$_REQUEST[dari]'");
										while($d2 = mysql_fetch_array($q2))
											{
											$dD	= mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan_vw WHERE id%2=0 AND nik='$d2[nik]'"));
											if($d2[totuharian]!='0'){
				                            	if($d2[status]=='0'){
						        ...
						        <?  
								$q1 = mysql_query("SELECT * FROM x23_uangharian WHERE id%2=0 AND dari='$_REQUEST[dari]'");
								while($d1 = mysql_fetch_array($q1))
	                            	{
						        ?>
							<!-- ################## MODAL ########################################################################################## -->
