<?
	if($submenu == 'saveA')
        ...
        $dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_lembur WHERE idkaryawan='$_REQUEST[idkaryawan]' AND tanggal='$tanggal'"));
        if(!empty($dCek[id]))
        	{
		...
		$dK    = mysql_fetch_array(mysql_query("SELECT nik FROM tbl_karyawan WHERE id='$_REQUEST[idkaryawan]'"));
		$dCek2 = mysql_fetch_array(mysql_query("SELECT id FROM abs_status WHERE EmployeeID='$dK[nik]' AND  awal <= '$tanggal' AND akhir >= '$tanggal'"));
		if(!empty($dCek2[id]))
			{
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM tbl_lembur WHERE id='$_REQUEST[del]'");
			}
?>
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>DAFTAR LEMBUR</small></h4>
                                    <div style="float:right" class="col-xs-6">
										...
										$q1 = mysql_query("SELECT * FROM tbl_lembur_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun'");
										?>
                                    		<tr>
									...
									<table id="example3" class="table table-striped">
			                            ...
			                            <?
	                                    	if($_SESSION[posisi]=='DIREKSI')
	                                    		{		
												$d2	= mysql_fetch_array(mysql_query("SELECT id FROM tbl_uanglembur WHERE idkaryawan='$d1[idkaryawan]' AND tanggal='$d1[tanggal]'"));
				                            	if(empty($d2[id]))
				                            		{
			                           	?>
				                    	...
				                    	<table border="0" width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="idkaryawan" class="form-control" id="select1" style="font-size:12px;padding:3px" required>
																		...
																		<?
																			$q1 = mysql_query("SELECT id,nama FROM tbl_karyawan WHERE posisi NOT IN ('1','6') ORDER BY nama");
...
<?
		...
		if($_REQUEST[mod] == "save")
			...
			if($_REQUEST[bayar]=="1")
				{
				$dEid = mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan WHERE id='$_REQUEST[idkaryawan]'"));
				$dCek = mysql_fetch_array(mysql_query("SELECT EmployeeID FROM abs_result WHERE EmployeeID='$dEid[nik]' AND substr(Scan4,1,10)='$_REQUEST[tanggal]'"));
				...
				<script type='text/javascript'>
					window.open('print/h1/bayarlembur.php?id=$iduanglembur','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
				</script>";
		...
		if(empty($_REQUEST[mod]))
			...
			if(!empty($_REQUEST[batal]))
				{
				mysql_query("DELETE FROM tbl_uanglembur WHERE id='$_REQUEST[batal]'");
				}
?>
				<aside class="right-side">
					<section class="content">
								...
								<div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
									...
									<h4>SDM <small>PEMBAYARAN UANG LEMBUR</small></h4>
                                    	<div style="float:right" class="col-xs-6">
											...
											$q1 = mysql_query("SELECT * FROM tbl_lembur_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idkaryawan IN (SELECT id FROM tbl_karyawan WHERE status='AKTIF')");
											$q2 = mysql_query("SELECT * FROM tbl_lembur_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idkaryawan IN (SELECT id FROM tbl_karyawan WHERE status='AKTIF')");
											?>
												<tr>
													<td width="70%"><select name="bulan" class="form-control" style="height:35px">
															<option value='' >- PILIH -</option>
												...
												?>
						                           		<td width="1%">
													<button type="button"  onclick="window.open('print/h1/pembayaranlembur.php?bulan=<?echo $_REQUEST[bulan]?>&tahun=<?echo $_REQUEST[tahun]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
										...
										<table id="example3" class="table table-striped">
				                            ...
				                            <?
													if(!empty($d2[id]))
														{
											?>			   
				                                    	<button type="button" class="btn btn-warning" onclick="window.open('print/h1/bayarlembur.php?id=<?echo $d2[id]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" style="padding:0 5px 0 5px;">
									...
									while($dA = mysql_fetch_array($q2))
										{
										//$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_lembur_vw WHERE id='$d2[id]'"));
									?>
								<!-- ################## MODAL ########################################################################################## -->
