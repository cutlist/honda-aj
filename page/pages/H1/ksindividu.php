                                    ...
                                    <div style="float:right;width:65%">
										...
										<?
											if($_SESSION[posisi]=='DIREKSI')
		                                    	...
		                                    	if(!empty($_REQUEST[idsales]))
													{
													$qB = mysql_query("SELECT * FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_REQUEST[idsales]'");
													$p1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_REQUEST[idsales]'"));
													$p2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_REQUEST[idsales]' AND (jnstransaksi='CASH' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER'))"));
													$p3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_REQUEST[idsales]' AND (jnstransaksi='KREDIT' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING'))"));
		                                    		}	
												else
													{
													$qB = mysql_query("SELECT * FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun'");
										...
										?>
                                    			<td width="65%"><select name="idsales" class="form-control" id="select1" style="font-size:12px;padding:3px">
																		...
																		<?
																			$q1 = mysql_query("SELECT id,nama FROM tbl_user_vw WHERE id_posisi IN ('2','7','6','9') AND status='AKTIF' ORDER BY nama");
										...
										?>
                                    			<td width="65%"><select name="idsales" class="form-control" id="select1" style="font-size:12px;padding:3px">
																		...
																		<?
																			$q1 = mysql_query("SELECT id,nama FROM tbl_user_vw WHERE id_posisi IN ('2','7','9') AND status='AKTIF' ORDER BY nama");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
																					<option value='<?echo $dA[id]?>' <?if($_REQUEST[idsales]==$dA[id]){?>selected=""<?}?>><?echo "$dA[nama]"?></option>
										...
										<?
												...
												{
												$qB = mysql_query("SELECT * FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]'");
												$p1 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]'"));
												$p2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]' AND (jnstransaksi='CASH' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER'))"));
												$p3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_ksindividu_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]' AND (jnstransaksi='KREDIT' OR (jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING'))"));
									...
									<div class="col-xs-12" style="width:100%;margin:0 auto;margin-top:10px">
				                        <table id="example3" class="table table-striped table-bordered">
											...
											while($dX = mysql_fetch_array($qB))
												{
												$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id='$dX[idpelanggan]'"));
												$d3 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_leasing WHERE id='$dX[idleasing]'"));
												$d6 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id='$dX[idsales]'"));
                            ...
                            <div class="tab-content" style="overflow-x:auto;overflow-y:auto;height:460px;">											
                                <div class="tab-pane active">
                                    ...
                                    <div style="float:right;width:50%">
										...
										if(!empty($_REQUEST[idsales]))
											{
											$qB = mysql_query("SELECT * FROM tbl_notajual_det WHERE nonota IN (SELECT nonota FROM tbl_notajual_qty WHERE tahun='$periode_tahun' AND idsales='$_REQUEST[idsales]')");
											$p1 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS total FROM tbl_notajual_qty WHERE tahun='$periode_tahun' AND idsales='$_REQUEST[idsales]'"));
											$p2 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS total FROM tbl_notajual_qty WHERE tahun='$periode_tahun' AND jnstransaksi!='KREDIT' AND idsales='$_REQUEST[idsales]'"));
											$p3 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS total FROM tbl_notajual_qty WHERE tahun='$periode_tahun' AND jnstransaksi='KREDIT' AND idsales='$_REQUEST[idsales]'"));
                                    		}
										else
											{
											$qB = mysql_query("SELECT * FROM tbl_notajual_det WHERE nonota IN (SELECT nonota FROM tbl_notajual_qty WHERE tahun='$periode_tahun')");
											$p1 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS total FROM tbl_notajual_qty WHERE tahun='$periode_tahun'"));
											$p2 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS total FROM tbl_notajual_qty WHERE tahun='$periode_tahun' AND jnstransaksi!='KREDIT'"));
											$p3 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS total FROM tbl_notajual_qty WHERE tahun='$periode_tahun' AND jnstransaksi='KREDIT'"));
											}
                                    	?>
                                    		<tr>
                                    			<td><select name="idsales" class="form-control" id="select1" style="font-size:12px;padding:3px">
																		...
																		<?
																			$q1 = mysql_query("SELECT id,nama FROM tbl_karyawan WHERE posisi IN ('2','7') ORDER BY nama");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
																					<option value='<?echo $dA[id]?>' <?if($_REQUEST[idsales]==$dA[id]){?>selected=""<?}?>><?echo "$dA[nama]"?></option>
									...
									<div class="col-xs-12" style="width:200%;margin:0 auto;margin-top:10px">
				                        <table id="example3" class="table table-striped table-bordered">
											...
											while($dX = mysql_fetch_array($qB))
												{
												$dB = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notajual WHERE nonota='$dX[nonota]'"));
												$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id='$dB[idpelanggan]'"));
												$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id='$dB[idsales]'"));
												$d3 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_leasing WHERE id='$dB[idleasing]'"));
												$d4 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_masterbarang WHERE id='$dX[idbarang]'"));
