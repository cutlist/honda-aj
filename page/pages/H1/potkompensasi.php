	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$dck = mysql_fetch_array(mysql_query("SELECT id FROM tbl_potkompensasi WHERE idkaryawan='$idkaryawan' AND tgl='$tgl' AND jumlah='$jumlah' AND metodebayar='$_REQUEST[metodebayar]' AND ket='$ket'"));
				if(!empty($dck[id]))
					{
				...
				if($_REQUEST[metodebayar]=='GAJI')	
					{
					$dCG = mysql_fetch_array(mysql_query("SELECT ugapok FROM tbl_karyawan WHERE id='$idkaryawan'"));
				...
				if($_REQUEST[metodebayar]=='TUNAI')	
			        ...
			        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM tbl_kwitansi WHERE jnskwitansi='tunai' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
				...
				{	
				$q1 = mysql_query("DELETE FROM tbl_potkompensasi WHERE id='$_REQUEST[del]'");
				$q2 = mysql_query("DELETE FROM tbl_kwitansi WHERE nomor='$_REQUEST[del]'");
				$q3 = mysql_query("DELETE FROM tbl_abis_dkonfirmasi WHERE idpotkompensasi='$_REQUEST[del]'");
				                   	...
				                   	<form method="post" action="" enctype="multipart/form-data">
                                    	<table width="100%">
                                    		<tr>
												...
												?>
						                           		<td width="1%">
													<button type="button"  onclick="window.open('print/h1/potongankompensasi.php?periode=<?echo $_REQUEST[periode]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="karyawan" class="form-control" id="select1" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			$q1 = mysql_query("SELECT * FROM tbl_karyawan WHERE id!='1' AND status='AKTIF' AND posisi!= '6' ORDER BY nama");
...
<?
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				{	
				$dPiuX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
				$dPbyX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>SDM <small>POTONGAN KOMPENSASI &nbsp; <i class="fa fa-angle-right"></i> &nbsp; Lihat Riwayat Piutang</small></h4>
			                		...
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
										...
										<?
										$dPiu = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
										$dPby = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_potkompensasi WHERE jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' GROUP BY idkaryawan"));
					                    		...
					                    		<tr>
					                    			<td align="center">
									                    ...
									                    <table id="example2" class="table table-striped" style="width:100%"> 
															...
															<tbody>
								                    	<?
								                    		$qA = mysql_query("SELECT * FROM tbl_potkompensasi WHERE idkaryawan='$_REQUEST[idkaryawan]' ORDER BY tgl DESC");
