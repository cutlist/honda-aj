			                ...
			                <div class="box box-danger" style="overflow-x:auto;overflow-y:auto;height:520px;">
			                    ...
			                    ?>
			                        <table id="example2" class="table table-bordered table-striped table-hover" style="width:600%;padding-right:20px">
										...
										$q1 = mysql_query("SELECT * FROM tbl_pesanan_vw WHERE tglpesan BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'");
							    ...
							    <?
			                        ...
			                        $dT1 = mysql_fetch_array(mysql_query("SELECT SUM(utitipan) AS dp FROM tbl_notajual WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jnstransaksi='KREDIT'"));
			                        $dT2 = mysql_fetch_array(mysql_query("SELECT SUM(utitipan) AS titipan FROM tbl_notajual WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jnstransaksi='CASH'"));
			                        $dT3 = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_kwitansi WHERE tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jnskwitansi='lunas'"));
			                        $dT4 = mysql_fetch_array(mysql_query("SELECT SUM(utitipan) AS dp FROM tbl_notajual WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING'"));
			                        $dT5 = mysql_fetch_array(mysql_query("SELECT SUM(utitipan) AS titipan FROM tbl_notajual WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jnstransaksi='CASH TEMPO' AND jnscashtempo='DEALER'"));
			                        $dT6 = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_history_bcashtempo WHERE tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'"));
				                ?>
					                <table width="35%" style="font-weight: bold">
					                	<tr>
					                		<td widtd="25%">TOTAL OMSET UANG MUKA</td>
					                		<td widtd="2%">:</td>
					                		<td widtd="5%">RP.</td>
					                		<td align="right"><?echo number_format($dT1[dp]+$dT4[dp],"0","",".")?></td>
					                	</tr>
					                	<tr>
					                		<td widtd="25%">TOTAL OMSET UANG TITIPAN</td>
					                		<td widtd="2%">:</td>
					                		<td widtd="5%">RP.</td>
					                		<td align="right"><?echo number_format($dT2[titipan]+$dT5[titipan],"0","",".")?></td>
					                	</tr>
					                	<tr>
					                		<td widtd="25%">TOTAL OMSET PELUNASAN</td>
					                		<td widtd="2%">:</td>
					                		<td widtd="5%">RP.</td>
					                		<td align="right"><?echo number_format($dT3[total]+$dT6[total],"0","",".")?></td>
					                	</tr>
					                </table>
			                    </div><!-- /.box-body -->
			                </div><!-- /.box -->
