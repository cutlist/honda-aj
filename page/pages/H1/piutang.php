		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$q2 = mysql_query("INSERT INTO tbl_abis_dkonfirmasi (
			        ...
			        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM tbl_kwitansi WHERE jnskwitansi='piutang' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
			                	...
			                	<h4>SDM <small>PIUTANG</small></h4>
			                        ...
			                        <table id="example3" class="table table-bordered table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT idkaryawan,nama,SUM(jumlah) AS total FROM tbl_piutang WHERE jenis='piutang' AND status='1' AND nama LIKE '%$_REQUEST[cari]%' GROUP BY idkaryawan ORDER BY id DESC LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT idkaryawan,nama,SUM(jumlah) AS total FROM tbl_piutang WHERE jenis='piutang' AND status='1' GROUP BY idkaryawan ORDER BY id DESC LIMIT 0,20");
											}
										
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$d2 = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_piutang WHERE jenis='pembayaran' AND status='1' AND idkaryawan='$d1[idkaryawan]' GROUP BY idkaryawan"));
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				if($_REQUEST[metodebayar]=='GAJI')	
					{
					$dCG = mysql_fetch_array(mysql_query("SELECT ugapok FROM tbl_karyawan WHERE id='$_REQUEST[idkaryawan]'"));
				...
				$dPiuX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_piutang WHERE jenis='piutang' AND idkaryawan='$_REQUEST[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
				$dPbyX = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_piutang WHERE jenis='pembayaran' AND idkaryawan='$_REQUEST[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
			                		...
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
										...
										<div style="margin-top:20px">
					                    	<table width="100%">
					                    		<tr>
					                    			<td align="center">
					                    				<button type="button"  onclick="window.open('print/h1/historypiutang.php?idkaryawan=<?echo $_REQUEST[idkaryawan]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
...
<?
					...
					if($mod=="del")
						{
						mysql_query("DELETE FROM tbl_piutang WHERE id='$_REQUEST[id]'");
						mysql_query("DELETE FROM tbl_abis_dkonfirmasi WHERE idbyrpiutang='$_REQUEST[id]'");
						mysql_query("DELETE FROM tbl_kwitansi WHERE jnskwitansi='tunai' AND nomor='$_REQUEST[id]' AND idpotkom='0'");
