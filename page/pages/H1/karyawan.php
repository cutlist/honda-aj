		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				if($_REQUEST['posisi']=="9")
					{
					$dCek = mysql_fetch_array(mysql_query("SELECT id,nama FROM tbl_karyawan WHERE posisi='9'"));
					if(!empty($dCek[id]))
						{
			...
			if(!empty($_REQUEST[ubah]))
				...
				$dC = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS totjum FROM tbl_potkompensasi WHERE idkaryawan='$_REQUEST[ubah]' AND metodebayar='GAJI' AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND status='1'"));
				$dD	= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_piutang WHERE substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$_REQUEST[ubah]' AND metodebayar='GAJI' AND status='1'"));
			                	...
			                	<h4>MASTER <small>KARYAWAN</small></h4>
	                           		<div style="float:right" class="col-xs-7">
										...
										if($_SESSION[posisi]=='DIREKSI' OR $_SESSION[posisi]=='PIC')
											{
										?>
											<button type="button"  onclick="window.open('print/h1/karyawan.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           			<?
	                           				}
	                           			?>
	                           		</div>
			                        <table id="example1" class="table table-bordered table-striped">
										...
										if($_SESSION[posisi]=="DIREKSI"){
											...
											$q1 = mysql_query("SELECT * FROM tbl_karyawan_vw WHERE id_posisi NOT IN ('1','6') ORDER BY nama");
											}
...
?>
						<script type="text/javascript">
							...
							function popup_print(){
								window.open('print/karyawanh1.php?id=<?echo $_REQUEST[id]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
								}
						</script>
			            <div class="col-xs-12">
			                <div class="box box-danger">
