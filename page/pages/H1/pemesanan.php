			                	...
			                	<h4>PENJUALAN <small>PEMESANAN</small></h4>
									...
									<table id="example2" class="table table-striped table-hover" style="width:140%">
			                            <thead style="color:#666;font-size:13px">
			                                <tr>
			                                    <th style="padding:7px">NAMA PELANGGAN <?//echo $_SESSION[id]?></th>
										...
										if(!empty($_REQUEST[cari]))
											...
											if($_SESSION[posisi]=='DIREKSI' OR $_SESSION[posisi]=='PIC')
												...
												$q1 = mysql_query("SELECT * FROM tbl_pelanggan WHERE id IN (SELECT idpelanggan FROM tbl_prospek WHERE idsales='$_SESSION[id]' AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND idpelanggan NOT IN (SELECT idpelanggan FROM tbl_notajual_det_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]' AND tglsampai!='0000-00-00' AND idpelanggan IN (SELECT idpelanggan FROM tbl_prospek WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]')))");
												$q2 = mysql_query("SELECT * FROM tbl_pelanggan WHERE id IN (SELECT idpelanggan FROM tbl_prospek WHERE idsales='$_SESSION[id]' AND bulan='$periode_bulan' AND tahun='$periode_tahun' AND idpelanggan NOT IN (SELECT idpelanggan FROM tbl_notajual_det_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]' AND tglsampai!='0000-00-00' AND idpelanggan IN (SELECT idpelanggan FROM tbl_prospek WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idsales='$_SESSION[id]')))");
												}
											}
			                            ...
			                            ?>
			                                <tr style="cursor:pointer">
			                                    ...
			                                    <td onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=B&id=$d1[id]"?>'"><?echo "$d1[alamat]. KEL. $d1[namakel], KEC. $d1[namakec], KAB. $d1[namakab]"?></td>
					            ...
					            <form name="inputpelanggan" onsubmit="return validA();" method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=saveB"?>" enctype="multipart/form-data">
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
				                    	...
				                    	?>
				                    		<tr>
				                    			<td colspan="2"></td>
				                    			<td colspan="2" style="background: #eee;padding: 0 10px"><?echo "$dBrg[kodebarang] | $dBrg[namabarang] | $dBrg[varian] | $dBrg[warna] | $dBrg[thnproduksi]"?></td>
