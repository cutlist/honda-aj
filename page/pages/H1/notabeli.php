			                	...
			                	<h4>BELI <small>NOTA BELI</small></h4>
			                        ...
			                        <table id="example3" class="table table-striped table-hover" style="width:120%">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE nodo!='' AND nonota LIKE '%$_REQUEST[cari]%' OR nodo LIKE '%$_REQUEST[cari]%' OR nopo LIKE '%$_REQUEST[cari]%' LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE nodo!='' ORDER BY id DESC LIMIT 0,20");
											}
...
?>
						<script type="text/javascript">
							...
							function popup_print(){
								window.open('print/notabeli.php?nonota=<?echo $d1[nonota]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
								}
						</script>
				                    			...
				                    			<td colspan="2"><select name="idbarang" class="form-control" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			...
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
																					<option value="<?echo $dA[id]?>" <?if($dA[id]==$d3[idbarang]){?>selected=""<?}?>><?echo "$dA[kodebarang] | $dA[namabarang] | $dA[varian] | $dA[warna] | $dA[thnproduksi]"?></option>
																		<?
																				}
																		?>
																    </select></td>
				                    			...
				                    			<td colspan="2"><select name="idbarang" class="form-control select1" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			...
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
																					<option value="<?echo $dA[id]?>" <?if($dA[id]==$d2[idbarang]){?>selected=""<?}?>><?echo "$dA[kodebarang] | $dA[namabarang] | $dA[varian] | $dA[warna] | $dA[thnproduksi]"?></option>
																		<?
																				}
																		?>
																    </select></td>
...
<?
		...
		echo "			
		<script type='text/javascript'>
			window.open('print/notabeli.php?nonota=$_SESSION[nonota]','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
		</script>";
			
		/*	
	   	$nohp = "085768933371";
	   	$sms = "$_SESSION[nonota] %0a $_SESSION[p_tahun] %0a $_SESSION[p_bulan],$_SESSION[tglnota],$_SESSION[nodo],$_SESSION[tgldo],$_SESSION[nopo],$_SESSION[tglpo],$_REQUEST[qty],$_REQUEST[grandtotal],$_REQUEST[grandtotalppn]";	
   		passthru('gammu-smsd-inject -c smsdrc TEXT '.$nohp.' -text "'.$sms.'"');
   		//atau
   		mysql_query("INSERT INTO outbox (DestinationNumber, TextDecoded, CreatorID) VALUES ('$nohp', '$sms', 'Gammu')");
   		*/
