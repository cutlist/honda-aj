<?
	if($submenu == 'A')
		...
		mysql_query("DELETE FROM temp_pindah_det WHERE user='$_SESSION[user]'");
			                	...
			                	<h4>GUDANG & PDI <small>PINDAH LOKASI</small></h4>
			                        ...
			                        <table id="example4" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_pindah_vw WHERE (tanggal LIKE '%$_REQUEST[cari]%' OR gudang1 LIKE '%$_REQUEST[cari]%' OR gudang2 LIKE '%$_REQUEST[cari]%')");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM tbl_pindah_vw ORDER BY id DESC LIMIT 0,20");
											}
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$dT  = mysql_fetch_array(mysql_query("SELECT COUNT(norangka) AS total FROM tbl_pindah_det WHERE idpindah='$d1[id]'"));
...
<?
		...
		{
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pindah WHERE id='$_REQUEST[id]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:280px;">
			                        <table id="example2" class="table table-striped">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE norangka IN (SELECT norangka FROM tbl_pindah_det WHERE idpindah='$d1[id]')");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            ?>
				                        ...
				                        <table width="90%" border="0">
				                        	...
				                        	<tr>
				                        		...
				                        		<td colspan="2"><select name="norangka" class="form-control" id="select1" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE idgudang='$_SESSION[idgudang1]' AND status='STOK' AND norangka NOT IN (SELECT norangka FROM tbl_pindah_det_vw WHERE idgudang1='$_SESSION[idgudang1]' AND status='0') AND norangka NOT IN (SELECT norangka FROM temp_pindah_det WHERE user='$_SESSION[user]')");
																			while($d1=mysql_fetch_array($q1))
																				{
																		?>
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example2" class="table table-striped">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE norangka IN (SELECT norangka FROM temp_pindah_det WHERE user='$_SESSION[user]')");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            ?>
			                                <tr style="cursor:pointer">
...
<?
		...
		$dT  = mysql_fetch_array(mysql_query("SELECT COUNT(norangka) AS total FROM temp_pindah_det WHERE user='$_SESSION[user]'"));
		if($dT[total]=='0')
			{
		...
		$no = 1;
		$qA = mysql_query("SELECT norangka FROM temp_pindah_det WHERE user='$_SESSION[user]'");
        while($dA = mysql_fetch_array($qA))
        	{
        	//mysql_query("UPDATE tbl_stokunit SET idgudang='$_SESSION[idgudang2]',updatex='$updatex' WHERE norangka='$dA[norangka]'");
        	mysql_query("INSERT INTO tbl_pindah_det VALUES ('','$idpindah','$dA[norangka]')");
        	$no++;
        	}
        	
		$dB  = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pindah_vw WHERE id='$idpindah'"));
		$jml = $no-1;
