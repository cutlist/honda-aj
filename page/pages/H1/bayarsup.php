			        ...
			        <div class="row">
			            ...
			            <div class="col-xs-4">		                
			                <div class="small-box bg-green" style="text-align:center;height:520px;border-radius:5px 5px 0 0;margin-top:0px;padding:20px 0 0 0;border-bottom: 5px solid #fff">
			                	...
			                	<?
								$pA = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_notabeli WHERE tahun='$periode_tahun' AND bulan='$periode_bulan' AND bayar NOT IN ('0','')"));
			                	?>
	                                <div class="inner">
			                		...
			                		<h4 style="border-bottom:1px dashed #ddd;padding-bottom:5px;width:96%">5 NOTA TERBARU (SUDAH BAYAR)</h4>
				                        <table class="table" style="width:96%">
				                            ...
				                            <tbody>
				                            <?
				                            $dc1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notabeli WHERE bayar NOT IN ('0','') ORDER BY tglbayar DESC"));
				                            if(empty($dc1[id]))
				                            	{
				                            ?>
											...
											$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar NOT IN ('0','') ORDER BY tglbayar DESC LIMIT 0,5");
			            ...
			            <div class="col-xs-4">			                
			                <div class="small-box bg-orange" style="text-align:center;height:520px;border-radius:5px 5px 0 0;margin-top:0px;padding:20px 0 0 0;border-bottom: 5px solid #fff">
			                	...
			                	<h5 style="margin-top:-10px">BULAN INI</h5>
			                	<?
								$pB = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_notabeli WHERE tahun='$periode_tahun' AND bulan='$periode_bulan' AND bayar IN ('0','') AND gtbayar!='' AND scan='1'"));
			                	?>
			                		...
			                		<h4 style="border-bottom:1px dashed #ddd;padding-bottom:5px;width:96%">5 NOTA TERBARU (BELUM BAYAR)</h4>
				                        <table class="table" style="width:96%">
				                            ...
				                            <tbody>
				                            <?
				                            $dc1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notabeli WHERE bayar IN ('0','') AND gtbayar!='' AND scan='1'"));
				                            if(empty($dc1[id]))
				                            	{
				                            ?>
				                                <tr style="cursor:pointer">
											...
											$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar IN ('0','') AND gtbayar!='' AND scan='1' ORDER BY tglbayar DESC LIMIT 0,5");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
				                                <tr style="cursor:pointer">
...
<?
					if(empty($mod))
						...
						if(!empty($_REQUEST[bayarnota]))
							...
							$q1 = mysql_query("UPDATE tbl_notabeli SET status='$status',bayar='$bayar',scan='1',tglbayar='$tglbayar',updatex='$updatex' WHERE id='$_REQUEST[bayarnota]'");
							$q2 = mysql_query("DELETE FROM tbl_notabeli_det WHERE nonota='$_REQUEST[nonota]' AND status='0'");
...
?>
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PEMBELIAN <small>NOTA BELI BELUM BAYAR</small></h4>
			                        <table id="example1" class="table table-striped table-hover" style="width:120%">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar IN ('0','') AND gtbayar!='' AND nodo!='' AND scan='1'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$d1A = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_notabeli_det WHERE nonota='$d1[nonota]' AND status='1'"));
			                            ?>
			                                <tr style="cursor:pointer">
		            ...
		            <?
						$q3 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar IN ('0','')");
...
<?
						...
						$d2 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS qty, SUM(hargabelibersih) AS total FROM tbl_notabeli_det2_vw WHERE nonota='$d1[nonota]' AND status='1'"));
						if(!empty($_REQUEST[bayarnota]))
							{
							$tglbayar   = date("Y-m-d", strtotime($_REQUEST['tglbayar']));
							...
							$q1 = mysql_query("UPDATE tbl_notabeli SET status='$status',scan='1',bayar='$bayar',tglbayar='$tglbayar',updatex='$updatex' WHERE id='$_REQUEST[bayarnota]'");
							$q2 = mysql_query("DELETE FROM tbl_notabeli_det WHERE nonota='$_REQUEST[nonota]' AND status='0'");
								  mysql_query("INSERT INTO tbl_bayarsup_history VALUES (
								  								'',
								  								'$_REQUEST[nonota]',
...
?>
		            ...
		            <?
						$q3 = mysql_query("SELECT * FROM tbl_notabeli WHERE id='$_REQUEST[id]'");
			            while($d3 = mysql_fetch_array($q3))
			            	{
		            ?>
	<!-- ################## MODAL UBAH BARANG ########################################################################################## -->
...
<?
		...
		if(!empty($_REQUEST[bayarnota]))
			...
			$q1 = mysql_query("UPDATE tbl_notabeli SET status='$status',bayar='$bayar',tglbayar='$tglbayar',updatex='$updatex' WHERE id='$_REQUEST[bayarnota]'");
			$q2 = mysql_query("DELETE FROM tbl_notabeli_det WHERE nonota='$_REQUEST[nonota]' AND status='0'");
		...
		if(!empty($_REQUEST[ubahbayarnota]))
			...
			$q1 = mysql_query("UPDATE tbl_notabeli SET status='$status',bayar='$bayar2',tglbayar='$tglbayar',updatex='$updatex' WHERE id='$_REQUEST[ubahbayarnota]'");
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PEMBELIAN <small>NOTA BELI SUDAH BAYAR</small></h4>
			                        ...
			                        <table id="example3" class="table table-striped table-hover" style="width:120%">
			                            ...
			                            <tbody>
			                            <?
							//SELECT COUNT(id) AS total FROM tbl_notabeli WHERE tahun='$periode_tahun' AND bulan='$periode_bulan' AND bayar NOT IN ('0','')

										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar NOT IN ('0','') AND nonota LIKE '%$_REQUEST[cari]%' OR nodo LIKE '%$_REQUEST[cari]%' AND nodo!='' LIMIT 0,20");
											$q3 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar NOT IN ('0','') AND nonota LIKE '%$_REQUEST[cari]%' OR nodo LIKE '%$_REQUEST[cari]%' AND nodo!='' LIMIT 0,20");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar NOT IN ('0','') ORDER BY id DESC LIMIT 0,20");
											$q3 = mysql_query("SELECT * FROM tbl_notabeli WHERE bayar NOT IN ('0','') ORDER BY id DESC LIMIT 0,20");
											}
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											...
											$d2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_stokunit WHERE nonota='$d1[nonota]'"));
											$d1A = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_notabeli_det WHERE nonota='$d1[nonota]' AND status='1'"));
			                            ?>
			                                <tr style="cursor:pointer">
			                                    <td onclick="location.href='<?echo "?opt=$opt&menu=$menu&submenu=$submenu&mod=view&id=$d1[id]"?>'"><?echo $d1[nonota]?></td>
...
<?
						...
						$q3 = mysql_query("SELECT * FROM tbl_notabeli WHERE status='1'");
			            while($d3 = mysql_fetch_array($q3))
			            	{
?>
	<!-- ################## MODAL BAYAR ########################################################################################## -->
...
<?
						...
						if(!empty($_REQUEST[ubah]))
							{
							$bayar  	= preg_replace( "/[^0-9]/", "",$_REQUEST['bayar']);
							$q1 = mysql_query("UPDATE tbl_bayarsup_history SET jumlah='$bayar' WHERE id='$_REQUEST[ubah]'");
							...
							$q2 = mysql_query("UPDATE tbl_notabeli SET status='$status',bayar='$bayarupdate',tglbayar=CURDATE(),updatex='$updatex' WHERE id='$_REQUEST[id]'");
							$q3 = mysql_query("INSERT INTO log_act VALUES (										
						                                    '',
						                                    'tbl_notabeli',
						                                    CURDATE(),
						...
						$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_bayarsup_history WHERE id='$_REQUEST[idbayar]'"));
?>
			            <div class="col-xs-12" style="margin-bottom:10px">
			                <div class="box box-danger">
...
<?
						...
						if(!empty($_REQUEST[bayarnota]))
							...
							$tglbayar   = date("Y-m-d", strtotime($_REQUEST['tglbayar']));
								...
								$dcek = mysql_fetch_array(mysql_query("SELECT tanggal FROM tbl_bayarsup_history WHERE nonota='$_REQUEST[nonota]' ORDER BY tanggal DESC LIMIT 0,1"));
					                ...
					                <div class="modal-content">
				                        ...
				                        <div class="modal-body">
					                        <table id="example2" class="table table-striped" style="width:100%">
					                            ...
					                            <tbody>
					                            <?
												$d2 = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM tbl_bayarsup_history WHERE nonota='$d1[nonota]'"));
												$q1 = mysql_query("SELECT * FROM tbl_bayarsup_history WHERE nonota='$d1[nonota]'");
					                            while($dA = mysql_fetch_array($q1))
					                            	{
					                            ?>
					                                <tr style="cursor:pointer">
...
?>
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PENCARIAN NOTA BELI</h4>
                                    	...
                                    	<table width="100%">
                                    		<tr>
												...
												if($_SESSION[posisi]=='DIREKSI' OR $_SESSION[posisi]=='PIC')
													{
												?>
													<button type="button"  onclick="window.open('print/h1/bayarsup.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
				                       			<?
				                       				}
				                       			?>
                                    			</td>
			                        ...
			                        <table id="example4" class="table table-bordered table-striped table-hover" style="width:160%">
			                            ...
			                            <tbody>
			                            <?			                            
										$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND nodo!=''");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	if($d1[status]=="1"){$status = "<span class='btn btn-primary' style='padding:0px 10px;font-size:12px;width:90px'> Bayar</span>";}
			                            	if($d1[status]=="0"){
		            ...
		            <?
						$q3 = mysql_query("SELECT * FROM tbl_notabeli WHERE status='0'");
			            while($d3 = mysql_fetch_array($q3))
			            	{
		            ?>
	<!-- ################## MODAL BAYAR ########################################################################################## -->
