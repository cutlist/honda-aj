<?
	if($submenu == 'save')
		...
		if(empty($_REQUEST[status])){
			$q1 = mysql_query("UPDATE tbl_bpkb SET pnopol='$pnopol',status='0' WHERE notajual='$_REQUEST[nonota]'");
			}
		else if($_REQUEST[status]=="1"){
			$q1 = mysql_query("UPDATE tbl_bpkb SET pnopol='$pnopol',status='1' WHERE notajual='$_REQUEST[nonota]'");
			}
			...
			*/
			...
			$q1 = mysql_query("UPDATE tbl_bpkb SET pnopol='$pnopol',harganopol='$harganopol',status='2',sisabayar='$sisabayar' WHERE notajual='$_REQUEST[nonota]'");
			}
...
?>
		<script type="text/javascript">
			...
			function popup_print(){
				window.open('print/pesannopol.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
				}
		</script>
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PENJUALAN <small>PEMESANAN NOPOL</small></h4>
			                        ...
			                        <table id="example1" class="table table-striped table-hover" style="width:160%">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM tbl_bpkb WHERE pnopol!='' AND notajual!=''");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notajual_vw WHERE nonota='$d1[notajual]'"));
			                            	$dB = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notajual_det_vw WHERE nonota='$d1[notajual]'"));
...
<?
		...
		$dC1 = mysql_fetch_array(mysql_query("SELECT id,idpelanggan FROM tbl_notajual WHERE nonota='$nonota'"));
		if(empty($dC1[id]))
			{
			...
			$dC2 = mysql_fetch_array(mysql_query("SELECT id,nopesan FROM tbl_bpkb WHERE notajual='$nonota' AND pnopol='' AND status='0'"));
			if(empty($dC2[id]))
				{
...
<?
		...
		$q1 = mysql_query("UPDATE tbl_bpkb SET pnopol='$pnopol',utitipannopol='$utitipannopol' WHERE notajual='$_REQUEST[nonota]' AND nopesan='$_REQUEST[nopesan]'");
