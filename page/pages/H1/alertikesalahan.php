	                                ...
	                                <div class="inner col-xs-12">
	                                	<div style="text-align:center;width:100%;height:380px;border-radius:10px;background:#fff;padding:10px;border:1px solid #ddd;cursor:pointer" onclick="location.href='<?echo "?opt=".md5(abis)."&submenu=A&menu=".md5(abis_ikesalahan)?>'">    
		                                	<div class="btn-danger" style="width:100%;height:358px;border-radius:5px;padding:5px;">
	                                			<?
												$dA  = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS tot FROM tbl_cekfisik_vw WHERE bulan='$periode_bulan'  AND tahun='$periode_tahun' AND bensinawal > literawal AND lihat='0'"));
												...
												$dE  = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS tot FROM tbl_pengeluaranunit_vw WHERE bulan='$periode_bulan'  AND tahun='$periode_tahun' AND tglsampai > tanggal AND ikesalahan='0' AND tglsampai2!='0000-00-00'"));
		                                	...
		                                	<div class="" style="width:100%;overflow-x:hidden;overflow-y:auto;height:358px;border-radius:5px;padding:5px;">
							                        ...
							                        <table id="example2" class="table table-hover" style="width:100%;padding-right:20px">
										               ...
										               ?>
														...
														<?
																$q1	 = mysql_query("SELECT *,COUNT(norangka) AS total FROM tbl_notabeli_det_vw WHERE bulan='$periode_bulan'  AND tahun='$periode_tahun' AND (status='0' OR ikesalahan='1') GROUP BY nonota ORDER BY tglnota");
																while($d1  = mysql_fetch_array($q1))
																	{
								                        ...
								                        <?
																$q1	 = mysql_query("SELECT * FROM tbl_pengeluaranunit_vw WHERE bulan='$periode_bulan'  AND tahun='$periode_tahun' AND tglsampai > tanggal AND ikesalahan='1' AND tglsampai2!='0000-00-00' ORDER BY tanggal");
																while($d1  = mysql_fetch_array($q1))
																	{
									                     ?>
