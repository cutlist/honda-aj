									...
									?>
	                           		<div style="float:right" class="col-xs-7">
										<a href="<?echo "?opt=$opt&menu=$menu&submenu=B"?>">
	                           				...
	                           				<button type="button"  onclick="window.open('print/h1/unitindent.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           		</div>
...
<?
	...
	if($submenu == 'C')
		...
		if(empty($_SESSION[nopesan]))
			...
			$dC1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pesanan WHERE nopesan='$_SESSION[nopesan]' AND status='0' AND indent='0'"));
			...
			if(!empty($dC1[utitipan]) || $dC1[utitipan]!='0')
				{
				$dC2 = mysql_fetch_array(mysql_query("SELECT id FROM tbl_kwitansi WHERE nomor='$_SESSION[nopesan]' AND jnskwitansi!='nopol'"));
				if(!empty($dC2[id])){
					$cek = "1";
					$red = "";
					}
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PENJUALAN <small>UNIT INDENT</small></h4>
				                	...
				                	<div style="padding:20px;min-height:200px;">
		                            	...
		                            	<?
		                            	$dQ = mysql_fetch_array(mysql_query("SELECT COUNT(nopesan) AS total FROM tbl_pesanan_det WHERE nopesan='$_SESSION[nopesan]' GROUP BY nopesan"));
		                            	$qTemp  = mysql_query("SELECT * FROM tbl_pesanan_det WHERE nopesan='$_SESSION[nopesan]'");
		                            	$qTemp2 = mysql_query("SELECT * FROM tbl_pesanan_det WHERE nopesan='$_SESSION[nopesan]'");
		                            	?>
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="norangka" class="form-control" id="select<?echo $dTemp2[id]?>" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			$q1 = mysql_query("SELECT * FROM tbl_stokunit WHERE idbarang='$dTemp2[idbarang]' AND status='STOK' AND norangka NOT IN (SELECT norangka FROM tbl_pesanan_det WHERE nopesan='$_SESSION[nopesan]')");
																			while($d1=mysql_fetch_array($q1))
																				{
																		?>
...
<?
	...
	if($submenu == 'E')
		{
			$qA = mysql_query("SELECT norangka FROM tbl_pesanan_det WHERE nopesan='$_SESSION[nopesan]'");
			while($dA=mysql_fetch_array($qA))
				{
				mysql_query("UPDATE tbl_stokunit SET 
											status='$_SESSION[nopesan]'
											WHERE norangka='$dA[norangka]'");
				}
				
			mysql_query("UPDATE tbl_pesanan SET 
										indent='1'
										WHERE nopesan='$_SESSION[nopesan]'");
