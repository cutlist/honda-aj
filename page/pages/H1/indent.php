...
<?
	...
	if($submenu == 'A')
		...
		if(!empty($_REQUEST[batal]))
			...
			$q1 = mysql_query("UPDATE tbl_pesanan SET status='MENUNGGU KONFIRMASI',batal='$_REQUEST[oleh]',updatex='$updatex' WHERE nopesan='$_REQUEST[batal]'");
...
?>
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>PENJUALAN <small>DAFTAR INDENT</small></h4>
									...
									<table id="example1" class="table table-striped">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM tbl_pesanan_vw WHERE status='0'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$d2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS stok FROM tbl_stokunit WHERE idbarang='$d1[idbarang]' AND status='STOK'"));
											$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_pesanan_vw WHERE status='0' AND nopesan='$d1[nopesan]' LIMIT 1"));
			                            	if($d1[indent]=='1')
			                            		{
			                            	...
			                            	if($d1[indent]=='0')
												...
												if(!empty($d1[utitipan]) || $d1[utitipan]!='0')
													{
													$dCek2 = mysql_fetch_array(mysql_query("SELECT id FROM tbl_kwitansi WHERE nomor='$d1[nopesan]' AND jnskwitansi!='nopol'"));
													if(!empty($dCek2[id])){
														$cek = "1";
														$red = "";
														}
	                    ...
	                    <?
							$q2 = mysql_query("SELECT * FROM tbl_pesanan_vw WHERE status='0'");
		                    while($d2 = mysql_fetch_array($q2))
		                    	{
								$d3 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pesanan WHERE nopesan='$d2[nopesan]'"));
		                ?>
						                ...
						                <div class="modal-content">
					                        ...
					                        <div class="modal-body" style="overflow-x:hidden;overflow-y:auto;height:450px;">
												<table class="table table-striped" >
						                            ...
						                            <?
						                            $no = 1;
						                            $q1 = mysql_query("SELECT * FROM tbl_hleasing_vw WHERE id_pelanggan='$d2[idpelanggan]' ORDER BY tanggal DESC");
						                            while($d1 = mysql_fetch_array($q1))
						                            	{
						                            	if($d1[status]=='1'){
...
<?
		...
		if(!empty($_REQUEST[del]))
			{
			$q1 = mysql_query("DELETE FROM tbl_pesanan_vw WHERE nopesan='$_REQUEST[nopesan]'");
			$q2 = mysql_query("DELETE FROM tbl_pesanan WHERE nopesan='$_REQUEST[nopesan]'");
			$q3 = mysql_query("INSERT INTO log_act VALUES (										
		                                    '',
		                                    'tbl_pesanan',
		                                    CURDATE(),
			                	...
			                	<h4>PENJUALAN <small>INDENT YANG DIBATALKAN</small></h4>
									...
									<table id="example2" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_pesanan_vw WHERE status IN ('BATAL','MENUNGGU KONFIRMASI') AND nama LIKE '%$_REQUEST[cari]%' OR nopesan LIKE '%$_REQUEST[cari]%' OR tglpesan LIKE '%$_REQUEST[cari]%'");
											}
										else
											{
											$q1 = mysql_query("SELECT * FROM tbl_pesanan_vw WHERE status IN ('BATAL','MENUNGGU KONFIRMASI') ORDER BY id DESC LIMIT 0,20");
											}
										while($d1 = mysql_fetch_array($q1))
			                            	{
...
<?
		...
		if(!empty($_REQUEST[deltemp]))
			{
			mysql_query("DELETE FROM tbl_pesanan_det WHERE id='$_REQUEST[deltemp]'");
		...
		if(!empty($_REQUEST[transaksi]))
			...
			mysql_query("UPDATE tbl_pesanan SET jnstransaksi='$_REQUEST[jnstransaksi]',tglpelunasan='$tglpelunasan',jnscashtempo='$_REQUEST[jnscashtempo]',idleasing='$_REQUEST[idleasing]',idleasing='$_REQUEST[idleasing]',utitipan='$utitipan',termin='$_REQUEST[termin]' WHERE nopesan='$_REQUEST[transaksi]'");
			}
			
		$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pesanan_vw WHERE id='$_REQUEST[id]'"));
...
?>
			...
			<script type="text/javascript">
				...
				function popup_print(){
					window.open('print/notapemesanan.php?nopesan=<?echo $dA[nopesan]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
					}
			</script>
			<aside class="right-side">
			    <section class="content">
...
<?
		...
		{
		$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pesanan_vw WHERE id='$_REQUEST[id]'"));
		$dCek1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_hutitipan WHERE nopesan='$dA[nopesan]' AND status='0'"));
		if(!empty($dCek1[id]))
			{
			echo "<script>alert ('Mohon Ulangi, Karena Ada Kwitansi Uang Muka/Titipan Yang Belum Dicetak.')</script>";
		...
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id='$dA[idpelanggan]'"));
		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_bpkb WHERE nopesan='$dA[nopesan]'"));
		$d3 = mysql_fetch_array(mysql_query("SELECT namaleasing FROM tbl_leasing WHERE id='$dA[idleasing]'"));
        ...
        $dNJ = mysql_fetch_array(mysql_query("SELECT nonota FROM tbl_notajual WHERE tglnota=CURDATE() ORDER BY SUBSTR(nonota,-3,3) DESC LIMIT 1"));
		...
		if(!empty($_REQUEST[temp]))
			...
			*/
				...
				$dC1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit WHERE norangka='$_REQUEST[norangka]'"));
				$ppnbeli 	= $dC1[ppn];
			                ...
			                <div class="box box-danger">
					            ...
					            <form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=D"?>" enctype="multipart/form-data">
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
		                            	...
		                            	<?
		                            	$dQ  = mysql_fetch_array(mysql_query("SELECT COUNT(nopesan) AS total FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]' GROUP BY nopesan"));
		                            	$dHB = mysql_fetch_array(mysql_query("SELECT SUM(hargabelibersih) AS total FROM tbl_stokunit WHERE norangka IN (SELECT norangka FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]')"));
		                            	$qTemp  = mysql_query("SELECT * FROM tbl_pesanan_det_vw WHERE nopesan='$dA[nopesan]'");
		                            	?>
				                    	<table width="70%">
				                    		<tr>
		                            	...
		                            	$dQ = mysql_fetch_array(mysql_query("SELECT COUNT(nopesan) AS total FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]' GROUP BY nopesan"));
		                            	$qTemp  = mysql_query("SELECT * FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]'");
		                            	$qTemp2 = mysql_query("SELECT * FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]'");
		                            	?>
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="norangka" class="form-control" id="select<?echo $dTemp2[id]?>" style="font-size:12px;padding:3px" required="">
																		<option value='' selected>Pilih</option>
																		<?
																			$q1 = mysql_query("SELECT * FROM tbl_stokunit WHERE idbarang='$dTemp2[idbarang]' AND status='STOK' AND norangka NOT IN (SELECT norangka FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]')");
																			while($d1=mysql_fetch_array($q1))
																				{
																		?>
				                    		...
				                    		<tr>
				                    			...
				                    			<td><div class="input-group">
				                                        <span class="input-group-addon">RP.</span>
				                                        	<input type="text" name="disc"  value="0" style="width:30%;text-align:right" class="form-control disc" onkeypress="return buat_angka(event,'1234567890')" required> 
				                                    </div>                                        		
				                                </td>
				                    		</tr>
			                        ...
			                        <div class="modal-body">
				                    	<table width="100%">
				                    		...
				                    		<tr>
				                    			...
				                    			<td><div class="input-group">
				                                        <span class="input-group-addon">RP.</span>
				                                        	<input type="text" name="disc"  placeholder="0" style="width:50%;text-align:right" class="form-control disc" onkeypress="return buat_angka(event,'1234567890')" required> 
				                                    </div>                                        		
				                                </td>
				                    		</tr>
...
<?
		...
		{
		mysql_query("UPDATE tbl_pesanan_det SET norangka='',otr='',disc='',disc='',hargabeli='',ppnbeli='' WHERE nopesan='$_REQUEST[nopesan]'");
		print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=A'/>";
		}
		...
		{
		$dA = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pesanan_vw WHERE id='$_REQUEST[id]'"));
		$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]' AND norangka=''"));
		if(!empty($dCek[id]))
			{
			                ...
			                <div class="box box-danger">
					            ...
					            <form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=save"?>" enctype="multipart/form-data">
			                		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
		                            	...
		                            	<?
		                            	$dQ  = mysql_fetch_array(mysql_query("SELECT COUNT(nopesan) AS total FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]' GROUP BY nopesan"));
		                            	$dHB = mysql_fetch_array(mysql_query("SELECT SUM(hargabelibersih) AS total,SUM(ppn) AS ppnbeli FROM tbl_stokunit WHERE norangka IN (SELECT norangka FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]')"));
		                            	$qTemp  = mysql_query("SELECT * FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]'");
		                            	?>
				                    	<table width="70%">
				                    		<tr>
				                    	...
				                    	while($dTemp = mysql_fetch_array($qTemp))
				                    		{
											$d4   = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit WHERE norangka='$dTemp[norangka]'"));
				                    	?>
				                    		<tr>
				                    			<td colspan="2"></td>
				                    	...
				                    	?>
				                    		...
				                    		<?
				                    		$dTot = mysql_fetch_array(mysql_query("SELECT SUM(otr) AS totr,SUM(disc) AS tdisc FROM tbl_pesanan_det WHERE nopesan='$dA[nopesan]'"));
				                    	...
				                    	<table width="70%">
					                    	...
					                    	-->   
					                    		<input type="hidden" name="bayar" id="bayar" class="form-control" value="<?echo number_format($tbayar,"0","",".")?>" style="width:34%;text-align:right" maxlength="20" onfocus="this.select();" onkeypress="return buat_angka(event,'0123456789')" readonly="">
...
<?
		...
		$q2 = mysql_query("UPDATE tbl_pesanan SET status='TERJUAL' WHERE nopesan='$_REQUEST[nopesan]'");
		$q3 = mysql_query("UPDATE tbl_bpkb SET notajual='$_REQUEST[nonota]' WHERE nopesan='$_REQUEST[nopesan]'");
		
		$qT = mysql_query("SELECT * FROM tbl_pesanan_det WHERE nopesan='$_REQUEST[nopesan]'");
		while($dT=mysql_fetch_array($qT))
			...
			mysql_query("INSERT INTO tbl_notajual_det (
												...
												VALUES (
													...
													'$dT[jumlah1]',
													'$dT[otrsetelahpajak]')");
			mysql_query("UPDATE tbl_stokunit SET status='TERJUAL',updatex='$updatex' WHERE norangka='$dT[norangka]'");
			}
			
        $dZ = mysql_fetch_array(mysql_query("SELECT SUM(jumlah1) AS laba,SUM(jual_plus_ppnbeli) AS tjual_plus_ppnbeli,SUM(ppnjual_min_ppnbeli) AS tppnjual_min_ppnbeli FROM tbl_notajual_det WHERE nonota='$_REQUEST[nonota]'"));
        $laba = $dZ[laba];
        $tjual_plus_ppnbeli 	= $dZ[tjual_plus_ppnbeli];
        $tppnjual_min_ppnbeli 	= $dZ[tppnjual_min_ppnbeli];
        	          
        $dNPDI = mysql_fetch_array(mysql_query("SELECT nopdi FROM tbl_notajual WHERE tahun='$p_tahun' AND bulan='$p_bulan' ORDER BY SUBSTR(nopdi,-3,3) DESC LIMIT 1"));
		...
		echo "			
		<script type='text/javascript'>
			window.open('print/notajual.php?nonota=$_REQUEST[nonota]','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
		</script>";
		...
		if($dA[utitipan]!='0')
			{
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_kwitansi WHERE nomor='$dA[nopesan]' AND jnskwitansi IN ('umuka','titip','penambahan') AND status='0'"));
			if(!empty($dCek[id])){
			...
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM tbl_kwitansi WHERE nomor='$dA[nopesan]' AND jnskwitansi IN ('umuka','titip')"));
			if(empty($dCek[id])){
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PENJUALAN <small>PEMESANAN</small></h4>
					           		<div style="padding:20px 0px 20px 20px;overflow-x:hidden;overflow-y:auto;height:380px;">
		                            	...
		                            	<div style="border-bottom:1px #aaa dashed;margin: 10px 0 10px"></div>
	                            	<?
	                            	$dchut = mysql_fetch_array(mysql_query("SELECT id FROM tbl_hutitipan WHERE nopesan='$dA[nopesan]'"));
	                            	if(!empty($dchut[id]))
	                            		{
									?>
				                    	<table width="70%">
					                    	...
					                    	<table width="70%">
					                    		<tr>
					                    			...
					                    			<td colspan="2">
						                    			...
						                    			<?
						                    			$qhut = mysql_query("SELECT * FROM tbl_hutitipan WHERE nopesan='$dA[nopesan]'");
		                    			...
		                    			<?
		                    			/*
		                    			$qhut2 = mysql_query("SELECT * FROM tbl_hutitipan WHERE nopesan='$dA[nopesan]'");
...
<?
		...
		$dCek = mysql_fetch_array(mysql_query("SELECT hargabelibersih FROM tbl_stokunit WHERE idbarang='$_REQUEST[idbarang]'"));
		if($dCek[hargabelibersih] < $tutitipan)
	        ...
	        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM tbl_kwitansi WHERE jnskwitansi='penambahan' AND tanggal=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
		...
		$q1 = mysql_query("INSERT INTO tbl_hutitipan (
										...
										VALUES (
											...
											'$updatex') 
							");
		$q2 = mysql_query("UPDATE tbl_pesanan SET utitipan='$tutitipan',updatex='$updatex' WHERE nopesan='$_REQUEST[nopesan]'");
		$q3 = mysql_query("INSERT INTO log_act VALUES (										
	                                    '',
	                                    'tbl_hutitipan',
	                                    CURDATE(),
	...
	/*
		...
		$q1 = mysql_query("UPDATE INTO tbl_hutitipan SET 
										...
										VALUES (
											...
											'$updatex') 
							");
		$q2 = mysql_query("UPDATE tbl_pesanan SET utitipan='$tutitipan' WHERE id='$_REQUEST[id]'");
		$q3 = mysql_query("INSERT INTO log_act VALUES (										
	                                    '',
	                                    'tbl_hutitipan',
	                                    CURDATE(),
	...
	*/
		...
		$q1 = mysql_query("DELETE FROM tbl_hutitipan WHERE id='$_REQUEST[idhut]'");
		$q2 = mysql_query("UPDATE tbl_pesanan SET utitipan='$tutitipan' WHERE nopesan='$_REQUEST[nopesan]'");
		$q3 = mysql_query("INSERT INTO log_act VALUES (										
	                                    '',
	                                    'tbl_hutitipan',
	                                    CURDATE(),
