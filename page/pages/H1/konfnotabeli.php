			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>KONFIRMASI NOTA BELI <small>UNIT MASUK</small></h4>
			                        ...
			                        <table id="example3" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM tbl_notabeli WHERE scan='0'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$dA = mysql_fetch_array(mysql_query("SELECT id FROM tbl_notabeli WHERE bayar='' AND id='$d1[id]'"));
											if(!empty($dA[id]))
												{
			                            ?>
...
<?
		...
		if(!empty($_REQUEST[scan]))
			{
			$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notabeli_det WHERE norangka='$_REQUEST[scan]'"));
			$ppn = 	ROUND(($d2[hargabelibersih]*10)/100);	
			
			if($d2[status]=='0')
				{
				mysql_query("UPDATE tbl_notabeli_det SET status='1' WHERE norangka='$_REQUEST[scan]' AND nonota='$d1[nonota]'");
				mysql_query("UPDATE tbl_notabeli SET gtbayar=gtbayar+$d2[hargabelibersih],gtbayarppn=gtbayarppn+$ppn WHERE nonota='$d1[nonota]'");
				}
			}
...
<?
		...
		foreach($_REQUEST['scan'] AS $scan)
			{
			$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notabeli_det WHERE norangka='$scan'"));
			
			mysql_query("UPDATE tbl_notabeli_det SET status='1' WHERE norangka='$scan'");
			mysql_query("UPDATE tbl_notabeli SET gtbayar=gtbayar+$d2[hargabelibersih] WHERE nonota='$d1[nonota]'");
			}
		*/
		if(!empty($_REQUEST['scan']))
			{
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>KONFIRMASI NOTA BELI <small>UNIT MASUK <i class="fa fa-angle-right"></i> <?echo $d1[nodo]?></small></h4>
			                        ...
			                        <table class="table table-striped" id="example2">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM tbl_notabeli_det2_vw WHERE nonota='$d1[nonota]' AND status='1'");
			                            while($dA = mysql_fetch_array($q1))
			                            	{
				                            if($dA[status]=='1'){
												$status = "<span class='label label-success'>ADA</span>";
...
<?
		...
		{
		$d1 = mysql_fetch_array(mysql_query("SELECT nodo,nonota,tglnota FROM tbl_notabeli WHERE id='$_REQUEST[id]'"));
		
		$qA = mysql_query("SELECT id FROM tbl_notabeli_det WHERE nonota='$d1[nonota]' AND status='1'");
        while($dA = mysql_fetch_array($qA))
        	...
        	mysql_query("UPDATE tbl_notabeli_det SET tgltiba='$tgltiba',idgudang='$idgudang' WHERE id='$id'");
        	}
		...
		*/
        if($dD[cek] > "0")
        	{
        	mysql_query("UPDATE tbl_notabeli SET ikesalahanacc='1' WHERE id='$_REQUEST[id]'");
        	}
			                	...
			                	<h4>KONFIRMASI NOTA BELI <small>UNIT MASUK <i class="fa fa-angle-right"></i> <?echo $d1[nodo]?></small></h4>
			                        ...
			                        <table class="table table-striped" id="example2">
			                            ...
			                            while($dA = mysql_fetch_array($q1))
											...
											$dB = mysql_fetch_array(mysql_query("SELECT gudang FROM tbl_gudang WHERE id='$dA[idgudang]'"));
			                            ?>
			                                <tr style="cursor:pointer">
			                                    <td><?echo $dA[kodebarang]?></td>
			                                    <td><?echo $dA[namabarang]?></td>
...
<?
		...
		{
			  mysql_query("DELETE FROM tbl_stokunit WHERE nonota='$_REQUEST[nonota]'");
		$qA = mysql_query("SELECT * FROM tbl_notabeli_det2_vw WHERE nonota='$_REQUEST[nonota]' AND status='1'");
        while($dA = mysql_fetch_array($qA))
        	{
			...
			$q1 = mysql_query("UPDATE tbl_notabeli SET scan='1',updatex='$updatex' WHERE nonota='$_REQUEST[nonota]'");
			$note =1;
		/*
		$dB = mysql_fetch_array(mysql_query("SELECT COUNT(nonota) AS qty FROM tbl_notabeli_det2_vw WHERE nonota='$_REQUEST[nonota]' AND status='1'"));
		$dC = mysql_fetch_array(mysql_query("SELECT qty FROM tbl_notabeli WHERE nonota='$_REQUEST[nonota]'"));
		if($dB[qty]==$dC[qty])
			{
			$q1 = mysql_query("UPDATE tbl_notabeli SET scan='1',updatex='$updatex' WHERE nonota='$_REQUEST[nonota]'");
			$note =1;
			...
			{
			$q1 = mysql_query("UPDATE tbl_notabeli SET updatex='$updatex' WHERE nonota='$_REQUEST[nonota]'");
			$note =2;
			}
		*/
        
			  mysql_query("DELETE FROM stok_accu WHERE nonota='$_REQUEST[nonota]'");
		$q2 = mysql_query("INSERT INTO stok_accu VALUES ('$_REQUEST[nonota]','$_REQUEST[accu]','')");
		
			  mysql_query("DELETE FROM stok_alaskaki WHERE nonota='$_REQUEST[nonota]'");
		$q3 = mysql_query("INSERT INTO stok_alaskaki VALUES ('$_REQUEST[nonota]','$_REQUEST[alaskaki]','')");
		
			  mysql_query("DELETE FROM stok_anakkunci WHERE nonota='$_REQUEST[nonota]'");
		$q4 = mysql_query("INSERT INTO stok_anakkunci VALUES ('$_REQUEST[nonota]','$_REQUEST[anakkunci]','')");
		
			  mysql_query("DELETE FROM stok_helm WHERE nonota='$_REQUEST[nonota]'");
		$q5 = mysql_query("INSERT INTO stok_helm VALUES ('$_REQUEST[nonota]','$_REQUEST[helm]','')");
		
			  mysql_query("DELETE FROM stok_spion WHERE nonota='$_REQUEST[nonota])");
		$q6 = mysql_query("INSERT INTO stok_spion VALUES ('$_REQUEST[nonota]','$_REQUEST[spion]','')");
		
			  mysql_query("DELETE FROM stok_toolkit WHERE nonota='$_REQUEST[nonota]'");
		$q7 = mysql_query("INSERT INTO stok_toolkit VALUES ('$_REQUEST[nonota]','$_REQUEST[toolkit]','')");
		
			  mysql_query("DELETE FROM stok_jaket WHERE nonota='$_REQUEST[jaket]'");
		$q8 = mysql_query("INSERT INTO stok_jaket VALUES ('$_REQUEST[nonota]','$_REQUEST[jaket]','')");
		
			  mysql_query("DELETE FROM stok_bukuservis WHERE nonota='$_REQUEST[nonota]'");
