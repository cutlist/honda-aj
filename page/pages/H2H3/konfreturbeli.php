			                	...
			                	<h4>GUDANG & PDI <small>KONFIRMASI RETUR BELI</small></h4>
			                        ...
			                        <table id="example4" class="table table-striped" style="width:140%">
										...
										if(!empty($_REQUEST[cari]))
											...
											{
											$q1 = mysql_query("SELECT * FROM x23_returbeli_vw WHERE status IN ('0','1') ORDER BY id DESC LIMIT 0,20");
											}
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$d2 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS totqty,SUM(total) AS totjum,SUM(qtykeluar) AS qtykeluar,SUM(totalkeluar) AS totalkeluar FROM x23_returbeli_det WHERE noretur='$d1[noretur]'"));
			            ...
			            <div class="col-xs-12" style="margin-bottom:10px">
			                <div class="box box-danger">
			                	...
			                	<h4>GUDANG & PDI <small>KONFIRMASI RETUR BELI</small></h4>
			                        ...
			                        <table id="example2" class="table table-striped table-hover" style="width:200%">
			                            <thead style="color:#666;font-size:13px">
			                                <tr>
			                            ...
			                            if($d1[status]=='0')
			                            	{
											$qX = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE noretur='$d1[noretur]' AND qtykeluar=''");
				                            while($dX = mysql_fetch_array($qX))
				                            	{
												$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det2_vw WHERE idbarang='$dX[idbarang]' AND idgudang='$dX[idgudang]' AND rak='$dX[rak]' AND nonota='$dX[nonota]'"));
			                           			$dSt = mysql_fetch_array(mysql_query("SELECT stok FROM x23_stokpart WHERE idbarang='$dX[idbarang]' AND idgudang='$dX[idgudang]' AND rak='$dX[rak]' AND nonota='$dX[nonota]'"));
									   ?>
			                            ...
			                            <?
			                            	...
			                            	{
											$qX = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE noretur='$d1[noretur]'");
				                            while($dX = mysql_fetch_array($qX))
				                            	{
												$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det2_vw WHERE idbarang='$dX[idbarang]' AND idgudang='$dX[idgudang]' AND rak='$dX[rak]' AND nonota='$dX[nonota]'"));
			                            		$dSt = mysql_fetch_array(mysql_query("SELECT stok FROM x23_stokpart WHERE idbarang='$dX[idbarang]' AND idgudang='$dX[idgudang]' AND rak='$dX[rak]' AND nonota='$dX[nonota]'"));
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>ADMINISTRASI <small>RETUR BELI <i class="fa fa-angle-right"></i> <?echo $d1[nodo]?></small></h4>
			                        ...
			                        <table id="example2" class="table table-striped table-hover" style="width:200%">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE noretur='$d1[noretur]' AND qtykeluar=''");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det2_vw WHERE idbarang='$d1[idbarang]' AND idgudang='$d1[idgudang]' AND rak='$d1[rak]' AND nonota='$d1[nonota]'"));
											...
											$dSt = mysql_fetch_array(mysql_query("SELECT stok FROM x23_stokpart WHERE idbarang='$d1[idbarang]' AND idgudang='$d1[idgudang]' AND rak='$d1[rak]' AND nonota='$d1[nonota]'"));
...
<?
		...
		$qA = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE noretur='$_REQUEST[noretur]'");
        while($dA = mysql_fetch_array($qA))
        	...
        	if(!empty($_REQUEST[qtyout.$id]))
        		...
        		mysql_query("UPDATE x23_returbeli_det SET qtykeluar='$qtykeluar',totalkeluar='$totalkeluar' WHERE id='$id'");
        		}
        	}
		...
		$q1 = mysql_query("UPDATE x23_returbeli SET
											...
											idgdg='$_SESSION[id]',
											updatex='$updatex'
										WHERE noretur='$_REQUEST[noretur]'
							");
