<?
	if($submenu == 'saveA')
        ...
        $dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_lembur WHERE idkaryawan='$_REQUEST[idkaryawan]' AND tanggal='$tanggal'"));
        if(!empty($dCek[id]))
        	{
		...
		$dK    = mysql_fetch_array(mysql_query("SELECT nik FROM x23_karyawan WHERE id='$_REQUEST[idkaryawan]'"));
		$dCek2 = mysql_fetch_array(mysql_query("SELECT id FROM abs_x23_status WHERE EmployeeID='$dK[nik]' AND  awal <= '$tanggal' AND akhir >= '$tanggal'"));
		if(!empty($dCek2[id]))
			{
   		...
   		$q1 = mysql_query("INSERT INTO x23_lembur (
   													idkaryawan, 
   													tahun, 
   													bulan, 
   													tanggal, 
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM x23_lembur WHERE id='$_REQUEST[del]'");
			}
?>
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>DAFTAR LEMBUR</small></h4>
                                    <div style="float:right" class="col-xs-6">
										...
										$q1 = mysql_query("SELECT * FROM x23_lembur_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun'");
										?>
                                    		<tr>
									...
									<table id="example2" class="table table-striped">
			                            ...
			                            <?
	                                    	if($_SESSION[posisi]=='DIREKSI')
	                                    		{		
												$d2	= mysql_fetch_array(mysql_query("SELECT id FROM x23_uanglembur WHERE idkaryawan='$d1[idkaryawan]' AND tanggal='$d1[tanggal]'"));
				                            	if(empty($d2[id]))
				                            		{
			                           	?>
				                    	...
				                    	<table border="0" width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="idkaryawan" class="form-control" id="select1" style="font-size:12px;padding:3px" required="">
																		...
																		<?
																			$q1 = mysql_query("SELECT id,nama FROM x23_karyawan WHERE posisi NOT IN ('1','6') AND status='AKTIF' ORDER BY nama");
...
<?
		...
		if($_REQUEST[bayar]=="1")
			{
			$dEid = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id='$_REQUEST[idkaryawan]'"));
			$dCek = mysql_fetch_array(mysql_query("SELECT EmployeeID FROM abs_x23_result WHERE EmployeeID='$dEid[nik]' AND substr(Scan4,1,10)='$_REQUEST[tanggal]'"));
			if(empty($dCek[EmployeeID]))
				{
				echo "<script>alert ('$dEid[nama] Tidak Masuk Pada Tanggal Yang Sama Dengan Tanggal Lembur.')</script>";
			...
			if($tglbayar < $_REQUEST[tanggal])
	   		...
	   		$q1 = mysql_query("INSERT INTO x23_uanglembur (
	   													idkaryawan, 
	   													nama, 
	   													posisi,
	   													tahun, 
			...
			$iduanglembur = mysql_insert_id();
				
			//echo "<script>alert ('Proses berhasil.')</script>";
			echo "			
			<script type='text/javascript'>
				window.open('print/h2/bayarlembur.php?id=$iduanglembur','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
			</script>";
		...
		if(!empty($_REQUEST[batal]))
			{
			mysql_query("DELETE FROM x23_uanglembur WHERE id='$_REQUEST[batal]'");
			}
?>
			<aside class="right-side">
			    <section class="content">
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>SDM <small>PEMBAYARAN UANG LEMBUR</small></h4>
                                    <div style="float:right" class="col-xs-6">
										...
										$q1 = mysql_query("SELECT * FROM x23_lembur_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idkaryawan IN (SELECT id FROM x23_karyawan WHERE status='AKTIF')");
										$q2 = mysql_query("SELECT * FROM x23_lembur_vw WHERE bulan='$periode_bulan' AND tahun='$periode_tahun' AND idkaryawan IN (SELECT id FROM x23_karyawan WHERE status='AKTIF')");
										?>
                                    		<tr>
                                    			<td width="70%"><select name="bulan" class="form-control" style="height:35px">
														<option value='' >- PILIH -</option>
										...
										if($_SESSION[posisi]=='DIREKSI' || $_SESSION[posisi]=='KEPALA BENGKEL')
												...
												?>
						                           		<td width="1%">
													<button type="button"  onclick="window.open('print/h2/pembayaranlembur.php?bulan=<?echo $_REQUEST[bulan]?>&tahun=<?echo $_REQUEST[tahun]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
									...
									<table id="example2" class="table table-striped">
			                            <thead style="color:#666;font-size:13px">
			                                <tr>
			                                    ...
			                                    <th style="padding:7px;width:1%">CETAK</th>
			                            ...
			                            <?
												if(!empty($d2[id]))
													{
										?>			   
			                                    	<button type="button" class="btn btn-warning" onclick="window.open('print/h2/bayarlembur.php?id=<?echo $d2[id]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" style="padding:0 5px 0 5px;">
								...
								while($dA = mysql_fetch_array($q2))
	                            	{
	                            	//$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_lembur_vw WHERE id='$d2[id]'"));
						        ?>
							<!-- ################## MODAL ########################################################################################## -->
