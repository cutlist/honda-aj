...
?>
			<script type="text/javascript">
				...
				function popup_print(){
					window.open('print/h2/laporanws3.php?periode_awal=<?echo $_SESSION[periode_awal]?>&periode_akhir=<?echo $_SESSION[periode_akhir]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
					}
			</script>
			        ...
			        <div class="row">
			            <div class="col-xs-12">	     
			                <div class="box box-danger" style="overflow-x:auto;overflow-y:auto;height:520px;">
			                	...
			                	<h4>KASIR <small>LAPORAN PENJUALAN WORKSHOP GEMBIRA RIA  </small></h4>	 
			                ...
			                <?
										...
										$dA = mysql_fetch_array(mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'"));
										$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jns='SPARE PART'"));
										$dC = mysql_fetch_array(mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jns='OLI'"));
										
										$q1 = mysql_query("SELECT *,SUM(total) AS total FROM x23_notajual_det_ws1c WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY nonota");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											/*
											if(!empty($d1[total])){
												$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_kwitansi WHERE nomor='$d1[nonota]'"));
												if($d2[status]=="0"){
													$cash = "0";
													}
												if($d2[status]=="1"){
											...
											*/
											...
											$d3 = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM x23_notajual_det_ws1c WHERE nonota='$d1[nonota]' AND jns='SPARE PART'"));
											$d4 = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM x23_notajual_det_ws1c WHERE nonota='$d1[nonota]' AND jns='OLI'"));
...
?>
			<script type="text/javascript">
				var s5_taf_parent = window.location;
				function popup_print(){
					window.open('print/pndharianpenjualan.php?periode=<?echo $_REQUEST[periode]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
					}
			</script>
			<aside class="right-side">
			    <section class="content">
			                        ...
			                        <table id="example2" class="table table-bordered table-striped" style="width:130%">
										<thead>
											<tr>
												...
												<th width="10%"><center>HARGA JUAL (RP)</center></th>
										...
										if(empty($_REQUEST[periode]))
											{
			                            	$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE tglnota=CURDATE() AND nonota IN (SELECT nonota FROM x23_notajual WHERE tglnota=CURDATE())");
											$dA = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS a,
																					SUM(diskon) AS b,
																					SUM(total) AS c
																				FROM x23_notajual_det WHERE tglnota=CURDATE() AND nonota IN (SELECT nonota FROM x23_notajual WHERE tglnota=CURDATE())"));
											}
										else
											{
			                            	$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE tglnota BETWEEN '$periode_awal' AND '$periode_akhir' AND nonota IN (SELECT nonota FROM x23_notajual WHERE tglnota BETWEEN '$periode_awal' AND '$periode_akhir')");
											$dA = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS a,
																					SUM(diskon) AS b,
																					SUM(total) AS c
																				FROM x23_notajual_det WHERE tglnota BETWEEN '$periode_awal' AND '$periode_akhir' AND nonota IN (SELECT nonota FROM x23_notajual WHERE tglnota BETWEEN '$periode_awal' AND '$periode_akhir')"));
			                            	}
			                            while($d1 = mysql_fetch_array($q1))
			                            	{											
			                            ?>
