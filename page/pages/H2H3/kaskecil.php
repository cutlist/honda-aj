<?
	if($submenu == 'A')
		...
		if($_REQUEST[input]=='1')
			...
			$dcek = mysql_fetch_array(mysql_query("SELECT id FROM x23_kaskecil WHERE jenis='$jenis' AND tanggal='$tanggal' AND keterangan='$keterangan' AND jumlah='$jumlah'"));
			if(!empty($dcek[id]))
				{
				print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=$submenu&periode=$_REQUEST[periode]'/>";
				exit();
			...
			$q1 = mysql_query("INSERT INTO x23_kaskecil (
													jenis,
													tanggal,
													keterangan,
													jumlah) 
			...
			print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=$submenu&input=0&periode=$_REQUEST[periode]'/>";
			exit();
			}
		...
		if(!empty($_REQUEST[ubah]))
			...
			$q1 = mysql_query("UPDATE x23_kaskecil SET jenis='$jenis', keterangan='$keterangan', jumlah='$jumlah', tanggal='$tanggal'
		...
		if(!empty($_REQUEST[delkas]))
			{
			$q1 = mysql_query("DELETE FROM x23_abis_dkonfirmasi WHERE idkaskecil='$_REQUEST[delkas]'");
			$q1 = mysql_query("DELETE FROM x23_kaskecil WHERE id='$_REQUEST[delkas]'");
			$q2 = mysql_query("INSERT INTO log_act VALUES (										
		                                    '',
		                                    'x23_kaskecil',
		                                    CURDATE(),
...
?>
			<aside class="right-side">
			    <section class="content">
			                ...
			                <div class="box box-danger" style="overflow-x:hidden;overflow-y:auto;height:520px;">
			                	...
			                	<h4>KASIR <small>KAS KECIL</small></h4>	 
			                        ...
			                        <table id="" class="table table-bordered table-striped">
										...
										$q1 = mysql_query("SELECT * FROM x23_kaskecil WHERE tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' ORDER BY id DESC");
										$dI = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_kaskecil WHERE jenis='INPUT' AND status='1'"));
										$dO = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_kaskecil WHERE jenis='OUTPUT' AND status='1'"));
										$dIx = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_kaskecil WHERE jenis='INPUT' AND tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND status='1'"));
										$dOx = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_kaskecil WHERE jenis='OUTPUT' AND tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND status='1'"));
			        ...
			        <?
			        $qM = mysql_query("SELECT * FROM x23_kaskecil WHERE tanggal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' ORDER BY id DESC");
			                        ...
			                        <div class="modal-body">
					                    <table width="100%">
					                    		...
					                    		<tr>
					                    			...
					                    			<td valign="top">:</td>
					                    			<td valign="top" colspan="3"><textarea name="keterangan" class="form-control" maxlength="400" required><?echo $dM[keterangan]?></textarea></td>
					                    		</tr>
					                    		<tr>
					                    			<td>JUMLAH</td>
					                    			<td>:</td>
			                        ...
			                        <div class="modal-body">
					                    <table width="100%">
					                    		...
					                    		<tr>
					                    			<td valign="top">KETERANGAN</td>
					                    			<td valign="top">:</td>
					                    			<td valign="top" colspan="3"><textarea name="keterangan" maxlength="400" class="form-control" required><?echo $d1[keterangan]?></textarea></td>
					                    		</tr>
					                    		<tr>
					                    			<td>JUMLAH</td>
					                    			<td>:</td>
				                ...
				                <div class="modal-content">
			                        ...
			                        <div class="modal-body" style="overflow-y:auto;overflow-x:hidden;height:500px;">
										<table id="example4" class="table table-striped">
				                            ...
				                            <?
				                            $no = 1;
				                            $q1 = mysql_query("SELECT * FROM log_act WHERE tbl='x23_kaskecil' ORDER BY id DESC LIMIT 100");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
				                                <tr>
