<?
	if($submenu == 'A')
		...
		if(!empty($_REQUEST[tutupservis]))
			...
			$dA = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_scanmasuk WHERE tanggal=CURDATE()"));
			$asmasukbengkel = round($dA[total]/2);
			$dB = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_notaservice WHERE tglnota=CURDATE()"));
			$asmulaiservis = $dB[total];
			$dC = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_notaservice WHERE tglnota=CURDATE() AND status!='0'"));
			$asselesaiservis = $dC[total];
			$dD = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_scankeluar WHERE tanggal=CURDATE()"));
			$askeluarbengkel = round($dD[total]/2);
			$dF = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM x23_notaservice WHERE tglnota=CURDATE() AND status='2'"));
...
<?
					if(empty($mod))
						{
						$dUM = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS tot FROM x23_notaservice_vw WHERE status='0'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                	...
			                	<h4>SERVIS <small>TUTUP SERVIS</small></h4>
									<?
									$dC = mysql_fetch_array(mysql_query("SELECT id FROM x23_tutupservis WHERE tanggal=CURDATE()"));
									if(empty($dC[id]))
										{
									?>
