...
<?
		...
		if(!empty($dX[idopname]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_opname_vw WHERE id='$dX[idopname]'"));
			if($dA[totselisih] < 0){
			                	...
			                	<h4>AKTIVITAS BISNIS <small>KONFIRMASI STOCK OPNAME</small></h4>		
			                		...
			                		<div style="padding:20px;overflow-y:auto;overflow-x:auto;height:300px;margin-top:-20px">
				                        <table id="example2" class="table table-striped table-hover" style="width:140%">
				                            ...
				                            <tbody>
				                            <?	
											$qIm = mysql_query("SELECT * FROM x23_opname_det_vw WHERE idopname='$dX[idopname]'");
...
<?
		...
		if(!empty($dX[idtutupservis]))
			...
			$d1 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id='$dA[iduser]'"));
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
...
<?
		...
		if(!empty($dX[idtutupharian]))
			{
			$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_tutupharian WHERE id='$dX[idtutupharian]'"));
			$d1 = mysql_fetch_array(mysql_query("SELECT nama FROM x23_user_vw WHERE id='$dA[iduser]'"));
			
			mysql_query("UPDATE x23_abis_dkonfirmasi SET status='3' WHERE id='$_REQUEST[id]'");
			if(!empty($_REQUEST[del])){
				mysql_query("DELETE FROM x23_tutupharian WHERE id='$_REQUEST[del]'");
