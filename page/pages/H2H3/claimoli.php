<?
	if($submenu == 'A')
		{
		mysql_query("UPDATE x23_claimoli_det SET statuscek='0' WHERE nonota=''");	
			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>PENAGIHAN <small>TAGIHAN OLI KE MPM</small></h4>
                                    ...
                                    <div style="float:left;width:45%">
		                           	...
		                           	<div style="float:right" class="col-xs-6">
										...
										if($_SESSION[posisi]=='DIREKSI' || $_SESSION[posisi]=='KEPALA BENGKEL')
											{
										?>
											<button type="button"  onclick="window.open('print/h2/claimoli.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           			<?
	                           				}
	                           			?>
	                           		</div>
			                        <table id="example3" class="table table-striped table-hover">
										...
										$q1 = mysql_query("SELECT * FROM x23_notabeli WHERE tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND jns='CLAIM'");
...
?>
						<script type="text/javascript">
							...
							function popup_print(){
								window.open('print/penagihanoli.php?nokwitansi=<?echo $d1[nokwitansi]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
								}
						</script>
			                    ...
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:auto;height:260px;">
			                        <table id="example2" class="table table-striped table-hover" style="width:150%">
			                            ...
			                            while($d1 = mysql_fetch_array($q1))
											...
											$dB  = mysql_fetch_array(mysql_query("SELECT gudang FROM x23_gudang WHERE id='$d1[idgudang]]'"));
			                            ?>
			                                <tr style="cursor:pointer">
...
<?
		...
		{
        $dCek1 = mysql_fetch_array(mysql_query("SELECT id FROM x23_claimoli_det WHERE nonota=''"));	
		if(empty($dCek1[id]))
			{
			echo "<script>alert ('Tidak Ada Oli Yang Dapat Diclaim.')</script>";
        ...
        $dPO = mysql_fetch_array(mysql_query("SELECT nopo FROM x23_notabeli WHERE tglnota=CURDATE() AND jns='CLAIM' ORDER BY SUBSTR(nopo,-3,3) DESC LIMIT 1"));
        ...
        $dNK = mysql_fetch_array(mysql_query("SELECT nokwitansi FROM x23_notabeli WHERE tglnota=CURDATE() ORDER BY SUBSTR(nokwitansi,-3,3) DESC LIMIT 1"));
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        ...
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
											  mysql_query("UPDATE x23_claimoli_det SET nonota='' WHERE nonota='$_SESSION[nonotaclaim]'");	
										$q1 = mysql_query("SELECT * FROM x23_claimoli_det WHERE nonota=''");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	if($d1[statuscek]=="0"){
...
<?
		...
		foreach($_REQUEST['scan'] AS $scan)
			{
			mysql_query("UPDATE x23_claimoli_det SET statuscek='1',nonota='$_SESSION[nonotaclaim]' WHERE id='$scan'");	
			}
			
        mysql_query("DELETE FROM x23_notabeli_det WHERE nonota='$_SESSION[nonotaclaim]'");
		$qT2 = mysql_query("SELECT * FROM x23_claimoli_det WHERE nonota='$_SESSION[nonotaclaim]'");
		...
		$d2 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS qty FROM x23_notabeli_det WHERE nonota='$_SESSION[nonotaclaim]'"));			                           
		$d3 = mysql_fetch_array(mysql_query("SELECT SUM(hargaoli) AS hargaoli FROM x23_claimoli_det WHERE nonota='$_SESSION[nonotaclaim]'"));			                           
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM x23_notabeli_det WHERE nonota='$_SESSION[nonotaclaim]'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$dD1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_claimoli_det WHERE id='$d1[id_claimoli_det]'"));
			                            ?>
...
<?
		...
		echo "			
		<script type='text/javascript'>
			window.open('print/penagihanoli.php?nokwitansi=$_SESSION[nokwitansi]','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
		</script>";
		...
		$q1 = mysql_query("UPDATE x23_notabeli SET 
										...
										updatex='$updatex'
									WHERE nonota='$_SESSION[nonotaclaim]'
						");
