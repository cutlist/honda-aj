			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>KONFIRMASI NOTA BELI <small>UNIT MASUK</small></h4>
			                        ...
			                        <table id="example1" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM x23_notabeli_vw WHERE scan='0'");
...
<?
		...
		{
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE id='$_REQUEST[id]'"));
		$qA = mysql_query("SELECT id FROM x23_notabeli_det WHERE nonota='$d1[nonota]'");
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
...
<?
        ...
        while($dA = mysql_fetch_array($qA))
	        ...
	        mysql_query("UPDATE x23_notabeli_det SET tgltiba=CURDATE() WHERE id='$id'");	
        ...
        */
        if($dD[cek] > "0")
        	{
        	$note = "2";
			mysql_query("UPDATE x23_notabeli SET updatex='$updatex',konf='1',iduserkonf='$_SESSION[id]' WHERE nonota='$_REQUEST[nonota]'");
        ...
        if($dD[cek] == "0")
        	{
        	$note = "1";
			mysql_query("UPDATE x23_notabeli SET scan='1',konf='1',gtbayar=grandtotalppn,iduserkonf='$_SESSION[id]',updatex='$updatex' WHERE nonota='$_REQUEST[nonota]'");
			}
        /*
		$dB = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS qty FROM x23_notabeli_det WHERE nonota='$_REQUEST[nonota]'"));
		$dC = mysql_fetch_array(mysql_query("SELECT SUM(stok) AS qty FROM x23_stokpart WHERE nonota='$_REQUEST[nonota]'"));
		if($dB[qty]==$dC[qty])
			{
			mysql_query("UPDATE x23_notabeli SET gtbayar=grandtotal WHERE nonota='$_REQUEST[nonota]'");
			mysql_query("UPDATE x23_notabeli SET scan='1',konf='1',updatex='$updatex' WHERE nonota='$_REQUEST[nonota]'");
			}
		else
			{
			mysql_query("UPDATE x23_notabeli SET updatex='$updatex',konf='1' WHERE nonota='$_REQUEST[nonota]'");
			}
		*/		
