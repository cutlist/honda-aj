		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				*/
		        ...
		        $dN = mysql_fetch_array(mysql_query("SELECT nik FROM x23_karyawan ORDER BY SUBSTR(nik,-3,3) DESC LIMIT 1"));
				...
				if (!empty($lokasi_file))
					...
					UploadKaryawanH2H3($nama_file_unik);
					}
				else{
					$nama_file_unik =""; 
					}
				...
				$pangkat	= strtoupper($_REQUEST['pangkat']);
				...
				$q1 = mysql_query("INSERT INTO x23_karyawan (
													nama, 
													nik, 
													posisi, 
													pangkat, 
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_karyawan WHERE id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_karyawan',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[ubah]))
				...
				$dC = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS totjum FROM x23_potkompensasi WHERE idkaryawan='$_REQUEST[ubah]' AND metodebayar='GAJI' AND substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND status='1'"));
				$dD	= mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE substr(tgl,6,2)='$periode_bulan' AND substr(tgl,1,4)='$periode_tahun' AND idkaryawan='$_REQUEST[ubah]' AND metodebayar='GAJI' AND status='1'"));
			                	...
			                	<h4>MASTER <small>KARYAWAN</small></h4>
	                           		<div style="float:right" class="col-xs-7">
										...
										if($_SESSION[posisi]=='DIREKSI' || $_SESSION[posisi]=='KEPALA BENGKEL')
											{
										?>
											<button type="button"  onclick="window.open('print/h2/karyawan.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           			<?
	                           				}
	                           			?>
	                           		</div>
			                        <table id="example1" class="table table-bordered table-striped">
										...
										if($_SESSION[posisi]=="DIREKSI"){
											$q1 = mysql_query("SELECT * FROM x23_karyawan_vw WHERE id !='1' ORDER BY nama");
											}
										else{
											$q1 = mysql_query("SELECT * FROM x23_karyawan_vw WHERE id_posisi NOT IN ('1','6') ORDER BY nama");
											}
			                            ...
			                            ?>
			                                <tr style="cursor:pointer">
			                                    ...
			                                    <td width="1%" align="center">
													<?
													if($_SESSION[posisi]=='DIREKSI' OR $_SESSION[posisi]=='KEPALA BENGKEL')
														{
													?>
			                        ...
			                        <div class="modal-body" style="overflow-y:auto;overflow-x:hidden;height:420px;">
				                    	<table>
				                    			...
				                    			<td colspan="2"><select class="form-control" name="posisi" onchange="populateSelect1(this.value)" required style="width: 50%">
																...
																<?
																	if($_SESSION[posisi]=="DIREKSI"){
																		...
																		$q1 = mysql_query("SELECT * FROM x23_posisi WHERE id NOT IN ('1','6') ORDER BY posisi");
																		}
				                    		...
				                    		<tr>
				                    			...
				                    			<td>:</td>
				                    			<td colspan="1"><input type="text" name="tmplahir" maxlength="30" class="form-control" required></td>
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan_vw WHERE id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
				                	...
				                	<div style="padding:20px">
				                    	<table style="width:50%;">
				                    		...
				                    		<tr>
				                    			<td>GOLONGAN</td>
				                    			<td>:</td>
				                    			<td colspan="2"><select class="form-control" name="pangkat" style="width: 50%">
																	<option value=''>Pilih</option>
																<?
																	$q1 = mysql_query("SELECT * FROM x23_pangkat ORDER BY pangkat");
																	while($dA=mysql_fetch_array($q1))
				                    		...
				                    		<tr>
				                    			<td>TEMPAT/TGL LAHIR</td>
				                    			<td>:</td>
				                    			<td colspan="1"><input type="text" name="tmplahir" value="<?echo $d1[tmplahir]?>" maxlength="30" class="form-control" readonly></td>
...
?>
						<script type="text/javascript">
							...
							function popup_print(){
								window.open('print/karyawanh23.php?id=<?echo $_REQUEST[id]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
								}
						</script>
			            <div class="col-xs-12">
			                <div class="box box-danger">
				                	...
				                	<div style="padding:20px">
				                    	<table style="width:50%;">
				                    		...
				                    		<tr>
				                    			<td>GOLONGAN</td>
				                    			<td>:</td>
				                    			<td colspan="2"><select class="form-control" name="pangkat" required style="width: 50%" <?if($d1[posisi]!="MEKANIK"){?>disabled=""<?}else{?>required<?}?>>
																	<option value=''>Pilih</option>
																<?
																	$q1 = mysql_query("SELECT * FROM x23_pangkat ORDER BY pangkat");
																	while($dA=mysql_fetch_array($q1))
									...
									?>
					                	<div class="col-xs-4" style="padding:20px;text-align: center;">
					                		<div style="border: 1px #ccc solid;padding:20px;width:80%;padding-top:10px">
						                		...
						                		<img src="../foto/H2H3/H2H3_<?echo $d1[photo]?>"/>
					                		</div>
					                	</div>
									<?
										}
...
?>
		...
		{
			pilihan = document.inputkaryawan.posisi.value;
			if(pilihan=='4'){
			document.inputkaryawan.pangkat.disabled = 0;
			}else{
			document.inputkaryawan.pangkat.disabled = 1;
			}
		}
		</script>
