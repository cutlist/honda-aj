	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[input]))
				...
				$d1 = mysql_fetch_array(mysql_query("SELECT user FROM x23_user WHERE user='$user' OR id_karyawan='$id_karyawan'"));
			...
			if(!empty($_REQUEST[resetpass]))
				...
				if($pass1 != $pass2)
					...
					{
					$q1 = mysql_query("UPDATE x23_user SET pass='$pass',pic_user='$_SESSION[user]' WHERE id='$_REQUEST[resetpass]'");
					$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_user',
			                                    CURDATE(),
			...
			if(!empty($_REQUEST[ubahdetail]))
				{
				$q1 = mysql_query("UPDATE x23_user SET hakakses='$_REQUEST[hakakses]',pic_user='$_SESSION[user]' WHERE id='$_REQUEST[ubahdetail]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
		                                    '',
		                                    'x23_user',
		                                    CURDATE(),
			...
			if(!empty($_REQUEST[deluser]))
				{
				$q1 = mysql_query("DELETE FROM x23_user WHERE id='$_REQUEST[deluser]'");
				$q2 = mysql_query("INSERT INTO log_act VALUES (										
			                                    '',
			                                    'x23_user',
			                                    CURDATE(),
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select class="form-control select1" name="id_karyawan" style="width:70%;font-size:12px;padding:3px" required>
																	<option value=''>Pilih</option>
																<?
																	$q1 = mysql_query("SELECT * FROM x23_karyawan WHERE posisi!='6' AND status='AKTIF' AND id NOT IN (SELECT id_karyawan FROM x23_user) ORDER BY nama");
																	while($dA=mysql_fetch_array($q1))
																		{
																?>
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_user_vw WHERE id='$_REQUEST[id]'"));
?>
			            <div class="col-xs-12">
			                <div class="box box-danger">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:520px;">
