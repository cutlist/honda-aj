...
<?
	if($submenu == 'saveB')
		...
		$qA = mysql_query("SELECT * FROM x23_returjual_det WHERE noreturjual='$_SESSION[noreturjual]'");
		...
		/*
		$qB = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE nonota='$_SESSION[nonotajual]'");
        while($dB = mysql_fetch_array($qB))
        	{
        	$dC = mysql_fetch_array(mysql_query("SELECT * FROM x23_returjual_det WHERE nonotajual='$_SESSION[nonotajual]' AND noreturjual='$_SESSION[noreturjual]' AND idbarang='$dB[idbarang]'"));
			mysql_query("UPDATE x23_stokpart SET stok=stok+$dC[qty] WHERE idgudang='$dB[idgudang]' AND rak='$dB[rak]' AND nonota='$dB[notabeli]' AND idbarang='$dB[idbarang]'");
			}
		*/
			                	...
			                	<h4>PENJUALAN <small>RETUR JUAL</small></h4>
									...
									<table id="example1" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM x23_returjual_vw WHERE noreturjual LIKE '%$_REQUEST[cari]%' OR nonotajual LIKE '%$_REQUEST[cari]%' LIMIT 0,20");
											$q2 = mysql_query("SELECT * FROM x23_returjual_vw WHERE noreturjual LIKE '%$_REQUEST[cari]%' OR nonotajual LIKE '%$_REQUEST[cari]%'");
											}
										else
											{
...
<?
        ...
        $dNR = mysql_fetch_array(mysql_query("SELECT noreturjual FROM x23_returjual WHERE tanggal=CURDATE() ORDER BY SUBSTR(noreturjual,-3,3) DESC LIMIT 1"));
...
<?
		...
		$dCek = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_vw WHERE nonota='$_SESSION[nonotajual]' AND grup='1' AND nonota NOT IN (SELECT nonotajual FROM x23_returjual)"));
		if(empty($dCek[id]))
			{
		...
		$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id='$dCek[idpelanggan]'"));
		...
		if(!empty($_REQUEST[temp]))
			...
			$dcs1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_det_vw WHERE idbarang='$_REQUEST[idbarang]' AND nonota='$_REQUEST[temp]'"));	
			if($qty > $dcs1[qty]){
		...
		if(!empty($_REQUEST[del]))
			{
			mysql_query("DELETE FROM x23_returjual_det WHERE id='$_REQUEST[del]'");
			}
			
		if(!empty($_REQUEST[back]))
			{
			mysql_query("DELETE FROM x23_returjual_det WHERE noreturjual='$_SESSION[noreturjual]'");
			print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=B'/>";
			exit();
			}
					
		$dB = mysql_fetch_array(mysql_query("SELECT *,SUM(qty) AS tqty,SUM(total) AS ttotal FROM x23_returjual_det_vw WHERE noreturjual='$_SESSION[noreturjual]'"));
?>
			        ...
			        <div class="row">
			                    	...
			                    	<div id="spoiler" style="display:none">
										...
										<table class="table table-striped table-hover" >
											<thead>
				                       		...
				                       		<tbody>
				                            <?
				                            $q1 = mysql_query("SELECT * FROM x23_notajual WHERE idpelanggan='$dCek[idpelanggan]' ORDER BY tglnota DESC");
				                            while($d1 = mysql_fetch_array($q1))
				                            	{
				                            ?>
			            ...
			            <div class="col-xs-12">
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                    <?
			                    $dC1 = mysql_fetch_array(mysql_query("SELECT id FROM x23_returjual_det_vw WHERE noreturjual='$_SESSION[noreturjual]'"));
			                    if(!empty($dC1[id]))
			                    	{
			                    ?>
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qA = mysql_query("SELECT * FROM x23_returjual_det_vw WHERE noreturjual='$_SESSION[noreturjual]'");
			                            while($dA = mysql_fetch_array($qA))
			                            	{
			                            ?>
				                    		...
				                    		<tr>
				                    			...
				                    			<td width="" colspan="2"><select name="idbarang" class="form-control select1" style="font-size:12px;padding:3px;width:100%" required="">
																		...
																		<?
																			...
																			$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE nonota='$_SESSION[nonotajual]' AND id NOT IN (SELECT idbarang FROM x23_returjual_det WHERE nonotajual='$_SESSION[nonotajual]') ORDER BY kodebarang");
