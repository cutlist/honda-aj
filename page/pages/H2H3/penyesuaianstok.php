			                ...
			                <div class="box box-danger">
			                	...
			                	<h4>GUDANG & PDI <small>STOCK OPNAME</small></h4>
			                        ...
			                        <table id="example3" class="table table-striped table-hover">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM x23_opname WHERE bulan='$periode_bulan' AND tahun='$periode_tahun'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$dG = mysql_fetch_array(mysql_query("SELECT gudang FROM x23_gudang WHERE id='$d1[idgudang]'"));
			                            	if(empty($dG[gudang])){
												$lokasi = "SEMUA GUDANG";
												}
											else{
...
?>
		<script type="text/javascript">
			...
			function popup_print(){
				window.open('print/h2/stokopname.php?id=<?echo $_REQUEST[id]?>','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')
				}
		</script>
			<aside class="right-side">
			    <section class="content">
...
<?
		...
		$qIm = mysql_query("SELECT * FROM x23_stokpart_opname_vw WHERE idgudang='$_SESSION[idgudang]'");
        while($d1 = mysql_fetch_array($qIm))
        	{
			                    ...
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example1" class="table table-striped table-hover">
										...
										if(!empty($_REQUEST[update])){
											...
											mysql_query("UPDATE temp_x23_opname_det SET opname='$opname' WHERE id='$_REQUEST[update]'");
											}
										
										$q3  = mysql_query("SELECT * FROM temp_x23_opname_det_vw WHERE idgudang='$_SESSION[idgudang]' AND rak='$_REQUEST[koderak]'");
										$qIm = mysql_query("SELECT * FROM temp_x23_opname_det_vw WHERE idgudang='$_SESSION[idgudang]' AND rak='$_REQUEST[koderak]'");
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example1" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?	
										$qIm = mysql_query("SELECT * FROM temp_x23_opname_det_vw WHERE idgudang='$_SESSION[idgudang]'");
...
<?
		...
		$dG = mysql_fetch_array(mysql_query("SELECT gudang FROM x23_gudang WHERE id='$_SESSION[idgudang]'"));
		...
		$dS  = mysql_fetch_array(mysql_query("SELECT SUM(selisih) AS totselisih, SUM(totalselisih) AS totjumselisih FROM x23_opname_det WHERE idopname='$idopname'"));
		       mysql_query("UPDATE x23_opname SET totselisih='$dS[totselisih]', totjumselisih='$dS[totjumselisih]' WHERE id='$idopname'");
