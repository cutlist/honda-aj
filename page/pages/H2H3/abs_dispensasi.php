	...
	if(empty($mod))
		...
		if(!empty($_REQUEST[input]))
			...
			$status	= strtoupper($_REQUEST['status']);
		 	...
		 	$qCek1 = mysql_query("SELECT * FROM abs_x23_status WHERE EmployeeID='$_REQUEST[EmployeeID]' AND substr(awal,1,4)='$tahun'");
			...
			$q1 = mysql_query("INSERT INTO abs_x23_status (
												EmployeeID,
												awal,
												akhir,
												status,
		...
		if(!empty($_REQUEST[del]))
			{
			$q1 = mysql_query("DELETE FROM abs_x23_status WHERE id='$_REQUEST[del]'");
			$q2 = mysql_query("INSERT INTO log_act VALUES (										
		                                    '',
		                                    'abs_x23_status',
		                                    CURDATE(),
		                        ...
		                        <table id="example1" class="table table-bordered table-striped">
		                            <thead style="color:#666;font-size:13px">
		                                <tr>
		                                    ...
		                                    <th width="5%" style="padding:7px">DEL</th>
		                                </tr>
		                            </thead>
		                            <tbody>
		                            <?
									if(!empty($_REQUEST[periode]))
										...
										$q1 = mysql_query("SELECT * FROM abs_x23_status_vw WHERE awal BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'");
							            }
							        else{
										$q1 = mysql_query("SELECT * FROM abs_x23_status_vw");
										}
		                            ...
		                            ?>
										...
										if($_SESSION[posisi]=='DIREKSI')
											{
										?>
		                                    	<a href="<?echo "?opt=$opt&menu=$menu&submenu=$submenu&del=$d1[id]&EmployeeID=$d1[EmployeeID]&status=$d1[status]"?>" onclick="return confirm('Anda yakin akan menghapus data?')"><i class="fa fa-trash-o"></i></a>
		                                    <?}?>
		                                    </td>
		                                </tr>
			                    	...
			                    	<table width="100%">
			                    		<tr>
			                    			...
			                    			<td><select name="EmployeeID" class="form-control" id="select1" required="">
	                    							<option value=''>PILIH KARYAWAN</option>
															<?
																$q1 = mysql_query("SELECT * FROM abs_x23_employee_vw WHERE posisi!='6' GROUP BY EmployeeID ORDER BY FirstName");
			                    		...
			                    		<tr>
			                    			...
			                    			<td><div class="input-group">
	                                            	...
	                                            	<input type="text" name="periode" readonly="" style="height:33px;" <?if(empty($_REQUEST[periode])){?>placeholder="Pilih Periode"<?} else {?>value="<?echo $_REQUEST[periode]?>"<?}?>  class="form-control pull-right reservation"/>
	                                            </div></td>
			                    		</tr>
			                    		<tr>
