...
<?
		...
		if(!empty($_REQUEST[delsvc]))
			...
			$dCsvc = mysql_fetch_array(mysql_query("SELECT *,COUNT(tarif) AS tqty, SUM(tarif) AS ttotal, SUM(tarifasli) AS tottarifasli, SUM(diskon) AS totdiskon FROM x23_notaservice_det WHERE nonota='$_SESSION[nonota]'"));
				                	...
				                	<form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=saveB"?>" enctype="multipart/form-data">
				           			<div class="col-xs-12">
					                	...
					                	<?
											...
											if(!empty($dcp[id])){
												$dKP = mysql_fetch_array(mysql_query("SELECT * FROM x23_kelompokjasa WHERE kode='$d3[kodepaket]'"));
												$dcp2 = mysql_fetch_array(mysql_query("SELECT id FROM x23_notajual_det_vw WHERE nonota='$_SESSION[nonota]' AND total='0' AND totdiskon='0' AND qty='$dKP[qtyoli]'"));	
												if(empty($dcp2[id])){
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="idbarang" class="form-control select1" onchange="populateSelectNO1(this.value)" style="font-size:12px;padding:3px" required="">
																		...
																		while($dA=mysql_fetch_array($q1))
																			{
																			$dB = mysql_fetch_array(mysql_query("SELECT id FROM x23_stokpart_vw WHERE idbarang='$dA[idoli]' AND stok>'0' AND hargajual!=''"));
																			if(!empty($dB[id])){
																		?>
...
<?
	...
	if($submenu == 'NJ2')
		...
		echo "<option value=''>Pilih</option>";
		$qA = mysql_query("SELECT * FROM x23_stokpart_vw WHERE idbarang='$_SESSION[idbarang]' AND nonota='$q' AND stok>'0' GROUP BY nonota,idbarang,gudang ORDER BY gudang");
		while($d = mysql_fetch_array($qA))
			{
