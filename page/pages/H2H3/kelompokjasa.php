	...
	if($submenu == 'A')
		...
		if(empty($mod))
			...
			if(!empty($_REQUEST[delnota]))
				{
				$q1 = mysql_query("DELETE FROM x23_kelompokjasa WHERE id='$_REQUEST[delnota]'");
			                	...
			                	<h4>MASTER <small>PENGELOMPOKAN JASA</small></h4>
	                           		<div style="float:right" class="col-xs-7">
										...
										if($_SESSION[posisi]=='DIREKSI' || $_SESSION[posisi]=='KEPALA BENGKEL')
											{
										?>
											<button type="button"  onclick="window.open('print/h2/kelompokjasa.php','page','toolbar=0,titlebar=0,fullscreen=0,directories=0,channelmode=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=1,height=1,left=150,top=100')" class="btn btn-danger pull-right"><i class="fa fa-print"></i> Export Ke Excel</button>
	                           			<?
	                           				}
	                           			?>
	                           		</div>
...
<?
						...
						{
						$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_kelompokjasa WHERE id='$_REQUEST[id]'"));
						
						if(!empty($_REQUEST[tambahbarang]))
							{
							$norangka 	= strtoupper($_REQUEST['norangka']);
							$dcs = mysql_fetch_array(mysql_query("SELECT id FROM x23_kelompokjasa_det WHERE kode='$d1[kode]' AND idtarifjasa='$_REQUEST[idtarifjasa]'"));	
							if(!empty($dcs[id]))
								{
						...
						if(!empty($_REQUEST[tambaholi]))
							{
							$dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_kelompokjasa_oli WHERE kode='$d1[kode]' AND idoli='$_REQUEST[idoli]'"));
							if(!empty($dCek[id]))
								{
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qA = mysql_query("SELECT * FROM x23_kelompokjasa_det_vw WHERE kode='$d1[kode]'");
										...
										$d2 = mysql_fetch_array(mysql_query("SELECT SUM(tarif) AS total FROM x23_kelompokjasa_det_vw WHERE kode='$d1[kode]'"));		
			                            ?>
			                            </tbody>
			                            <tfoot>
			                            	<tr>
			                        ...
			                        <table id="example3" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qA2 = mysql_query("SELECT * FROM x23_kelompokjasa_oli WHERE kode='$d1[kode]'");
			                            while($dA2 = mysql_fetch_array($qA2))
			                            	{
			                            	$dA3 = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id='$dA2[idoli]'"))
			                            ?>
			                                <tr style="cursor:pointer">
				                    	...
				                    	<table width="100%">
				                    		<tr>
				                    			...
				                    			<td colspan="2"><select name="idoli" class="form-control" id="select2" style="font-size:12px;padding:3px" required="">
																		<option value='' selected>Pilih</option>
																		<?
																			$q1 = mysql_query("SELECT * FROM x23_masterbarang WHERE jns='OLI' ORDER BY kodebarang");
																			while($dA=mysql_fetch_array($q1))
																				{
																		?>
...
<?
        ...
        $dK = mysql_fetch_array(mysql_query("SELECT kode FROM x23_kelompokjasa WHERE SUBSTR(kode,3,2)='$p_tahun2' AND SUBSTR(kode,5,2)='$p_bulan' ORDER BY SUBSTR(kode,-3,3) DESC LIMIT 1"));
		...
		mysql_query("DELETE FROM x23_kelompokjasa_det WHERE kode='$kode'");
		mysql_query("DELETE FROM x23_kelompokjasa_oli WHERE kode='$kode'");
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
...
<?
		...
		if(!empty($_REQUEST[tambahbarang]))
			{
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_kelompokjasa_det WHERE kode='$_SESSION[kode]' AND idtarifjasa='$_REQUEST[idtarifjasa]'"));
			if(!empty($dCek[id]))
				{
				echo "<script>alert (' Mohon Ulangi, Karena Jasa Dan Kelompok Ini Sudah Ada Pada Database.')</script>";
		...
		if(!empty($_REQUEST[tambaholi]))
			{
			$dCek = mysql_fetch_array(mysql_query("SELECT id FROM x23_kelompokjasa_oli WHERE kode='$_SESSION[kode]' AND idoli='$_REQUEST[idoli]'"));
			if(!empty($dCek[id]))
				{
				echo "<script>alert (' Mohon Ulangi, Karena Oli Ini Sudah Ada Pada Database.')</script>";
				print "<meta http-equiv='refresh' content='0;url=?opt=$opt&menu=$menu&submenu=$submenu&direct=1'/>";
		...
		$d2 = mysql_fetch_array(mysql_query("SELECT SUM(tarif) AS total FROM x23_kelompokjasa_det_vw WHERE kode='$_SESSION[kode]'"));			                           
?>
			<aside class="right-side">
			    <section class="content">
			        <div class="row">
				                	...
				                	<form method="post" action="<?echo "?opt=$opt&menu=$menu&submenu=D"?>" enctype="multipart/form-data">
				           			<div class="col-xs-12">
				                        <div class="modal-footer clearfix">
					                    	...
					                    	<?
											$dTS = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS ts FROM x23_kelompokjasa_det WHERE kode='$_SESSION[kode]'"));
											if($dTS[ts]!="0"){
												if($_SESSION[oli]=='YA'){
												$dTS2 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS ts FROM x23_kelompokjasa_oli WHERE kode='$_SESSION[kode]'"));
													if($dTS2[ts]!="0"){
											?>
			                ...
			                <div class="box box-info">
			                    <div class="box-body table-responsive" style="overflow-y:auto;overflow-x:hidden;height:260px;">
			                        <table id="example2" class="table table-striped table-hover">
			                            ...
			                            <?
										$no=1;
										$q1 = mysql_query("SELECT * FROM x23_kelompokjasa_det_vw WHERE kode='$_SESSION[kode]'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            ?>
			                                <tr style="cursor:pointer">
			                        ...
			                        <table id="example3" class="table table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?
										$qA2 = mysql_query("SELECT * FROM x23_kelompokjasa_oli WHERE kode='$_SESSION[kode]'");
			                            while($dA2 = mysql_fetch_array($qA2))
			                            	{
			                            	$dA3 = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id='$dA2[idoli]'"))
			                            ?>
