			                    ...
			                    ?>
			                        ...
			                        <table id="example3" class="table table-bordered table-striped table-hover" style="width:230%;padding-right:20px">
			                            ...
			                            if(!empty($_REQUEST[periode]))
											...
											while (strtotime($awal) <=  strtotime($akhir)) 
				                            	{
					                            $dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_notajual_det_vw WHERE notabeli!='CLAIM' AND tglnota='$awal' GROUP BY idbarang,idgudang,rak,notabeli"));
			                                	if(!empty($dA[id]))
			                                		{
					                            ?>
					                                <tr style="cursor:pointer">
