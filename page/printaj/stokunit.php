...
$dG = mysql_fetch_array(mysql_query("SELECT * FROM tbl_gudang WHERE id%2=0 AND id='$_REQUEST[idgudang]'"));
...
?>
    ...
    <table>
		...
		if(!empty($_REQUEST[idgudang]))
			{
        	$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND status='STOK' AND idgudang='$_REQUEST[idgudang]' ORDER BY kodebarang");
			}
		else{
        	$q1 = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND status='STOK' ORDER BY gudang,kodebarang");
			}
