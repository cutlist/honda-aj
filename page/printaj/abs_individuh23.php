...
$tgl = date("Ymd");
$judul = "ABSENSI_INDIVIDU_H2H3_$_SESSION[periode_awal]_SD_$_SESSION[periode_akhir].xls";
header("Content-Disposition: attachment; filename=$judul");
 
 $dnk = mysql_fetch_array(mysql_query("SELECT nama FROM x23_karyawan WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
...
?>
	<h4>ABSENSI INDIVIDU H2H3 PERIODE TANGGAL ABSENSI <?echo date("d-m-Y",strtotime($_SESSION[periode_awal]))." SAMPAI DENGAN ".date("d-m-Y",strtotime($_SESSION[periode_akhir]));?></h4>
	<h4>NAMA KARYAWAN : <?echo $dnk[nama]?></h4>
	            <table>
	                ...
	                //echo  $awal.$akhir;
										...
										while (strtotime($awal) <=  strtotime($akhir)) 
			                            	{
											$d1  = mysql_fetch_array(mysql_query("SELECT * FROM abs_x23_status WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND awal <= '$awal' AND akhir >= '$awal'"));
											...
											$d2  = mysql_fetch_array(mysql_query("SELECT * FROM abs_x23_result_vw WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND substr(Date,1,10)='$awal'"));
											...
											$d3  = mysql_fetch_array(mysql_query("SELECT * FROM abs_x23_status WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND awal <= '$awal' AND akhir >= '$awal'"));
											if(empty($d3['id'])){
												if($jamkeluar == "-"){
													$status = "TIDAK HADIR";
													$tanpaket++;
						...
						$dNm = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
							echo"
	            ...
	            <div style="border-bottom:1px #aaa dashed;margin: 10px 0 10px"></div>
									...
									$jangka 	  = selisihHari($periode_awal, $akhir);
			                        ...
			                        $dterlambat = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_x23_terlambat WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
			                        $dovertime  = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_x23_overtime WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
									?>
	            	<table width="100%">
	            		<tr>
