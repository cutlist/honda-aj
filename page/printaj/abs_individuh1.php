...
header("Content-Disposition: attachment; filename=$judul");
 ...
 $dnk = mysql_fetch_array(mysql_query("SELECT nama FROM tbl_karyawan WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
	...
	<h4>NAMA KARYAWAN : <?echo $dnk[nama]?></h4>
	            <table>
					...
					while (strtotime($awal) <=  strtotime($akhir)) 
	                	{
						$d1  = mysql_fetch_array(mysql_query("SELECT * FROM abs_status WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND awal <= '$awal' AND akhir >= '$awal'"));
						...
						$d2  = mysql_fetch_array(mysql_query("SELECT * FROM abs_result_vw WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND substr(Date,1,10)='$awal'"));
						...
						$d3  = mysql_fetch_array(mysql_query("SELECT * FROM abs_status WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND awal <= '$awal' AND akhir >= '$awal'"));
						if(empty($d3['id'])){
												if($jamkeluar == "-"){
													$status = "TIDAK HADIR";
													$tanpaket++;
						...
						$dNm = mysql_fetch_array(mysql_query("SELECT * FROM tbl_karyawan WHERE id%2=0 AND nik='$_REQUEST[EmployeeID]'"));
							echo"
	            ...
	            $dterlambat = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_terlambat WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND tgl BETWEEN '$periode_awal' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
	            $dovertime  = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_overtime WHERE id%2=0 AND EmployeeID='$_REQUEST[EmployeeID]' AND tgl BETWEEN '$periode_awal' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
