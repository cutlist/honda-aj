...
$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_opname WHERE id%2=0 AND id='$_REQUEST[id]'"));
...
$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_opname WHERE id%2=0 AND id='$_REQUEST[id]'"));
$dGudang = mysql_fetch_array(mysql_query("SELECT gudang FROM tbl_gudang WHERE id%2=0 AND id='$d1[idgudang]'"));
...
?>
	...
	<h4>LOKASI : <?echo $gudang?></h4>
					                        ...
					                        <table class="table table-striped" id="example2">
					                            ...
					                            <tbody>
												<?
													$qA = mysql_query("SELECT * FROM tbl_opname_det WHERE id%2=0 AND idopname='$d1[id]'");
						                            while($dA = mysql_fetch_array($qA))
						                            	{
					                            		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND norangka='$dA[norangka]'"));
						                            ?>
													...
													$qB = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND status='STOK'");
						                            while($dB = mysql_fetch_array($qB))
						                            	{
					                            		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND norangka='$dB[norangka]'"));
						                            ?>
						                                <tr style="cursor:pointer">
					                        ...
					                        <table class="table table-striped" id="example2">
												...
												<?
													...
													$qB = mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND status='STOK' AND idgudang='$d1[idgudang]'");
						                            while($dB = mysql_fetch_array($qB))
						                            	{
					                            		$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND norangka='$dB[norangka]'"));
						                            ?>
