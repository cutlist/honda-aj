...
?>
	<h4>DAFTAR PEMBAYARAN LEASING & MAIN DEALER PERIODE TANGGAL NOTA JUAL <?echo date("d-m-Y",strtotime($_SESSION[periode_awal]))?> SAMPAI DENGAN <?echo date("d-m-Y",strtotime($_SESSION[periode_akhir]))?></h4>
			                        	<table id="example2" class="table table-bordered table-striped" style="min-width:420%">
				                            ...
				                            <tbody>
				                            <?
											//$q1 = mysql_query("SELECT * FROM tbl_notajual_det_vw WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND (jnstransaksi='CASH TEMPO' AND jnscashtempo='LEASING' OR jnstransaksi='KREDIT')");
											$q1 = mysql_query("SELECT * FROM tbl_notajual_det_vw WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND tglsampai!='0000-00-00'");
											while($d1 = mysql_fetch_array($q1))
				                            	{
				                            	$dBrg = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND norangka='$d1[norangka]'"));
				                            	$dUK  = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pengeluaranunit WHERE id%2=0 AND nonota='$d1[nonota]'"));
				                            	$dPlg  = mysql_fetch_array(mysql_query("SELECT nama FROM tbl_pelanggan WHERE id%2=0 AND id='$d1[idpelanggan]'"));
				                            	$dSls  = mysql_fetch_array(mysql_query("SELECT nama FROM tbl_user_vw WHERE id%2=0 AND id='$d1[iduser]'"));
				                            	$dLsg  = mysql_fetch_array(mysql_query("SELECT namaleasing FROM tbl_leasing WHERE id%2=0 AND id='$d1[idleasing]'"));
				                            	
				                            	//$dTot = mysql_fetch_array(mysql_query("SELECT SUM(otr) AS totr,SUM(gross) AS tgross,SUM(subsidipajak) AS tsubsidipajak,SUM(matrixpajak) AS tmatrixpajak,SUM(scpahm) AS tscpahm,SUM(scpmd) AS tscpmd FROM tbl_notajual_det_vw WHERE id%2=0 AND jnstransaksi='KREDIT' AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]'"));
				                            	$dTot = mysql_fetch_array(mysql_query("SELECT SUM(otr) AS totr,SUM(gross) AS tgross,SUM(subsidipajak) AS tsubsidipajak,SUM(matrixpajak) AS tmatrixpajak,SUM(scpahm) AS tscpahm,SUM(scpmd) AS tscpmd FROM tbl_notajual_det_vw WHERE id%2=0 AND tglnota BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' AND  tglsampai!='0000-00-00'"));
