...
?>
    ...
    <table>
										...
										if(!empty($_REQUEST[cari]))
											{
											$q1 = mysql_query("SELECT * FROM tbl_opname WHERE id%2=0 AND bulan LIKE '%$_REQUEST[cari]%' OR tahun LIKE '%$_REQUEST[cari]%'");
											}
										else
											{
			                            ...
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$dG = mysql_fetch_array(mysql_query("SELECT gudang FROM tbl_gudang WHERE id%2=0 AND id='$d1[idgudang]'"));
			                            	if(empty($dG[gudang])){
												$lokasi = "SEMUA GUDANG";
												}
											else{
