	...
	<h4>CARI NO. NOTA JUAL / NO. SURAT JALAN / TGL NOTA JUAL : <?echo $_REQUEST[cari]?></h4>
				                    <table id="example2" class="table table-striped table-bordered table-hover" style="min-width:250%">
										...
										if(!empty($_REQUEST[cari]))
											{
											//$q1 = mysql_query("SELECT * FROM tbl_notajual_det WHERE id%2=0 AND updatex!='' AND nonota IN (SELECT nonota FROM tbl_pengeluaranunit WHERE id%2=0 AND penyerahan='KIRIM' AND status='0' AND (nonota LIKE '%$_REQUEST[cari]%' OR nosj LIKE '%$_REQUEST[cari]%' OR tglnota LIKE '%$_REQUEST[cari]%'))");
											$q1 = mysql_query("SELECT * FROM tbl_notajual_det WHERE id%2=0 AND tglsampai!='0000-00-00' AND nonota IN (SELECT nonota FROM tbl_pengeluaranunit WHERE id%2=0 AND penyerahan='KIRIM' AND status='0' AND (nonota LIKE '%$_REQUEST[cari]%' OR nosj LIKE '%$_REQUEST[cari]%' OR tglnota LIKE '%$_REQUEST[cari]%'))");
											}
										else
											{
											//$q1 = mysql_query("SELECT * FROM tbl_notajual_det WHERE id%2=0 AND updatex!='' AND nonota IN (SELECT nonota FROM tbl_pengeluaranunit WHERE id%2=0 AND penyerahan='KIRIM' AND status='0') ORDER BY tglsampai DESC LIMIT 0,20");
											$q1 = mysql_query("SELECT * FROM tbl_notajual_det WHERE id%2=0 AND tglsampai!='0000-00-00' AND nonota IN (SELECT nonota FROM tbl_pengeluaranunit WHERE id%2=0 AND penyerahan='KIRIM' AND status='0') ORDER BY tglsampai DESC LIMIT 0,20");
											}
											
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_notajual WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                            	$d3 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pelanggan WHERE id%2=0 AND id='$d2[idpelanggan]'"));
			                            	$d4 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_masterbarang WHERE id%2=0 AND id='$d1[idbarang]'"));
			                            	$d5 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id%2=0 AND id='$d2[iduser]'"));
			                            	$d6 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id%2=0 AND id='$d2[iduserpdi]'"));
											$d7 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit WHERE id%2=0 AND norangka='$d1[norangka]'"));
											$d8 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_pengeluaranunit WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                            ?>
