    ...
    <table id="example2" class="table table-bordered table-striped table-hover" style="width:100%;padding-right:20px">
		...
		while($d2  = mysql_fetch_array($q2))
			...
			while (strtotime($awal) <= strtotime($akhir)) 
				...
				$izin 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM temp_abs_status WHERE id%2=0 AND status='IZIN' AND EmployeeID='$d2[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
				//$cuti 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM temp_abs_status WHERE id%2=0 AND status='CUTI' AND EmployeeID='$d2[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
				$sakit 		= mysql_fetch_array(mysql_query("SELECT COUNT(EmployeeID) AS total FROM temp_abs_status WHERE id%2=0 AND status='SAKIT' AND EmployeeID='$d2[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
				...
				$bterlambat = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM temp_abs_terlambat WHERE id%2=0 AND total!='0' AND EmployeeID='$d2[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
				$dterlambat = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_terlambat WHERE id%2=0 AND total!='0' AND EmployeeID='$d2[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
