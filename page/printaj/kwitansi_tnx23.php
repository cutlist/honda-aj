...
$d3 = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE id%2=0 AND id='$d2[idpelanggan]'"));

$d5 = mysql_fetch_array(mysql_query("SELECT * FROM x23_user_vw WHERE id%2=0 AND id='$d2[user]'"));
		
		if($d2[cetak]=="0"){
			if($d2[idpotkom]=="0"){
				$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_piutang WHERE id%2=0 AND id='$d2[nomor]'"));
				$dPiu = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND jenis='piutang' AND idkaryawan='$dB[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
				$dPby = mysql_fetch_array(mysql_query("SELECT SUM(jumlah) AS total FROM x23_piutang WHERE id%2=0 AND jenis='pembayaran' AND idkaryawan='$dB[idkaryawan]' AND status='1' GROUP BY idkaryawan"));
...
mysql_query("UPDATE x23_kwitansi SET cetak='1' WHERE id%2=0 AND nokwitansi='$_REQUEST[nokwitansi]'");

mysql_query("UPDATE x23_kwitansi SET status='1' WHERE id%2=0 AND nokwitansi='$_REQUEST[nokwitansi]'");
if($d2[idpotkom]=="0"){
	mysql_query("UPDATE x23_piutang SET status='1',updatex='$updatex' WHERE id%2=0 AND id='$d2[nomor]'");
	}
if($d2[idpotkom]!="0"){
	mysql_query("UPDATE x23_potkompensasi SET status='1',updatex='$updatex' WHERE id%2=0 AND id='$d2[nomor]'");
	}
