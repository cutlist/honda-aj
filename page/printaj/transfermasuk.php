...
$d1 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_transfer WHERE id%2=0 AND notransfer='$_REQUEST[notransfer]'"));
$dB = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty, SUM(harga) AS total FROM tbl_transfer WHERE id%2=0 AND notransfer='$_REQUEST[notransfer]' GROUP BY notransfer"));
$ppn = ROUND($dB[total] * 0.1,0);

$d2 = mysql_fetch_array(mysql_query("SELECT nama FROM tbl_pelanggan WHERE id%2=0 AND id='$d1[idasal]'"));

						$dA1 = mysql_fetch_array(mysql_query("SELECT accu FROM stok_accu WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA2 = mysql_fetch_array(mysql_query("SELECT alaskaki FROM stok_alaskaki WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA3 = mysql_fetch_array(mysql_query("SELECT anakkunci FROM stok_anakkunci WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA4 = mysql_fetch_array(mysql_query("SELECT helm FROM stok_helm WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA5 = mysql_fetch_array(mysql_query("SELECT spion FROM stok_spion WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA6 = mysql_fetch_array(mysql_query("SELECT toolkit FROM stok_toolkit WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA7 = mysql_fetch_array(mysql_query("SELECT jaket FROM stok_jaket WHERE id%2=0 AND nonota='$d1[notransfer]'"));
						$dA8 = mysql_fetch_array(mysql_query("SELECT bukuservis FROM stok_bukuservis WHERE id%2=0 AND nonota='$d1[notransfer]'"));

?>

	<div class="col-xs-12" style="text-align:center">
		<h5><b>NOTA MUTASI MASUK</br>CV ANUGRAH JAYA</b></h5>
	</div>
	</br>
	<div class="col-xs-5">
        <table width="100%">
    ...
    <div class="col-xs-12">
        <table id="example2" class="table table-striped table-bordered">
            ...
            <tbody>
            <?
			$q1 = mysql_query("SELECT * FROM tbl_transfer WHERE id%2=0 AND notransfer='$_REQUEST[notransfer]'");
            while($d1 = mysql_fetch_array($q1))
            	{
				$d2 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_stokunit_vw WHERE id%2=0 AND norangka='$d1[norangka]'"));
            ?>
            ...
            <tfoot>
            	...
            	<tr>
            		<th colspan="5"></th>
            		<th colspan="" align="right">QTY MUTASI MASUK</th>
					           			...
					           			<div class="col-xs-4">
					                        <table width="100%">
					                        	<tr>
					                        		<td width="30%">HELM</td>
					                        		<td width="3%">:</td>
					                        		<td><?echo $dA4[helm]?> PCS</td>
					                        	</tr>
					                        	<tr>
					                        		<td>BUKU SERVIS</td>
					                        		<td>:</td>
					                        		<td><?echo $dA8[bukuservis]?> PCS</td>
					                        	</tr>
					                        	<tr>
					                        		<td>SPION</td>
					                        		<td>:</td>
					                        		<td><?echo $dA5[spion]?> PCS</td>
					                        	</tr>
					                        </table>
					                    </div>
					           			<div class="col-xs-4">
					                        <table width="100%">
					                        	<tr>
					                        		<td width="30%">ACCU</td>
					                        		<td width="3%">:</td>
					                        		<td><?echo $dA1[accu]?> PCS</td>
					                        	</tr>
					                        	<tr>
					                        		<td>JAKET</td>
					                        		<td>:</td>
					                        		<td><?echo $dA7[jaket]?> PCS</td>
					                        	</tr>
					                        	<tr>
					                        		<td>TOOLKIT</td>
					                        		<td>:</td>
					                        		<td><?echo $dA6[toolkit]?> PCS</td>
					                        	</tr>
					                        </table>
					                    </div>
					           			<div class="col-xs-4">
					                        <table width="100%">
					                        	<tr>
					                        		...
					                        		<td width="3%">:</td>
					                        		<td><?echo $dA3[anakkunci]?> PCS</td>
					                        	</tr>
					                        	<tr>
					                        		<td>ALAS KAKI</td>
					                        		<td>:</td>
					                        		<td><?echo $dA2[alaskaki]?> PCS</td>
					                        	</tr>
					                        </table>
					                    </div>
