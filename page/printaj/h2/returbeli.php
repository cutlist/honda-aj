                ...
                <th>&nbsp;</th>
										...
										while($d1 = mysql_fetch_array($q1))
											...
											$d2 = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS totqty,SUM(total) AS totjum,SUM(qtykeluar) AS totqty2,SUM(totalkeluar) AS totjum2 FROM x23_returbeli_det
											                                     WHERE id%2=0 AND noretur='$d1[noretur]' AND tanggal='$d1[tanggal]'"));
											...
											$dB = mysql_fetch_array(mysql_query("SELECT SUM(stok) AS tstok FROM x23_stokpart WHERE id%2=0 AND nonota='$d1[nonota]'"));
			                            ?>
