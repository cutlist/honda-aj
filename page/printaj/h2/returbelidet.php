...
header("Content-Disposition: attachment; filename=$judul");
		...
		$d3 = mysql_fetch_array(mysql_query("SELECT * FROM x23_supplier WHERE id%2=0 AND id='$d1[idsupplier]'"));
        ...
        <thead>
            ...
            <tr>
                <th>&nbsp;</th>
			                            ...
			                            <tbody>
			                            <?
										$q1 = mysql_query("SELECT * FROM x23_returbeli_det_vw WHERE id%2=0 AND noretur='$d1[noretur]'");
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
											$dA = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli_det2_vw WHERE id%2=0 AND idbarang='$d1[idbarang]' AND idgudang='$d1[idgudang]' AND rak='$d1[rak]'"));
											$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_stokpart WHERE id%2=0 AND idbarang='$d1[idbarang]' AND idgudang='$d1[idgudang]' AND rak='$d1[rak]' AND nonota='$d1[nonota]'"));
				                           	if(empty($d2[totqty2])){
													$totqty2 = "-";
													}
												else{
