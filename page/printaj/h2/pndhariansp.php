...
<h4>PENDAPATAN HARIAN PENJUALAN BARANG PERIODE TANGGAL NOTA JUAL <?echo date("d-m-Y",strtotime($periode_awal))?> SAMPAI DENGAN <?echo date("d-m-Y",strtotime($periode_akhir))?></h4>
			                        <table id="example2" class="table table-bordered table-striped" style="width:130%">
										...
										if(empty($_REQUEST[periode]))
											{
			                            	$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND tglnota=CURDATE() AND nonota IN (SELECT nomor FROM x23_kwitansi WHERE id%2=0 AND tanggal=CURDATE() AND jnskwitansi IN ('penjualan') AND jumlah>0 AND status='1')");
											$dA = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS a,
																					SUM(diskon) AS b,
																					SUM(total) AS c
																				FROM x23_notajual_det WHERE id%2=0 AND tglnota=CURDATE() AND nonota IN (SELECT nomor FROM x23_kwitansi WHERE id%2=0 AND tanggal=CURDATE() AND jnskwitansi IN ('penjualan') AND jumlah>0 AND status='1')"));
											}
										else
											{
			                            	$q1 = mysql_query("SELECT * FROM x23_notajual_det_vw WHERE id%2=0 AND tglnota BETWEEN '$periode_awal' AND '$periode_akhir' AND nonota IN (SELECT nomor FROM x23_kwitansi WHERE id%2=0 AND tanggal BETWEEN '$periode_awal' AND '$periode_akhir' AND jnskwitansi IN ('penjualan') AND jumlah>0 AND status='1')");
											$dA = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS a,
																					SUM(diskon) AS b,
																					SUM(total) AS c
																				FROM x23_notajual_det WHERE id%2=0 AND tglnota BETWEEN '$periode_awal' AND '$periode_akhir' AND nonota IN (SELECT nomor FROM x23_kwitansi WHERE id%2=0 AND tanggal BETWEEN '$periode_awal' AND '$periode_akhir' AND jnskwitansi IN ('penjualan') AND jumlah>0 AND status='1')"));
			                            	}
