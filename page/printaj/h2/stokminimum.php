...
$d3 = mysql_fetch_array(mysql_query("SELECT nama,posisi FROM x23_user_vw WHERE id%2=0 AND id='$d1[user]'"));
	...
	<td colspan="6">TANGGAL : <?echo date("d-m-Y")?></td>
			                        ...
			                        <table id="example1" class="table table-bordered table-striped">
			                            ...
			                            while($d1 = mysql_fetch_array($q1))
			                            	{
			                            	$d2 = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id%2=0 AND id='$d1[idbarang]'"));
			                            	$d3 = mysql_fetch_array(mysql_query("SELECT SUM(stok) AS totqty FROM x23_stokpart WHERE id%2=0 AND idbarang='$d1[idbarang]' GROUP BY idbarang"));
			                            ...
			                            ?>
			                                <tr style="cursor:pointer">
			                                	...
			                                	<td align="right"><span style="padding-right:20%"><?echo number_format(round($d3[totqty]/2),"0","",".")?> PCS</span></td>
