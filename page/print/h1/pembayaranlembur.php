...
?>
	<h4>DAFTAR PEMBAYARAN UANG LEMBUR <?echo "BULAN $_REQUEST[bulan] TAHUN $_REQUEST[tahun]"?></h4>
			                        <table id="example2" class="table table-bordered table-striped table-hover">
			                            ...
			                            <tbody>
			                            <?			   
											$q1 = mysql_query("SELECT * FROM tbl_lembur_vw WHERE bulan='$_REQUEST[bulan]' AND tahun='$_REQUEST[tahun]' AND idkaryawan IN (SELECT id FROM tbl_karyawan WHERE status='AKTIF')");
