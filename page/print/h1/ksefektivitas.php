...
?>
	<h4>DAFTAR EFEKTIVITAS PROSPEK SALES <?echo "BULAN $_REQUEST[periode_bulan] TAHUN $_REQUEST[periode_tahun]"?></h4>
    <table>
			                            <thead style="color:#666;font-size:13px">
			                                <tr>
												<th style="height:45px;background:#37A58A;color:#fff;">NAMA KARYAWAN</th>
												<th style="height:45px;background:#37A58A;color:#fff;">JUMLAH PROSPEK (ORANG)</th>
												<th style="height:45px;background:#37A58A;color:#fff;">JUMLAH TINDAK LANJUT PADA PROSPEK (TINDAKAN)</th>
												<th style="height:45px;background:#37A58A;color:#fff;">JUMLAH PENJUALAN PADA PROSPEK (TRANSAKSI)</th>
			                                </tr>
			                            </thead>
			                            <tbody>
											<?
											...
											while($dA = mysql_fetch_array($qA))
												...
												$h3 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS total FROM tbl_notajual_det_vw WHERE bulan='$_REQUEST[periode_bulan]' AND tahun='$_REQUEST[periode_tahun]' AND idsales='$dA[id]' AND tglsampai!='0000-00-00' AND idpelanggan IN (SELECT idpelanggan FROM tbl_prospek WHERE bulan='$_REQUEST[periode_bulan]' AND tahun='$_REQUEST[periode_tahun]' AND idsales='$dA[id]')"));
												if(!empty($h3[total])){
