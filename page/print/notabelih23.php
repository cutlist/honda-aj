...
$d1 = mysql_fetch_array(mysql_query("SELECT * FROM x23_notabeli WHERE nonota='$_REQUEST[nonota]'"));

$d2  = mysql_fetch_array(mysql_query("SELECT SUM(qty) AS qty, SUM(hargabelibersih) AS total, SUM(total) AS gtotal FROM x23_notabeli_det WHERE nonota='$_REQUEST[nonota]'"));			                           
if($d1[idsupplier]=="1"){
...
$d3 = mysql_fetch_array(mysql_query("SELECT nama,posisi FROM x23_user_vw WHERE id='$d1[iduserbeli]'"));

$d4 = mysql_fetch_array(mysql_query("SELECT * FROM x23_supplier WHERE id='$d1[idsupplier]' ORDER BY nama"));
    ...
    <div class="col-xs-12">
	    <table width="100%" class="table table-striped table-bordered">
	        ...
	        while($dA = mysql_fetch_array($q1))
	        	{
            	$dB = mysql_fetch_array(mysql_query("SELECT * FROM x23_masterbarang WHERE id='$dA[idbarang]'"));
            	$dC = mysql_fetch_array(mysql_query("SELECT gudang FROM x23_gudang WHERE id='$dA[idgudang]'"));
	        ?>
