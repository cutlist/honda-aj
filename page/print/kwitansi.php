...
//echo $_REQUEST['nv'];
mysql_query("UPDATE tbl_kwitansi SET cetak='1',status='1' WHERE nokwitansi='$_REQUEST[nokwitansi]'");
...
$d4 = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user_vw WHERE id='$d2[user]'"));

    	if($d2[jnskwitansi]=='lunas'){
			$d5 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_notajual_det WHERE nonota='$d2[nomor]'"));
			$jenis1 = "PELUNASAN";
			$jenis = "PELUNASAN UNTUK $d5[qty] UNIT MOTOR";
			$nota  = "JUAL";
			}
		else if($d2[jnskwitansi]=='umuka'){
			$d5 = mysql_fetch_array(mysql_query("SELECT COUNT(id) AS qty FROM tbl_pesanan_det WHERE nopesan='$d2[nomor]'"));
			mysql_query("UPDATE tbl_hutitipan SET status='1' WHERE nopesan='$d2[nomor]'");
			$jenis1 = "UANG MUKA";
