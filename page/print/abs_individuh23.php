...
header("Content-Disposition: attachment; filename=$judul");
 ...
 $dnk = mysql_fetch_array(mysql_query("SELECT nama FROM x23_karyawan WHERE nik='$_REQUEST[EmployeeID]'"));
	...
	<h4>NAMA KARYAWAN : <?echo $dnk[nama]?></h4>
	            <table>
	                ...
	                //echo  $awal.$akhir;
						...
						$dNm = mysql_fetch_array(mysql_query("SELECT * FROM x23_karyawan WHERE nik='$_REQUEST[EmployeeID]'"));
							echo"
	            ...
	            <div style="border-bottom:1px #aaa dashed;margin: 10px 0 10px"></div>
			                        ...
			                        $dterlambat = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_x23_terlambat WHERE EmployeeID='$_REQUEST[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
			                        $dovertime  = mysql_fetch_array(mysql_query("SELECT SUM(total) AS total FROM temp_abs_x23_overtime WHERE EmployeeID='$_REQUEST[EmployeeID]' AND tgl BETWEEN '$_SESSION[periode_awal]' AND '$_SESSION[periode_akhir]' GROUP BY EmployeeID"));
									?>
	            	<table width="100%">
	            		<tr>
